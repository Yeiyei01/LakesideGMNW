...
function commandsHelp()
	...
	if (loggedIn == 1) then
		if (myCommandsWindow == nil) then
			...
			myCommandsWindow = guiCreateWindow (0.15, 0.15, 0.7, 0.7, "Índice Comandos de Jugador v4 (10/8/2021)", true)
			guiWindowSetSizable(myCommandsWindow, false)
			local tabPanel = guiCreateTabPanel (0, 0.1, 1, 1, true, myCommandsWindow)
			local tlBackButton = guiCreateButton(0.8, 0.04, 0.2, 0.06, "Cerrar", true, myCommandsWindow) -- close button
			...
			local commands =
			{
				...
				{
					name = "Chat",
					{ "'u'", "Utiliza 'u' [Texto]", "Quickly reply a PM.", "'u' Hello, my name is Jack. Who are you?" },
					{ "'t'", "Utiliza 't' [IC Texto]", "This is the local in character chat.", "'t' Hello, my name is Jack. Who are you?" },
					{ "'y' or /r", "/r [IC Texto]", "This can be used by people who have a radio and are on the same frequency.", "/r What is your position? Over." },
					{ "/tuneradio", "/tuneradio [Radio id] [Frecuencia]", "Tunes a radio on a frequency, allowing you to speak with everyone using the same frequency. If no radio id is specified, the first radio is tuned.", "/tuneradio 100" },
					{ "/toggleradio", "/toggleradio [slot]", "Turns your radio on that slot (or if not specified, all radios you have) on/off.", "/toggleradio" },
					{ "'b' or /b", "/b [OOC Texto]", "This is local out of character chat.", "/b Hey, what's going on around here?" },
					{ "/ooc", "/ooc [OOC Texto]", "This is the global out of character chat.", "/ooc How is everyone?" },
					{ "/toggleooc", "/toggleooc", "Toggles global out of character chat.", "/toggleooc" },
					{ "/me", "/me [IC Accion]", "Use this to simulate actions of your character.", "/me shakes the man's hand." },
					{ "/do", "/do [IC Entorno]", "This is used to simulate events and the surroundings. Similar to /me.", "/do The engine breaks down." },
					{ "/pm", "/pm [Usuario] [OOC Texto]", "Sends an out of character message to another player.", "/pm John_Doe Thanks for helping me." },
					{ "/togpm", "/togpm", "Toggles your PMs on or off. You can still recieve PMs from admins or friends. Only for Donators.", "/togpm" },
					{ "/s", "/s [IC Texto]", "This is used to simulate your player shouting.", "/s Help! The man stole my wallet!" },
					{ "/f", "/f [OOC Texto]", "This is out of character faction chat to organize roleplay. Can only be used by factions (LSPD, ...)", "/f How are my fellow faction members doing?" },
					{ "/m", "/m [IC Texto]", "This allows you to speak to people in a wide radius with a megaphone.", "/m LSPD! Pull over your Vehicle!" },
					{ "/w", "/w [Usuario] [IC Texto]", "Use this to whisper to a player close to you. Only the two of you can see it.", "/w Jack_Konstantine He's looking right at me." },
					{ "/cw", "/cw [IC Texto]", "Use this to whisper to all players in a vehicle with you. Only you and the other occupants can see it.", "/cw Keep an eye out. I'll be right back." },
					{ "/c", "/c [IC Texto]", "Use this to talk quietly to all the players around you.", "/c He is walking right over here." },
					{ "/d or /department", "/d [IC Texto]", "Department Radio for LSPD, SPD, LSES, Gov and Hex Towing.", "" },
					{ "/gov", "/gov [IC Texto]", "In Character Government announcement and news.", "/gov Idlewood Gas is currently closed." },
					{ "/district", "/district [IC Texto]", "Use this to talk to players in the same area of the map.", "/district Loud gunshots could be heard from around El Corona." },
					{ "/highlight", "/highlight [Usuario] [r] [g] [b]", "Highlights all in character messages of your chosen people.", "/highlight Liza_Hawkins 0 255 255" },
				},
				{
					name = "Factions",
