...
function showStaff( thePlayer, commandName )
	...
	if(logged==1) then
		...
		if #admins == 0 then
			if isOverlayDisabled then
				outputChatBox("-    Actualmente no hay Administradores en línea.", thePlayer)
			else
				table.insert(info, {"-    Actualmente no hay Administradores en línea.", 255, 255, 255, 255, 1, "default"})
			end
		end
	...
	--[[if(logged==1) then
		...
		admins = {}
		if isOverlayDisabled then
			outputChatBox("MODERATORS:", thePlayer, 255, 194, 14)
		else
			table.insert(info, {"Moderator Team:", 255, 194, 14, 255, 1, "title"})
			table.insert(info, {""})
		end
		for k, arrayPlayer in ipairs(players) do
			local logged = getElementData(arrayPlayer, "loggedin")
		...
		if #admins == 0 then
			if isOverlayDisabled then
				outputChatBox("-    Actualmente no hay Moderadores en línea.", thePlayer)
			else
				table.insert(info, {"-    Actualmente no hay Moderadores en línea.", 255, 255, 255, 255, 1, "default"})
			end
		end
	...
	if(logged==1) then
		...
		if counter == 0 then
			if isOverlayDisabled then
				outputChatBox("-    Actualmente no hay miembros en línea.", thePlayer)
			else
				table.insert(info, {"-    Actualmente no hay miembros en línea.", 255, 255, 255, 255, 1, "default"})
			end
		end
		...
		if counter == 0 then
			if isOverlayDisabled then
				outputChatBox("-    Actualmente no hay Scripters en línea.", thePlayer)
			else
				table.insert(info, {"-    Actualmente no hay Scripters en línea.", 255, 255, 255, 255, 1, "default"})
			end
		end
...
function toggleOverlay(thePlayer, commandName)
	if getElementData(thePlayer, "hud:isOverlayDisabled") then
		...
		outputChatBox("Has activado los menús superpuestos.",thePlayer)
	else
		setElementData(thePlayer, "hud:isOverlayDisabled", true)
		outputChatBox("Has desactivado los menús superpuestos.", thePlayer)
	end
end
