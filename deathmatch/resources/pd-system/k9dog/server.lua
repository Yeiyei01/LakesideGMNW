...
addCommandHandler("k9status", function (p)
	...
	if currentStatus == 1 then
		...
		setElementData(ped[p], "k9:sits", false)
		outputChatBox("El perro está ahora aguantando.", p)
	elseif currentStatus == 0 then
		setElementData(ped[p], "k9:status", 1)
		outputChatBox("El perro ahora te sigue.", p)
	else
		setElementData(ped[p], "k9:status", 1)
		setElementData(ped[p], "k9:sits", false)
		outputChatBox("El perro ahora te sigue.", p)
...
addCommandHandler("k9attack", function (p, cmd, target)
	...
	if not target then
		outputChatBox("SYNTAX: /"..cmd.." [Objetivo]", p)
		return
	end
	...
	if isElement(targetPlayer) then
		...
		if getDistanceBetweenPoints3D(x,y,z,tx,ty,tz) > 500 then
			outputChatBox("Su perro no logra detectar un objetivo.",p)
			triggerEvent("sendAme", ped[p], "mira a su alrededor, pareciendo confundido.")
			return false
		end

		setElementData(ped[p], "k9:status", 2)
		setElementData(ped[p], "k9:target", targetPlayer)
		outputChatBox("El perro está ahora en contra de "..targetPlayerName, p)
	else
		outputChatBox("No se ha encontrado al jugador.", p, 255, 0, 0)
	end
end)
