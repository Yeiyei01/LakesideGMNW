        ...
        if (fID == 1) or (fID == 50) then
            if exports.global:hasItem(vehicle, 284) then
                if getElementData(vehicle, "anpr:state") then
                    ...
                    outputChatBox("El sistema ANPR ha sido desactivado en este vehiculo.", player, 255, 180, 20, false)
                    ...
                    if not isANPRExisting(player) then
                        ...
                        outputChatBox("Ha activado el sistema ANPR.", player, 255, 180, 20, false)
                ...
                outputChatBox("Debe estar en un vehículo de las fuerzas del orden con un sistema ANPR.", player, 255, 0, 0, false)    
            end    
        else
...
addCommandHandler({"resetanpr", "resetanprcam", "resetanprcamera"},
	function(player, cmd)
		if isANPRExisting(player) then
			...
			outputChatBox("No hay ningún sistema ANPR activo.", player, 255, 0, 0, false)
		end
	end, false, false
)
                        ...
                        if isVerifiedElement(radar, "vehicle") and getElementData(radar, "anpr:state") then
                            ...
                            if hasVehicleANPR(hitElement) then
                                for seat,player in pairs(getVehicleOccupants(radar)) do
                                    ...
                                    outputChatBox("[RADAR] Se registró un resultado de ANPR en un " .. getColorName(c1, c2) .. " " .. exports.global:getVehicleName(hitElement).." with plates "..getElementData(hitElement, "plate")..".", player, 255, 0, 0, false)
...
addEventHandler("onPlayerVehicleExit", root,
	function(vehicle, seat, jacked)
		if getElementData(vehicle, "anpr:state") and (seat == 0) then
			...
			outputChatBox("El sistema ANPR ha sido desactivado.", source, 255, 180, 20, false)
		end
	end
)
	...
	function()
		for _,v in ipairs(getElementsByType("vehicle")) do
			if getElementData(v, "anpr:state") then
				...
				setElementData(source, "anpr:vehicleColor", "No info", true )
				setElementData(source, "anpr:vehicleName", "No info", true )
				outputChatBox("El sistema ANPR ha sido desactivado.", getVehicleController(v), 255, 180, 20, false)
			end
		end
