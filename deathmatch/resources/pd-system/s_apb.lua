		...
		if (exports.factions:isInFactionType(thePlayer, 2) or exports.factions:isInFactionType(thePlayer, 3)) then
			...
			while true do
				local row = mysql:fetch_assoc( mQuery )
					...
					outputChatBox("ID: "..tostring(row["id"]).." Emitido por "..issuerName.." at "..row["newtime"], thePlayer)
					outputChatBox(" ", thePlayer)
				end
		...
		if (factionType==2) or (factionType==3) then
			if not  (...) then
				...
				for key, value in ipairs(teamPlayers) do
					outputChatBox("" .. factionRankTitle .. " " .. username .." acaba de añadir un nuevo APB.", value, 0, 102, 255)
				end
			end
		end
	end
	...
	if (logged==1) then
		...
		if ftype == 3 or ftype == 2 then
			if not id or not (tonumber(id)) or (tonumber(id) < 2) then
				...
				for key, value in ipairs(teamPlayers) do
					outputChatBox("" .. factionRankTitle .. " " .. username .." acaba de añadir un nuevo APB. " .. mysql:escape_string(id) .. ".", value, 0, 102, 255)
				end
			end
		end
	end
