local backupColours = {
	...
	{ 0, 255, 0, "Verde", false },
	{ 0, 0, 255, "Azul", false },
	{ 255, 255, 0, "Amarillo", false },
	{ 255, 0, 255, "Rosado", false },
	{ 0, 255, 255, "Azul Claro", false },
	{ 255, 255, 255, "Blanco", false }
}
	...
	--if getPedOccupiedVehicle(thePlayer) then
		...
		if veh then
			...
			if veh and vehiclesDashboardRadio[model] then
				triggerEvent('sendAme', thePlayer, "pulsa un interruptor en su radio del salpicadero.")
			else
				triggerEvent('sendAme', thePlayer, "acciona un interruptor en su radio.")
			end
		else
			triggerEvent('sendAme', thePlayer, "acciona un interruptor en su radio.")
		end
				...
				if pTheTeam then
					...
					if allowedFactionTypes[pFactionType]  then
						if (faction > 0) then
							...
							outputChatBox("El '".. backupColours[availableColourIndex][4]  .."' de (" .. factionShortName ..") ya no requiere asistencia. Reanudar la patrulla normal.", theArrayPlayer, 255, 194, 14)
						end
					end	
				end
			end	
				...
				if pTheTeam then
					...
					if allowedFactionTypes[pFactionType]  then
						if (faction > 0) then
							...
							outputChatBox("¡El '".. backupColours[availableColourIndex][4]  .."' de (" .. factionShortName ..") ha habilitado su radiofaro de apoyo!", theArrayPlayer, 255, 194, 14)
						end
					end	
				end
			end		
		else
			outputChatBox("Todas las balizas de reserva ya están en uso.", thePlayer, 255, 194, 14)
		end
	--end
	end
end
addCommandHandler("respaldo", startBackup)
addCommandHandler("res", startBackup)
addCommandHandler("referencia", startBackup)
addCommandHandler("ref", startBackup)
