	...
	if (logged==1) then
		...
		if (factionType==2) and playerCol then
			if not (targetPlayerNick) or not (fine) or not (jailtime) or not (...) or (jailtime<1) or (jailtime>1441) then
				outputChatBox("SYNTAX: /arrestar [Nombre] [Multa] [Tiempo (Minutos 1->1440)] [Crimenes]", thePlayer, 255, 194, 14)
			else
				...
				if targetPlayer then
					...
					if not targetCol then
						outputChatBox("El jugador no está dentro del alcance del mostrador de reservas.", thePlayer, 255, 0, 0)
					elseif targetCol ~= playerCol then
						outputChatBox("El jugador está de pie frente a otra celda.", thePlayer, 255, 0, 0)
					else
						...
						if (jailTimer) then
							...
							outputChatBox("Fuiste encarcelado por " .. targetPlayerNick .. " durante " .. jailtime .. " minutos.", thePlayer, 255, 0, 0)
							...
							outputChatBox("Fuiste arrestado por " .. username .. " durante " .. jailtime .. " minuto(s).", targetPlayer, 0, 102, 255)
							outputChatBox("Crímenes Cometidos: " .. reason .. ".", targetPlayer, 0, 102, 255)
							if (finebank == true) then
								outputChatBox("El resto de la multa se ha retirado de su cuenta bancaria.", targetPlayer, 0, 102, 255)
							end

							for key, value in ipairs(teamPlayers) do
								outputChatBox(factionRankTitle .. " " .. username .. " arresto a " .. targetPlayerNick .. " durante " .. jailtime .. " minuto(s).", value, 0, 102, 255)
								outputChatBox("Delitos Cometidos: " .. reason .. ".", value, 0, 102, 255)
							end
...
function showJailtime(thePlayer, commandName)
	...
	if ajailtime then
		outputChatBox("Usted tiene " .. ajailtime .. " minutos restantes en su cárcel de administración.", thePlayer, 255, 194, 14)
	else
		outputChatBox("No estás en la cárcel de la administración.", thePlayer, 255, 0, 0)
	end

	--[[local isJailed = getElementData(thePlayer, "pd.jailtimer")
	if isJailed then
		local jailtime = getElementData(thePlayer, "pd.jailtime")
		outputChatBox("Usted tiene " .. jailtime .. " minutos restantes de su arresto.", thePlayer, 255, 194, 14)
	else
		outputChatBox("No está detenido.", thePlayer, 255, 0, 0)
	end--]]
end
	...
	if (logged==1) then
		...
		if factionType == 2 and isInArrestColshape(thePlayer) or exports.integration:isPlayerTrialAdmin(thePlayer) and getElementData(thePlayer, "duty_admin") == 1 then
			if not (targetPlayerNick) then
				outputChatBox("SYNTAX: /release [Nombre]", thePlayer, 255, 194, 14)
			else
				local targetPlayer, targetPlayerNick = exports.global:findPlayerByPartialNick(thePlayer, targetPlayerNick)
				...
				if targetPlayer then
					...
					if (jailTimer) then
						...
						outputChatBox("Este jugador no está cumpliendo una pena de cárcel.", thePlayer, 255, 0, 0)
					end
				end
			end
		end
