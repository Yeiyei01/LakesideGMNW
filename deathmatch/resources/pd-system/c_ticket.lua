...
function getShortTime(timestamp)
	local months = { "ENE", "FEB", "MAR", "ABR", "MAY", "JUN", "JUL", "AUG", "SEP", "OCT", "NOV", "DIC" }
...
addEventHandler("showPayGUI", getRootElement(), function(tickets)
	if GUIEditor.window[2] and isElement(GUIEditor.window[2]) then
        ...
        GUIEditor.window[2] = guiCreateWindow(501, 210, 637, 342, "Multas Pendientes", false)
        ...
        guiGridListAddColumn(GUIEditor.gridlist[1], "Fecha Emisión", 0.25)
        guiGridListAddColumn(GUIEditor.gridlist[1], "Infracciones", 0.5)
        guiGridListAddColumn(GUIEditor.gridlist[1], "Total", 0.22)
        ...
        GUIEditor.button[5] = guiCreateButton(399, 301, 109, 29, "Pagar via Banco", false, GUIEditor.window[2])
				...
				if row > 0 then
					...
					if source == GUIEditor.button[4] then -- cash
						if toPay > getElementData(localPlayer, "money") then
							outputChatBox("No tiene suficiente dinero para pagar en efectivo.", 255, 0, 0)
							return false
						end
						...
						if toPay > getElementData(localPlayer, "bankmoney") then
							outputChatBox("No tiene suficiente dinero para pagar por banco.", 255, 0, 0)
							return false
						end
						triggerServerEvent("chargePlayer", localPlayer, toPay, 2, tickets[row][1])
						triggerServerEvent("sendAmeClient", localPlayer, "saca su tarjeta de cajero automático del bolsillo y la introduce en el cajero.")
					end
...
addEventHandler("showTicketGUI", getRootElement(), function(vehicle)
	if GUIEditor.window[1] and isElement(GUIEditor.window[1]) then
		...
		GUIEditor.window[1] = guiCreateWindow(1075, 186, 377, 332, "Libro de Multas", false)
		exports.global:centerWindow( GUIEditor.window[1] )
		guiWindowSetSizable(GUIEditor.window[1], false)

		GUIEditor.label[5] = guiCreateLabel(38, 30, 104, 15, "Multa Emitida a:", false, GUIEditor.window[1])
        ...
        GUIEditor.label[1] = guiCreateLabel(15, 61, 143, 18, "Matricula:", false, GUIEditor.window[1])
        GUIEditor.radiobutton[2] = guiCreateRadioButton(152, 30, 71, 15, "Vehicle", false, GUIEditor.window[1])
        GUIEditor.label[2] = guiCreateLabel(200, 61, 143, 18, "Total:", false, GUIEditor.window[1])
        ...
        GUIEditor.label[4] = guiCreateLabel(15, 165, 143, 18, "Infracciones:", false, GUIEditor.window[1])
		...
		addEventHandler("onClientGUIClick", GUIEditor.window[1], function()
			...
			elseif source == GUIEditor.button[2] then
				if not tonumber(guiGetText(GUIEditor.edit[2])) then
					outputChatBox("No ha introducido el importe de la multa.", 255, 0, 0)
				elseif tonumber(guiGetText(GUIEditor.edit[2])) < 50 or tonumber(guiGetText(GUIEditor.edit[2])) > 10000 then
					outputChatBox("Sólo se puede multar a alguien por entre 50 y 10.000 dólares.", 255, 0, 0)
				elseif string.len(guiGetText(GUIEditor.memo[1])) > 100 then
					...
					if guiRadioButtonGetSelected(GUIEditor.radiobutton[1]) then
						...
						if not found then
							outputChatBox("No se ha podido encontrar al jugador " .. guiGetText(GUIEditor.edit[1]) .. ".", 255, 0, 0)
							return false
						end
						...
						if getElementData(found, "faction") > 0 or getElementData(found, "job") > 0 then
							outputChatBox("No puedes multar a un vehículo de facción o de trabajo civil.", 255, 0, 0)
							return false
						end
