...
--Faction IDs that can use thermal vision. If vehicle belongs to the faction and player is on duty for that faction, vision will show.
local factionsWithVision = {
		[1] = true, --Sheriff's Department
		[2] = true, --Fire Department
	}
--Vehicle IDs (VINs) that can use thermal vision. Independant of MTA vehicle model and faction.
local vehicleVINsWithVision = {
		
	}
--MTA vehicle model IDs that can use thermal vision.
local vehicleModelsWithVision = {
		[417] = true, --Leviathan
		[487] = true, --Maverick
...
function doVision()
	...
	if getVehicleEngineState(tV) then
			...
			if isElementStreamedIn(value) and (isElementOnScreen(value)) and (tV~=value) then
				...
				if (isLineOfSightClear(px, py, pz, x, y, z, true, false, false, true, false, false, true, tV)) then
					...
					if (tx) then
						...
						local plate = getElementData(value, "show_plate") == 1 and ("Matricula: "..getElementData(value,"plate").." | ") or ""
...
function applyVision(thePlayer, seat, setVehicle)
	...
	if (thePlayer==localPlayer) then
		...
		if(vehicleVINsWithVision[vehID]) then
			...
			if(vehicleModelsWithVision[model]) then
				local vehFaction = getElementData(source, "faction")
				if(factionsWithVision[vehFaction]) then
					--local playerFaction = exports.factions:getCurrentFactionDuty(thePlayer)
					--if(playerFaction == vehFaction)
						--if (seat == 0 or seat == 1) then
							isVisionActive = true
							addEventHandler("onClientRender", getRootElement(), doVision)
						--end
