		...
		if (vehicle) then
			...
			if (getVehicleType(vehicle) == "Boat" or getVehicleType(vehicle) == "Plane" or getVehicleType(vehicle) == "Helicopter") and getElementModel(vehicle) ~= 539 then
				...
				if(getVehicleType(vehicle) == "Plane" or getVehicleType(vehicle) == "Helicopter") then
					...
					callsign = "CODIGO: "..tostring(callsign)

					local airinfo = "ALT: "..tostring(altitude).."    LAT: "..tostring(headingString)
			...
			dxDrawText( "Kilometraje: "..tostring(math.floor(getDistanceTraveled()/1000)), x - 150, y - 215, x-70, 5, tocolor (255,255,255, 200), 1, 'default', 'center' )
			dxDrawText( "Bateria: "..tostring(math.floor( getElementData( vehicle, 'battery' ) or 100 )).."%", x - 135, y - 145, x-70, 5, tocolor (255,255,255, 200), 1, 'default', 'center' )
		end
	end
end
