...
addCommandHandler("separar", unhookTrailer)
...
function seatbelt(thePlayer)
	if getPedOccupiedVehicle(thePlayer) and getElementData(thePlayer, "realinvehicle") == 1 then
		...
		if (getVehicleType(theVehicle) == "BMX") or (noBelt[getElementModel(theVehicle)] and getVehicleOccupant(theVehicle, 0) ~= thePlayer) then
			outputChatBox("Extraño... ¡No hay cinturón de seguridad en este vehículo!", thePlayer, 255, 0, 0)
		else
			local helmet = false
			if (getElementData(thePlayer, "seatbelt") == true) then
				if getVehicleType(theVehicle) ~= "Bike" then
					...
					outputChatBox("Te desabrochaste el cinturón de seguridad.", thePlayer, 255, 0, 0)
					triggerEvent("sendAme", thePlayer, "se desabrocha el cinturón de seguridad.")
				elseif getVehicleType(theVehicle) == "Bike" then
					...
					if helmet then
						triggerEvent("sendAme", thePlayer, "se quita el casco.")
				...
				if getVehicleType(theVehicle) ~= "Bike" then
					...
					outputChatBox("Te abrochaste el cinturón de seguridad.", thePlayer, 0, 255, 0)
					triggerEvent("sendAme", thePlayer, "se abrocha el cinturón de seguridad.")
				elseif getVehicleType(theVehicle) == "Bike" then
					...
					if helmet then
						exports.anticheat:changeProtectedElementDataEx(thePlayer, "seatbelt", true, true)
						triggerEvent("sendAme", thePlayer, "se pone el casco.")
					end
				end
			end
		end
...
addCommandHandler("cinturon", seatbelt)
...
addEventHandler("onVehicleStartExit", root, function(player)
	if getVehicleType(source) == "Bike" then
		if bikers[player] then
			...
			triggerEvent("sendAme", player, "se quita el casco.")
...
function addSeatbelt(thePlayer)
	...
	if helmet then
		exports.anticheat:changeProtectedElementDataEx(thePlayer, "seatbelt", true, true)
		bikers[thePlayer] = {thePlayer, v, value}
		triggerEvent("sendAme", thePlayer, "asegura su casco.")
	end
end
