	...
	function ()
											...
											if tx then
												if i == 4 then
													if holder == localPlayer then
														outputChatBox("El cable no puede pasar por aquí", 255, 255, 255, true)
														exports.infoboxebertz:addBox ( source, "El cable no puede pasar por aquí", "error")
													end
		...
		if isElement(myTowTruck) then
			...
			if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) > 20 then
				outputChatBox("El cable no puede llegar tan lejos.", 255, 0, 0, true)
				...
				if state ~= "up" then
					...
					for k, v in pairs(keys) do
						if v == key then
							...
							if press then
								outputChatBox("Primero tira de la rampa.", 255, 255, 255, true)
								exports.infoboxebertz:addBox ( source, "Tire de la rampa primero", "error")
							end
						end
					end
				end
						...
						if not rampInAnim[veh] then
							if key == "cardown" then
								if getElementData(veh, "packerState") == "down" then
									...
									outputChatBox("Baja la rampa primero.", 255, 255, 255, true)
									exports.infoboxebertz:addBox ( source, "Primeiro tire a rampa!", "error")
								end
							elseif key == "carup" then
								if getElementData(veh, "packerState") == "down" then
									...
									outputChatBox("Primeiro tire a rampa.", 255, 255, 255, true)
									exports.infoboxebertz:addBox ( source, "Baja la rampa primero", "error")
								end
							...
							elseif key == "winch" then
								if getElementData(veh, "packerState") == "down" then
									...
									outputChatBox("Baja la rampa primero", 255, 255, 255, true)
									exports.infoboxebertz:addBox ( source, "Baje la rampa primero", "error")
								end
							else
									...
									if getVehicleType(clickedElement) == "Automobile" then
										...
										if getDistanceBetweenPoints3D(px, py, pz, tx, ty, tz) <= 5 then
											...
											if isInsideColShape(detectorColShape, x, y, z) and dist <= 2.5 then
												...
												outputChatBox("O carro não está atrás do Truck!", 255, 255, 255, true)
												exports.infoboxebertz:addBox ( source, "El vehiculo no está atras del auto", "error")
											end
										else
											outputChatBox("Voce esta longe do veiculo!", 255, 255, 255, true)
											exports.infoboxebertz:addBox ( source, "¡Estás lejos del vehículo!", "error")
										end
									else
										outputChatBox("Solo puedes conectar un vehiculo", 255, 255, 255, true)
										exports.infoboxebertz:addBox ( source, "Solo puedes conectar un vehiculo", "error")
									end
								end
							end
						end
