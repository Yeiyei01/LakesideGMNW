...
function carshop_buyVehicle(paymentMethod)
	...
	if not exports.global:canPlayerBuyVehicle(client) then
		if isOverlayDisabled then
			...
			exports.hud:sendBottomNotification(client, "Límite máximo de vehículos", "Ya ha alcanzado el número máximo de vehículos. /stats para detalles.")
		end
		return false
	end
	...
	if (paymentMethod == "cash") then
		if not exports.global:hasMoney(client, costCar) or costCar == 0 then
			if isOverlayDisabled then
				outputChatBox("No tienes suficiente dinero a mano para este auto..", client, 0, 255, 0)
			else
				exports.hud:sendBottomNotification(client, "El dinero siempre es un problema...", "No tienes suficiente dinero a mano para este auto...")
			end
			return false
		else
	...
	elseif (paymentMethod == "bank") then
		...
		if money < 0 or costCar == 0 then
			if isOverlayDisabled then
				outputChatBox("No tienes suficiente dinero en tu cuenta bancaria para este auto...", client, 0, 255, 0)
			else
				exports.hud:sendBottomNotification(client, "El dinero siempre es un problema...", "No tienes suficiente dinero en tu cuenta bancaria para este auto...")
			end
			return false
		else
	...
	elseif (paymentMethod == "token") then
		if costCar <= 35000 then
			if not exports.global:takeItem(client, 263) then
				outputChatBox("Hubo un problema al procesar su solicitud.", client, 255, 0 ,0)
			end
		else
			outputChatBox("No se puede utilizar una ficha en un coche de más de $35,000.", client, 255, 0, 0)
		end
	else
		if isOverlayDisabled then
			outputChatBox("No.", client, 0, 255, 0)
		else
			exports.hud:sendBottomNotification(client, "No!", "Simplemente no...")
		end
		return false
	end
	...
	if result then
		if tonumber(result["stock"]) < 1 then
			if isOverlayDisabled then
				outputChatBox("Lo siento, parece que este está cogido.", client, 0, 255, 0)
			else
				exports.hud:sendBottomNotification(client, "Agotado", "Lo sentimos, parece que este modelo está actualmente agotado.")
			end
			return false
		end
	end
	...
	local license = (getElementData(client, "license.car") == 1) and "" or "No tienes licencia de conducir. Será mejor que no conduzcas esto en la calle."

	if isOverlayDisabled then
		outputChatBox("Felicitaciones, ha comprado un vehiculo", client)
		outputChatBox("Asegúrate de aparcarlo con /park en el lugar que quieras antes de una hora,", client)
		outputChatBox("de lo contrario, el vehículo será eliminado, no recuperable.", client)
		outputChatBox(license, client)
		outputChatBox("Edita la descripcion del vehiculo con /ed o /editdescription.", client)
	else
		local content = {}
		table.insert(content, {"Recuerda usar /park para estacionarlo y que cuando te respawnee aparezca donde pusiste park."})
		table.insert(content, {license})
		table.insert(content, {"Edita la descripcion del vehiculo con /ed o /editdescription."})

		exports.hud:sendBottomNotification(client, "Felicitaciones, ha comprado un vehiculo", content, 30)
	end
		...
		if shopname and shopnicename and vehicle_shop_id and vehYear and vehBrand and vehModel and vehPrice then
			...
			if orderedVehID and tonumber(orderedVehID) == tonumber(vehicle_shop_id) then
				if exports.global:hasItem(player, 2) then
					...
					outputChatBox("["..languagename.."] SMS de "..shopnicename..": ¡Hola! Como ha pedido, ahora tenemos un "..itemName.." en stock por $"..exports.global:formatMoney(vehPrice)..". Ven para verlo" , player, 120, 255, 80)
				end
			end
		end
	end
