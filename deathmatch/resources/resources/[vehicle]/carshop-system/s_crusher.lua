...
local cOutsideCol = createColCuboid(168.505859375, -318.62890625, 1.7529792785645, 15.5, 15.5, 6)
setElementID(cOutsideCol, 'crusher')
createBlip(168.505859375, -318.62890625, 1.7529792785645, 11, 2, 255, 0, 0, 255, 0, 300) -- Crusher
...
function resetPrice(theVehicle, matching)
	if isElement(theVehicle) and getElementType(theVehicle) == "vehicle" then
		if getElementData(theVehicle, "crushing") then
			...
			if thePlayer then
				outputChatBox("Vuelve cuando hayas pensado en ello", thePlayer, 255, 194, 14)
		...
		if theVehicle and getVehicleOccupant(theVehicle) == thePlayer then -- player is driver
			if getElementData(theVehicle, "owner") == getElementData(thePlayer, "dbid") then
				...
				elseif price == 0 and not getElementData(theVehicle, "token") then
					outputChatBox("Este " .. exports.global:getVehicleName(theVehicle) .. " no vale nada.", thePlayer, 255, 0, 0 )
					outputChatBox("(( Contacte con un administrador para vender su vehículo especial. ))", thePlayer, 255, 194, 14)
				elseif getVehicleType(theVehicle) == "Trailer" then
					outputChatBox("Este " .. exports.global:getVehicleName(theVehicle) .. " no puede ser desguazado.", thePlayer, 255, 0, 0 )
					outputChatBox("(( Contacte con un administrador para vender su vehículo especial. ))", thePlayer, 255, 194, 14)
				else
