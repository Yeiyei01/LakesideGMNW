...
function changeSpeedradar(player, commandName)
	if isCameraExisting(player) then
		if getElementData(player, "speedradar_chat") then
			...
			outputChatBox("Speedcam cambiado a GUI", player, 255, 180, 20)
			...
			outputChatBox("Cambio del radar por el chatbox", player, 255, 180, 20)
		end
	else
		outputChatBox("Es necesario tener un radar para poder utilizar este comando", player, 255, 0, 0)
	end	
end
				...
				if getElementData(vehicle, "speedcamera:state") then
					...
					if not isCameraExisting(player) then
						...
						if speed then
							...
							outputChatBox("SYNTAX: /" .. cmd .. " [30 o más]", player, 255, 180, 20, false)
						end
					else
						outputChatBox("Tiene que desactivar el radar existente.", player, 255, 0, 0, false)
					end
				end
			else
				outputChatBox("Debe estar en un vehículo de las fuerzas del orden para activar el radar de velocidad.", player, 255, 0, 0, false)
			end
		end
	end, false, false
)
...
addCommandHandler({"resetspeed", "resetspeedcam", "resetspeedcamera"},
	function(player, cmd)
		if isCameraExisting(player) then
			...
			setElementData(player, "speedcamera:vehicleName", "No info", true )
			setElementData(player, "speedcamera:vehicleDirection", "No info", true )
			outputChatBox("Todos sus radares de velocidad están ahora desactivados.", player, 255, 180, 20, false)
		else
			outputChatBox("No existen radares de velocidad.", player, 255, 0, 0, false)
		end
	end, false, false
)
							...
							setTimer(function(hitElement, x, y, z, kmh)
								...
								if isVerifiedElement(radar, "vehicle") and getElementData(radar, "speedcamera:state") then
									...
									for seat,player in pairs(getVehicleOccupants(radar)) do
										...
										if getElementData(player, "speedradar_chat") then
											outputChatBox("[RADAR] Un " .. getColorName(c1, c2) .. " " .. exports.global:getVehicleName(hitElement) .. " fue cronometrado viajando a " .. math.floor(tonumber(kmh) * 0.621371) .. " mph (" .. tonumber(kmh) .. " km/h) y se dirige a " .. direction .. ".", player, 255, 180, 20, false)										
										else
											outputConsole( "[RADAR] Un " .. getColorName(c1, c2) .. " " .. exports.global:getVehicleName(hitElement) .. " fue cronometrado viajando a " .. math.floor(tonumber(kmh) * 0.621371) .. " mph (" .. tonumber(kmh) .. " km/h) y se dirige a " .. direction .. ".", player )
										end
									end
								end
...
addEventHandler("onPlayerVehicleExit", root,
	function(vehicle, seat, jacked)
		if getElementData(vehicle, "speedcamera:state") and (seat == 0) then
			...
			outputChatBox("El radar de velocidad ha sido desactivado.", source, 255, 180, 20, false)
