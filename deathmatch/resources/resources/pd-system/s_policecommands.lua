			...
			if not (targetPlayerNick) then
				...
				if targetPlayer then
					...
					if (distance<=10) then
						...
						outputChatBox(targetPlayerName .. "'s Huella dactilar: " .. tostring(fingerprint) .. ".", thePlayer, 255, 194, 14)
					else
...
addCommandHandler("huelladactilar", fingerprintPlayer, false, false)
	...
	if faction then
		...
		if (factionType==2) and (leader) then
			...
			if exports.global:hasItem(targetVehicle, 138) then
				...
				exports.global:sendLocalDoAction(targetPlayer, "oiría un silencio repentino y un ligero clic cuando algunos cerrojos se deslizarían, bloqueando la puerta en su lugar.")
			else
				outputChatBox("Este vehículo no tiene el sistema antirrobo.", thePlayer, 255, 0, 0)
			end
		end
	end
end
