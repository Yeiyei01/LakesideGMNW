...
function makeGUI()
	if not getPedOccupiedVehicle(localPlayer) then
		outputChatBox( "Este comando no funciona si no estás en un camión escalera.", 255, 194, 14 )
		return
	end
		if getElementModel(getPedOccupiedVehicle(localPlayer)) == 544 and (not controlPanel) then
			...
			controlPanel = guiCreateWindow((screenW - 262) / 2, (screenH - 339) / 2, 262, 339, "Control de Escalera", false)
