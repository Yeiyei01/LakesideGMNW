...
local CKs = {}
local araces = {"Blanco", "Asiatico", [0] = "Negro"}
local descriptionNames = {"Color Pelo", "Estilo Pelo", "Descipcion Faciales", "Descripcion Fisica", "Ropa", "Accesorios"}
...
function addCharacterKillBody( x, y, z, rotation, skin, id, name, interior, dimension, age, race, weight, height, desc, cod, gender )
	...
	CKs[ped] = { name = name:gsub("_", " "), text = "El cadáver parece ser un " .. araces[race] .. " " .. ( gender == 1 and "Mujer" or "Hombre" ) .. "  entre las edades de " .. age - 2 .. " y " .. age + 2 .. ". El cadáver pesa alrededor de " .. weight .. "kg y mide alrededor " .. height .. "cm tall.\n" .. getCharacterDescription(desc) .. "\n\nPosible Causa Muerte: \n" .. cod }
...
function loadAllCorpses(res)
	...
	if result then
		...
		if garages then
			...
			outputDebugString( "Fallo en la carga de los Estados de Garaje" )
		end
	end
end
...
function getNearbyCKs(thePlayer, commandName)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) or exports.factions:isInFactionType(thePlayer, 4) then
		...
		outputChatBox("Cuerpos de Personajes Cercanos:", thePlayer, 255, 126, 0)
		local count = 0
