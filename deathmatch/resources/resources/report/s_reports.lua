	...
	for key , report in pairs(reports) do
		if report[8] ~= key then
			if isSupporterReport(report[7]) then
				...
				if showMessage then
					outputChatBox("Tu reporte ID#"..report[8].." ha subido a ID#"..key.." porque otros reportes han sido resueltos.", report[1], 70, 200, 30)
				end
			else -- Admin report
				adminNotice = adminNotice.."#"..report[8]..", "
				gmNotice = gmNotice.."#"..report[8]..", "
				if showMessage then
					outputChatBox("Tu reporte ID#"..report[8].." ha subido a ID#"..key.." porque otros reportes han sido resueltos.", report[1], 255, 195, 15)
				end
			end
	...
	if showMessage then
		if adminNotice ~= "" then
			...
			for key, value in ipairs(admins) do
				...
				if (adminduty==1) then
					outputChatBox(" Reportes con la ID "..adminNotice.." han cambiado su puesto en la cola de espera.", value, 255, 195, 15)
				end
			end
		end
		if gmNotice ~= "" then
			...
			for key, value in ipairs(gms) do
				...
				if (gmDuty == 1) then
					outputChatBox(" Reportes con la ID "..gmNotice.." han cambiado su puesto en la cola de espera.", value, 70, 200, 30)
				end
			end
		end
...
function showCKList(thePlayer)
	if exports.integration:isPlayerTrialAdmin(thePlayer) then
		outputChatBox("~~~~~~~~~ Requisitos de Self-CK ~~~~~~~~~", thePlayer, 255, 194, 15)
		...
		for key, value in ipairs(players) do
			...
			if (logged==1) then
				...
				if requested=="requested" then
					ckcount = 1
					outputChatBox("Requisito de Self-CK  de'" .. pname .. "' ("..playerID..") con la razón: '" .. reason .. "'.", thePlayer, 255, 195, 15)
				end
			end
		end
		...
		if ckcount == 0 then
			...
			outputChatBox("Usa /cka [id] o /ckd [id] para responder a las peticion(es).", thePlayer, 255, 194, 15)
		end
	end
end
			...
			if reports[id] then
				...
				if staff then
					...
					if (isElement(admin)) then
						...
						outputChatBox(" [#" .. id .."] Este reporte " .. getPlayerName(admin) .. " ("..adminName..").", thePlayer, 70, 200, 30)
					else
				...
				outputChatBox("ID de reporte inválido.", thePlayer, 255, 0, 0)
			end
		end
	end
end
...
function changeReportType(thePlayer, commandName, id, rID)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) or exports.integration:isPlayerScripter(thePlayer) then
		if not (id) or not (rID) then
			outputChatBox("SYNTAX: " .. commandName .. " [Reporte ID] [Tipo de Reporte ID]", thePlayer, 255, 194, 15)
			outputChatBox("SYNTAX: Tipos de Reporte:", thePlayer, 255, 194, 15)
			...
			if reportID > #reportTypes or reportID < 1 then
				outputChatBox("Error: No hay un tipo de reporte con esa ID.", thePlayer, 255, 0, 0)
				return
			end
			if reports[id] then
				local reporter = reports[id][1]
				...
				if not staff then
					outputChatBox("No hay miembros del Staff auxiliarios conectados.", thePlayer, 255, 0, 0)
				...
				outputChatBox("Tu reporte ha sido cambiado '"..oname.."'' a '"..name.."' por "..adminUser, reporter, 255, 126, 0)
				for k, v in ipairs(staff) do
					if string.find(auxiliaryTeams, v) then
						outputChatBox("Reporte #"..id.." - Tipo de Reporte cambiado por "..name..".", thePlayer, 255, 126, 0)
						for key, value in pairs(players) do
							if getElementData(value, "loggedin") == 1 then
								if string.find(getElementData(value, "forum_perms"), v) then
									...
									outputChatBox("Razón: " .. reason, value, 200, 240, 120)
								end
							end
						end
					else
						if isSupporterReport(oldReportType) then
							for key, value in pairs(GMs) do
								local gmDuty = getElementData(value, "duty_supporter")
								if (gmDuty == 1) then
									outputChatBox("Reporte #"..id.." - Tipo de Reporte cambiado por '"..oname.. "'' a '"..name.."'", value, 255, 126, 0)
								end
							end
...
addCommandHandler("cambiarreporte", changeReportType, false, false)
						...
						for key, value in ipairs(getElementsByType("players")) do
							if getElementData(value, "loggedin") == 1 then
								if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
									...
									outputChatBox(" ["..abrv.." #" .. i .."] Reporte sin asignar (" .. adminName .. " ha dejado el juego)", value, 255, 126, 0)
									alreadyTold[value] = true
									update = true
								end
							end
						...
						for key, value in ipairs(getElementsByType("players")) do
							if getElementData(value, "loggedin") == 1 then
								if string.find(getElementData(value, "forum_perms"), usergroup) and not alreadyTold[value] then
									...
									if adminduty == 1 or gmduty == 1 then
										local adminName = getElementData(source, "account:username")
										outputChatBox(" ["..abrv.." #" .. i .."] Reporte sin asignar (" .. adminName .. " ha dejado el juego)", value, 255, 126, 0)--200, 240, 120)
										alreadyTold[value] = true
										update = true
									end
								end
