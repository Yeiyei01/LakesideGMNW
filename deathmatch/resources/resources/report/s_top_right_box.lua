				...
				if staff then
					for k,v in ipairs(staff) do
						if string.find(adminTeams, v) and not seenReport[i] then
							table.insert(info, {abrv.." Reporte #" .. tostring(i) .. ": '" .. tostring(getPlayerName(reporter)) .. "' esta reportando '" .. tostring(getPlayerName(reported)) .. "' at " .. timestring .. ". Handler: " .. handler .. "", r, g, b})
							seenReport[i] = true
						end
					end
					count = count + 1
				...
				if staff then
					for k,v in ipairs(staff) do
						if SUPPORTER == v and not seenReport[i] then
							table.insert(info, {abrv.." Reporte #" .. tostring(i) .. ": '" .. tostring(getPlayerName(reporter)) .. "' está reportando '" .. tostring(getPlayerName(reported)) .. "' a las " .. timestring .. ". Tomado por: " .. handler .. "", r, g, b})
							seenReport[i] = true
						end
					end
					count = count + 1
...
function updateReports()
	local info = {}
	table.insert(info, {string.upper(" Todos los reportes"), 255,194,14,255,1,"default-bold" })
			...
			if staff then
				for k,v in ipairs(staff) do
					if not seenReport[i] then
						table.insert(info, {abrv.." Reporte #" .. tostring(i) .. ": '" .. tostring(getPlayerName(reporter)) .. "' reportó '" .. tostring(getPlayerName(reported)) .. "' a las " .. timestring .. ". Tomado por: " .. handler .. "", r, g, b})
						seenReport[i] = true
					end
				end
				count = count + 1
