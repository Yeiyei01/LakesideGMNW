...
function F1RPhelp( key, keyState )
	...
	if not myWindow then
		...
		myWindow = guiCreateWindow ( 0, 0, 800, 600, "Nine Nine Centro de Ayuda", false )
		...
		gui.reportWindow.main = guiCreateTab( "Centro de Reportes", tabPanel )
        -- draw the upper part of the tab.
        populateReportCenter()


		local tabCommands = guiCreateTab( "Ayuda de Comandos", tabPanel )
		local tabCommands2, newCmdBtn = nil, nil

		local tabRules = guiCreateTab( "Normativas", tabPanel )
		...
		addEventHandler("onClientBrowserCreated", browser, function()
			if isBrowserDomainBlocked("https://docs.google.com/document/d/18nAVe2CWNIOPb_SRzsAstrZ0LYYLS-OglPiphS2Qwx4/edit?usp=sharing") or isBrowserDomainBlocked("media.readthedocs.org") then
				requestBrowserDomains({"https://docs.google.com/document/d/18nAVe2CWNIOPb_SRzsAstrZ0LYYLS-OglPiphS2Qwx4/edit?usp=sharing", "media.readthedocs.org"}, false, function(accepted, newDomains)
					loadBrowserURL(source, "https://docs.google.com/document/d/18nAVe2CWNIOPb_SRzsAstrZ0LYYLS-OglPiphS2Qwx4/edit?usp=sharing")
				end)
			else
				loadBrowserURL(source, "https://docs.google.com/document/d/18nAVe2CWNIOPb_SRzsAstrZ0LYYLS-OglPiphS2Qwx4/edit?usp=sharing")
			end
		...
		local tabExplained = guiCreateTab( "Conceptos de Rol", tabPanel )
		...
		local tabOverview = guiCreateTab( "Explicacion Roleplay", tabPanel )
		...
		if canEditCmds() then
			...
			newCmdBtn = guiCreateButton(0, 0.95, 1 , 0.05, "Crear Nuevo CMD",true,tabCommands)
		...
		for category = 1, 7 do
			...
			col[category][2] = guiGridListAddColumn (grid[category], "Comando", 0.15)
			col[category][3] = guiGridListAddColumn (grid[category], "Tecla", 0.15)
			col[category][4] = guiGridListAddColumn (grid[category], "Explicacion", 0.5)
			col[category][5] = guiGridListAddColumn (grid[category], "Permisos", 0.1)
			if canEditCmds() then
		...
		local tabMantis = guiCreateTab( "Reporte de Bugs", tabPanel )
...
local function checkNameExists()
	...
	if (count>1) then
		guiSetText(gui.reportWindow.label_player_desc, "Se han encontrado multiples resultados.")
		guiLabelSetColor(gui.reportWindow.label_player_desc, 255, 255, 0)
	elseif (count==1) then
		guiSetText(gui.reportWindow.label_player_desc, "Encontrado: " .. getPlayerName(found) .. " (ID #" .. getElementData(found, "playerid") .. ")")
		guiLabelSetColor(gui.reportWindow.label_player_desc, 0, 255, 0)
		reportedPlayer = found
	elseif (count==0) then
		guiSetText(gui.reportWindow.label_player_desc, "No se ha encontrado.")
...
function populateReportCenter( submited )
	...
	if submited then
		local l = guiCreateLabel ( 0, 0, 1, 1, "Su reporte ha sido enviado, por favor espere un momento.", true, gui.reportWindow.topPanel )
	...
	elseif pending_report then
		local l = guiCreateLabel ( 0, 0, 1, 1, "Tu Reporte ID #" .. (pending_report[8] or "").. " esta pendiente.\nPor favor espere o use /er para cancelar el reporte.", true, gui.reportWindow.topPanel )
		guiLabelSetVerticalAlign( l, 'center' )
		guiLabelSetHorizontalAlign( l, 'center' )
	else
		gui.reportWindow.label_type = guiCreateLabel(23, 22, 351, 22, "Que necesita reportar?", false, gui.reportWindow.topPanel)
        guiSetFont(gui.reportWindow.label_type, "default-bold-small")

        gui.reportWindow.combo_type = guiCreateComboBox(23, 44, 351, 27, "Tipo", false, gui.reportWindow.topPanel)
        ...
        gui.reportWindow.label_details = guiCreateLabel(23, 81, 351, 22, "Los Detalles:", false, gui.reportWindow.topPanel)
        ...
        gui.reportWindow.label_player = guiCreateLabel(23, 265, 351, 22, "Usuario Reportado (Opcional):", false, gui.reportWindow.topPanel)
        guiSetFont(gui.reportWindow.label_player, "default-bold-small")

        gui.reportWindow.edit_player = guiCreateEdit(23, 287, 351, 34, "Nombre / ID", false, gui.reportWindow.topPanel)
        ...
        gui.reportWindow.btn_submit = guiCreateButton(23, 343, 143, 34, "Enviar", false, gui.reportWindow.topPanel)
...
function getCmdPerms(perm)
	...
	elseif perm >=11 and perm <=20 then --Supporters
		return exports.integration:isPlayerSupporter(localPlayer) or exports.integration:isPlayerTrialAdmin(localPlayer), "Moderator"
...
function openNewCommand(id, perm, cmd, key, ex)
	...
	local lineH2 = lineH
	local col1 = 100
	gui.l1 = guiCreateLabel(margin, margin, w-margin*2, lineH, "CREAR NUEVO COMANDO", false, gui.wNewStation)
	...
	gui.l5 = guiCreateLabel(margin, margin+lineH2, col1, lineH, "Categoria:", false, gui.wNewStation)
	...
	gui.l6 = guiCreateLabel(margin, margin+lineH2, col1, lineH, "Permisos:", false, gui.wNewStation)
	...
	gui.l2 = guiCreateLabel(margin, margin+lineH2, col1, lineH, "Comando:", false, gui.wNewStation)
	...
	gui.l3 = guiCreateLabel(margin, margin+lineH2, col1, lineH, "Tecla:", false, gui.wNewStation)
	...
	gui.l4 = guiCreateLabel(margin, margin+lineH2, col1, lineH, "Explicacion:", false, gui.wNewStation)
	...
	gui.bOk = guiCreateButton(margin, margin+lineH/2+lineH2, buttonW , lineH, id and "Salvar" or "Crear",false,gui.wNewStation)
	...
	gui.bDel = guiCreateButton(margin+buttonW, margin+lineH/2+lineH2, buttonW , lineH, "Borrar",false,gui.wNewStation)
	...
	gui.bClose1 = guiCreateButton(margin+buttonW*2, margin+lineH/2+lineH2, buttonW , lineH, "Cancelar",false,gui.wNewStation)
