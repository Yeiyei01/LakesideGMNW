...
local actionIcons =
{
	[TAB_WALLET] = { -48, tocolor( 255, 255, 255, 50 ), "Billetera" },
	[TAB_ITEMS] = { 48, tocolor( 255, 255, 255, 50 ), "Artículos" },
	[TAB_KEYS] = { -203, tocolor( 255, 255, 255, 50 ), "Llaves" },
	[TAB_WEAPONS] = { -204, tocolor( 255, 255, 255, 50 ), "Armas" },
	
	[ACTION_DROP] = { -202, tocolor( 127, 255, 127, 63 ), "Tirar Item", "Mantén CTRL y selecciona un item para tirarlo rápidamente." },
	[ACTION_SHOW] = { -201, tocolor( 127, 127, 255, 63 ), "Mostrar Item" },
	[ACTION_DESTROY] = { -200, tocolor( 255, 127, 127, 63 ), "Destruir Item", "Mantén DELETE y selecciona un item para borrarlo rápidamente." },
...
local function getOverlayText( itemID, itemValue, metadata, isGrouped )
	...
	elseif itemID == 153 then
		text = "Licencia de Moto"
	elseif itemID == 155 then
		text = "Licencia de Bote"
	elseif itemID == 154 then
		text = "Permiso de Pesca"
	elseif itemID == 78 then
		text = "Certificado de Piloto"
	end
...
local function getTooltipFor( itemID, itemValue, isGrouped, isProtected, metadata )
	...
	if itemID == 80 then
		if isGrouped then
			return isGrouped .. " Items Genericos", ""
		else
	...
	if itemID == 214 and isGrouped then
		return isGrouped .. " Drogas", ""
	end

	if itemID == 223 and isGrouped then
		return isGrouped .. " Items de Almacenamiento"
	end
	...
	if isGrouped then
		return isGrouped .. "x " .. name, "Haga clic para mostrar todos estos elementos."
	end
	...
	elseif not guiGetInputEnabled( ) and not isMTAWindowActive( ) and isCursorShowing( ) and not isPlayerMapVisible( ) then
		...
		if element then
			...
			if itemID ~= 81 and itemID ~= 103 and not getElementData ( localPlayer, "exclusiveGUI" ) then
				...
				tooltip( cursorX, cursorY, tooltipText1, getElementData(element, "transfering") and "Espere.." or tooltipText2)
			end
					...
					if state == "up" and clickItemSlot then
						...
						elseif not clickItemSlot.group then
									...
									if hoverElement == nil then
										if itemID > 0 then
											if itemID == 48 and countItems( localPlayer, 48 ) == 1 and getCarriedWeight( localPlayer ) - getItemWeight( 48, 1 ) > 10 then
												outputChatBox( "Tienes demasiadas cosas en tu inventario.", 255, 0, 0 )
											else
							...
							elseif hoverAction == ACTION_DROP then
								if isWatchingTV() then
									outputChatBox( "No estás EN tu televisor. Sólo sentado frente a ella.", 255, 0, 0 )
									clickItemSlot = nil
									return
								end
								local item = inventory[ clickItemSlot.invslot ]
							...
							elseif hoverAction == ACTION_SHOW then
								...
								if item[1] == 72 then
									itemName = itemName .. ", lee " .. itemValue
								...
								elseif item[1] == 133 or item[1] == 153 or item[1] == 154 or item[1] == 155 or item[1] == 78 then--Cards
									itemName = itemName .. ", emitido por "..itemValue..""
								elseif item[1] == 150 then--ATM card
									...
									itemName = itemName .. ", emitido para "..owner
							...
							elseif hoverAction == ACTION_DESTROY then
								...
								if itemID == 48 and countItems( localPlayer, 48 ) == 1 then -- backpack
									if getCarriedWeight( localPlayer ) - getItemWeight( 48, 1 ) > 10 then
										outputChatBox("Tienes demasiadas cosas en tu inventario.")
									else
										triggerServerEvent( "destroyItem", localPlayer, itemSlot )
									end
								elseif itemID == 134 then --Money
									outputChatBox("¿Qué haces? No puedes destruir el dinero.")
								else
									triggerServerEvent( "destroyItem", localPlayer, itemSlot )
								end
							elseif hoverAction == ACTION_SPLIT then
								...
								if splittableItems[itemID] then
									...
									outputChatBox(itemName.." no se puede dividir, coloca '/splits' para una lista de los items dividibles.",255,0,0)
								end
							end
						end
						hoverItemSlot = false
	...
	function( )
		if getElementData(localPlayer, "loggedin") == 1 then
			...
			elseif ((not getElementData(localPlayer, "adminjailed")) or exports.integration:isPlayerTrialAdmin(localPlayer)) and getElementData(getLocalPlayer(), "viewingInterior")~=1 then -- Chaos | Allowed admins to view inventory in jail, not in previewing interiors
				...
				outputChatBox("No puedes acceder a tu inventario en la carcel o en una vista previa de una propiedad.", 255, 0, 0)
			end
		end
	end
)
...
function splitItem(itemID, itemName, itemValue, item)
	...
	elseif itemID == 115 then
		...
		local GUIEditor_Label = guiCreateLabel(12,20,54,24,"Cantidad:",false,wSplitting)
			guiLabelSetVerticalAlign(GUIEditor_Label,"center")
			guiSetFont(GUIEditor_Label,"default-bold-small")
		local eAmount = guiCreateEdit(66,20,146,24,"un número entero",false,wSplitting)
		local bOK = guiCreateButton(12,48,100,21,"OK",false,wSplitting)
		local bCancel = guiCreateButton(112,48,100,21,"CANCELAR",false,wSplitting)
		...
		addEventHandler("onClientGUIClick", bOK, function()
			...
			if not amount then
				guiSetText(wSplitting,"La cantidad tiene que ser un numero")
				return false
			end
			if amount%1 ~= 0 then
				guiSetText(wSplitting,"La cantidad tiene que ser un número entero, como 1, 2, 3...")
				return false
			else
				if amount <= 0 then
					guiSetText(wSplitting,"La cantidad tiene que ser mayor a 0")
					return false
				end
			end
