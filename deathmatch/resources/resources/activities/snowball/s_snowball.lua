...
function makeSnowball(thePlayer)
	if not getElementData(thePlayer, "snowball") then
		...
		setTimer(function()
			...
			triggerEvent('sendAme', thePlayer, "toma algo de nieve y forma una bola.")
				...
				if isElement(thePlayer) then 
					if getElementData(thePlayer, "snowball") then
						if getElementData(thePlayer, "snowball") == obj then
							triggerEvent('sendAme', thePlayer, "se le derretió la bola de nieve en su mano.")
...
function throwSnowball(thePlayer, x, y, z, hitElement, hit)
	if getElementData(thePlayer, "snowball") then
		...
		triggerEvent('sendAme', thePlayer, " tira una bola de nieve.")
		unbindKey(thePlayer, "mouse1", "down", shootBall)
		...
		setTimer(function()
			...
			setTimer(function()
				...
				destroyElement(obj)
				if hitElement then
					triggerEvent('sendAme', hitElement, "ha sido golpeado por una bola de nieve.")
