...
function togglePinVD()
	...
	if enableOverlayDescription ~= "0" and enableOverlayDescriptionVeh ~= "0" then
		...
		if enableOverlayDescriptionVehPin == "1" then
			...
			--exports.hud:sendBottomNotification(localPlayer, "Descripción Propiedad", "Usted tiene la descripción de la propiedad PINED en su pantalla.")
				...
				if getDistanceBetweenPoints3D(cx,cy,cz,x,y,z) <= viewDistance then --Within radius viewDistance
					...
					if px and isLineOfSightClear(cx, cy, cz, x, y, z, true, false, false, true, true, false, false) then				
						...
						if getElementData(theVehicle, "carshop") then
							...
							toBeShowed = toBeShowed.."Precio: $"..exports.global:formatMoney(price).."\n Impuesto: $"..exports.global:formatMoney(taxes)
							lines = lines+ 2
						else
							...
							if job == 1 then
								descToBeShown = "RS Deliverys - ¡Rapidos y Seguros!"
								lines = lines + 1
							elseif job == 2 then
								descToBeShown = "Un Taxi Amarillo\nLlama al #8294!"
								lines = lines + 2
							elseif job == 3 then
								descToBeShown = "Riverside Transport"
								lines = lines + 1
							else
							local faction = getElementData(theVehicle, "faction")
							if faction == 20 then
								descToBeShown = "RS American Transport\n¡Nos Deshacemos de su Carga!"
								lines = lines + 1
					                elseif faction == 9 then
								descToBeShown = "Golden Gate Company\n¡Cortes de Primera!"
							else
								lines = lines + 2
								for j = 1, 5 do
							...
							if not noPlateVehs[getElementModel(theVehicle)] then
								if getElementData(theVehicle, "show_plate") == 0 then
									if getElementData(localPlayer, "duty_admin") == 1 then
										toBeShowed = toBeShowed.."((Matricula: "..plate.."))\n"
										lines = lines + 1
									else
							...
							if vowner == getElementData(localPlayer, "dbid") or exports.global:isStaffOnDuty(localPlayer) or exports.integration:isPlayerScripter(localPlayer) or exports.integration:isPlayerVCTMember(localPlayer) then
								...
								if vowner > 0 then 
									...
									if protected then
										...
										protectedText = "[Protección Contra Inactividad restante: "..details.."]"
										local toBeAdded = "\n"..protectedText
										...
										if active and (powner == getElementData(localPlayer, "dbid") or exports.integration:isPlayerStaff(localPlayer)) then
											...
											if owner_last_login and tonumber(owner_last_login) then
												...
												inactiveText = inactiveText.." Propietario visto última vez "..owner_last_login_text.." "
											else
												inactiveText = inactiveText.." El propietario fue visto última vez irrelevante | "
											end
											...
											if lastused and tonumber(lastused) then
												...
												inactiveText = inactiveText.."Última vez que se utilizó "..lastusedText.."]"
											else
												inactiveText = inactiveText.."El último utilizado es irrelevante]"
											end
											local toBeAdded = "\n"..inactiveText
											toBeShowed = toBeShowed..toBeAdded
											local len = dxGetTextWidth(toBeAdded)
	...
	if getPedOccupiedVehicle(getLocalPlayer()) then
		...
		if exports.global:hasItem(getLocalPlayer(), 3, dbid) or owner == playerId or exports.global:hasItem(theVehicle, 3, dbid) or exports.factions:isPlayerInFaction(getLocalPlayer(), factionid) or exports.integration:isPlayerTrialAdmin(getLocalPlayer()) and getElementData(getLocalPlayer(), "duty_admin") == 1 then
			if dbid > 0 then
				...
				showCursor(true)
				wEditDescription = guiCreateWindow(x,y,441,230,"Editar Descripción Vehículo",false)
				...
				bSave = guiCreateButton(10,198,210,75,"Salvar",false,wEditDescription)
				bClose = guiCreateButton(220,198,210,75,"Cerrar",false,wEditDescription)
				...
				exports.hud:sendBottomNotification(getLocalPlayer(), "Descripción Vehículo", "No se pueden establecer descripciones en vehículos temporales.")
			end
		else
			exports.hud:sendBottomNotification(getLocalPlayer(), "Descripción Vehículo", "Usted no tiene las llaves de este vehículo.")
		end
	else
		exports.hud:sendBottomNotification(getLocalPlayer(), "Descripción Vehículo", "Debe estar en el vehículo del que desea cambiar la descripción.")
	end
end
