...
function adminLoginToPlayerCharacter(thePlayer, commandName, ...)
    if exports.integration:isPlayerLeadAdmin(thePlayer) then
        if not (...) then
            outputChatBox("SYNTAX: /" .. commandName .. " [Nombre exacto del PJ]", thePlayer, 255, 194, 14, false)
            outputChatBox("Loguearse en ese PJ.", thePlayer, 255, 194, 0, false)
        else
            ...
            if not fetchData then
                outputChatBox("No se ha encontrado.", thePlayer, 255,0,0)
                return false
            end
    ...
    if characterData then
        ...
        if isElement(playerWithNick) and (playerWithNick~=client) then
            if theAdmin then
                ...
                kickPlayer(playerWithNick, getRootElement(), adminTitle.." "..adminUsername.." se ha logueado en tu cuenta.")
                outputChatBox("Cuenta "..targetAccountName.." ("..tostring(characterData["charactername"]):gsub("_"," ")..") ha sido kickeada de la partida.", theAdmin, 0, 255, 0 )
            else
                kickPlayer(playerWithNick, getRootElement(), "Alguien mas se ha metido en tu personaje.")
            end
        end
        ...
        if (tonumber(characterData["blindfold"])==1) then
            ...
            outputChatBox("Tu PJ está vendado los ojos, de ser esto un error avisa a un staff via F2.", client, 255, 194, 15)
        ...
        if (tonumber(characterData["cuffed"])==1) then
            outputChatBox("Tu PJ está esposado.", client, 255, 0, 0)
        end
...
function Characters_onCharacterChange()
    ...
    triggerEvent("savePlayer", client, "Cambiar de PJ")
    triggerEvent('setDrunkness', client, 0)
    setElementDataEx(client, "alcohollevel", 0, true)
