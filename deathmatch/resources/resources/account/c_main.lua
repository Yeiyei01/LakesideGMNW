...
whiteMales = {23, 26, 27, 29, 30, 32, 33, 34, 35, 36, 37, 38, 43, 44, 45, 46, 47, 48, 50, 51, 52, 53, 58, 59, 60, 61, 62, 68, 70, 72, 73, 78, 81, 82, 94, 95, 96, 97, 98, 99, 100, 101, 108, 109, 110, 111, 112, 113, 114, 115, 116, 120, 121, 122, 124, 125, 126, 132, 133, 135, 137, 147, 154, 155, 158, 159, 160, 164, 165, 170, 171, 173, 174, 175, 177, 179, 181, 184, 186, 187, 188, 189, 200, 202, 204, 206, 209, 212, 213, 217, 223, 230, 234, 235, 236, 240, 241, 242, 247, 248, 250, 252, 254, 255, 258, 259, 261, 264, 272 }
asianMales = {49, 57, 58, 59, 60, 117, 118, 120, 121, 122, 123, 170, 186, 187, 203, 210, 227, 228, 229, 294}
blackFemales = {9, 10, 11, 12, 13, 40, 41, 63, 64, 69, 76, 91, 139, 148, 190, 195, 207, 215, 218, 219, 238, 243, 244, 256 }
...
local label = guiCreateLabel( 0, 0, screenX, 15, "Discord: ¡Click aquí! v"..exports.global:getScriptVersion().."", false )
...
addEventHandler('onClientGUIClick', label, function()
	local url = "https://discord.gg/Q9SbJeA9du"
	--triggerEvent('debug:releases', localPlayer)
	setClipboard(url)
	outputChatBox("Copiado '".. url .. "' al borrador")
end, false)

function updateSoundLabel(setting)
	if setting == 0 then 
		guiSetText(toggleSoundLabel, "Desactivar Musica")
	else 
		guiSetText(toggleSoundLabel, "Activar Musica")
	end
end
...
function stopNameChange(oldNick, newNick)
	if (source==getLocalPlayer()) then
		...
		if (oldNick~=newNick) and (legitNameChange==0) then
			...
			outputChatBox("Click 'Cambiar Personaje' Cambiar tu personaje.", 255, 0, 0)
		end
	end
end
...
addEventHandler("account:showMusicLabel", root, function()
	toggleSoundLabel = guiCreateLabel( 0, 0, screenX, 15, "Activar Musica", false )
