...
function moneyUpdateFX(state, amount, toEachother)
	if amount and tonumber(amount) and tonumber(amount) > 0  then
		local info = {{"Actualización Finanzas Personales"},{""}}
		...
		if state then
			...
			table.insert(info, {"   - Efectivo: $"..exports.global:formatMoney(money).." ("..moneyFloat["text"]..")"})
			if toEachother then
				table.insert(info, {"   - En el Banco: $"..exports.global:formatMoney(bankmoney-amount)})
			else
	...
	if vehicle and not windowsDown and vehicle ~= getPedOccupiedVehicle(localPlayer) and getElementData(vehicle, "tinted") then
		...
		if seat0 or seat1 then
			if distance > 1.4 then
				name = "Persona Desconocida (Tinte)"
				tinted = true
			end
		else
			name = "Persona Desconocida (Tinte)"
		...
		for k, v in pairs(badges) do
			...
			if title then
				if v[5] then
					...
					name = "Persona Desconocida (Bandana)"
					badge = true
				else
							...
							local collision, cx, cy, cz, element = processLineOfSight(lx, ly, lz, rx, ry, rz+1, true, true, true, true, false, false, true, false, vehicle)
								...
								if (sx) and (sy) then
									...
									if (sx) and (sy) then
										...
										local pedName = getElementData(player,"name") and tostring(getElementData(player,"name")):gsub("_", " ") or "El Almacenista"
