function showStats(thePlayer, commandName, targetPlayerName)
	if getElementData(thePlayer, "loggedin") ~= 1 then
		outputChatBox("No estás conectado.", thePlayer, 255, 0, 0)
		return
	end
	...
	if exports.integration:isPlayerTrialAdmin(thePlayer) and targetPlayerName then
		...
		if targetPlayer then
			if getElementData(targetPlayer, "loggedin") == 1 then
				...
				outputChatBox("El jugador no está conectado.", showPlayer, 255, 0, 0)
				return
			end
		else
			return
	...
	elseif (carlicense==3) then
		carlicense = "Examen Teórico Aprobado"
	else
		carlicense = "No"
	end
	...
	elseif (bikelicense==3) then
		bikelicense = "Examen Teórico Aprobado"
	else
		bikelicense = "No"
	end
	...
	if (properties=="") then properties = "Ninguna.  " end
	if (carids=="") then carids = "Ninguno.  " end
	--FETCH ABOVE
	...
	if isOverlayDisabled then
		...
		outputChatBox(" Licencia Conducir: " .. carlicense, showPlayer)
		outputChatBox(" Licencia Motocicleta: " .. bikelicense, showPlayer)
		outputChatBox(" Licencia Navegación: " .. boatlicense, showPlayer)
		outputChatBox(" Licencia Piloto: " .. pilotlicense, showPlayer)
		outputChatBox(" Licencia Armas Nivel 1: " .. gunlicense , showPlayer)
		outputChatBox(" Licencia Armas Nivel 2: " .. gun2license , showPlayer)
		outputChatBox(" Permiso Pesca: " .. fishlicense, showPlayer)
		outputChatBox(" Vehículos (" .. printCar .. "): " .. string.sub(carids, 1, string.len(carids)-2) , showPlayer)
		...
		outputChatBox(" Tiempo: " .. hoursplayed .. " horas." , showPlayer)
		outputChatBox(" Idiomas: " , showPlayer)
	else
		info = {
			{getPlayerName(thePlayer):gsub("_", " ")},
			{""},
			{" Fecha de nacimiento: "..exports.global:getPlayerDoB(thePlayer)},
			{" Licencia Conducir: " .. carlicense},
			{" Licencia Motocicleta: " .. bikelicense},
			{" Licencia Navegación: " .. boatlicense},
			{" Licencia Piloto: " .. pilotlicense},
			{" Licencia Armas Nivel 1: " .. gunlicense},
			{" Licencia Armas Nivel 2: " .. gun2license},
			{" Permiso Pesca: " .. fishlicense},
			{" Vehículos (" .. printCar .. "): " .. string.sub(carids, 1, string.len(carids)-2)},
			{" Propiedades (" .. numproperties .. "/"..(getElementData(thePlayer, "maxinteriors") or 10).."): " .. string.sub(properties, 1, string.len(properties)-2)},
			{" Tiempo: " .. hoursplayed .. " horas."},
			{" Idiomas: "},
		}
	end
	--LANGUAGES
	...
	if isOverlayDisabled then
		outputChatBox( " Finanzas: ", showPlayer)
		outputChatBox( "   - GCs: "..exports.global:formatMoney(currentGC), showPlayer)
		outputChatBox( "   - Efectivo: $"..exports.global:formatMoney(money), showPlayer)
		outputChatBox( "   - En el Banco: $"..exports.global:formatMoney(bankmoney), showPlayer)
	else
		table.insert(info, {" Finanzas: "})
		table.insert(info, {"   - GCs: "..exports.global:formatMoney(currentGC)})
		table.insert(info, {"   - Efectivo: $"..exports.global:formatMoney(money)})
		table.insert(info, {"   - En el Banco: $"..exports.global:formatMoney(bankmoney)})
	end
