		...
		if upgrade then
			if type == "add" then
				addVehicleUpgrade(vehicle, upgrade)
                                dbExec(exports.mysql:getConn('mta'), "UPDATE `tunnings` SET optical= ? WHERE id = ?", vehicle, upgrade)
			...
			if not data[1][2] then -- Default upgrade
				for _, property in ipairs(data) do
					setVehicleHandling(vehicle, property[1], nil, false)
                                        dbExec(exports.mysql:getConn('mta'), "UPDATE `tunnings` SET performance= ? WHERE id = ?", vehicle, property[1], nil, false)
				end
			else
				for _, property in ipairs(data) do
					...
					setVehicleHandling(vehicle, property[1], defaultHandling, false)
                                        dbExec(exports.mysql:getConn('mta'), "UPDATE `tunnings` SET performance= ? WHERE id = ?", vehicle, property[1], defaultHandling, false)
					setVehicleHandling(vehicle, property[1], defaultHandling + property[2], false)
                                        dbExec(exports.mysql:getConn('mta'), "UPDATE `tunnings` SET performance= ? WHERE id = ?", defaultHandling + property[2], false)
				end
			end
		end
	end
		...
		if property then
			if value then
				setVehicleHandling(vehicle, property, value, false)
                                dbExec(exports.mysql:getConn('mta'), "UPDATE `tunnings` SET handling= ? WHERE id = ?", vehicle, property, value, false)
			else
				setVehicleHandling(vehicle, property, getOriginalHandling(getElementModel(vehicle))[property], false)
                                dbExec(exports.mysql:getConn('mta'), "UPDATE `tunnings` SET handling= ? WHERE id = ?", vehicle, property, false)
			end
		end
	end
end)
	...
	if vehicle then
		if type then
			...
			if side then
				if side == "front" then
					setVehicleHandlingFlags(vehicle, 3, type)
                                        dbExec(exports.mysql:getConn('mta'), "UPDATE `tunnings` SET wheelwidth= ? WHERE id = ?", vehicle, 3, type)
				elseif side == "rear" then
					setVehicleHandlingFlags(vehicle, 4, type)
                                        dbExec(exports.mysql:getConn('mta'), "UPDATE `tunnings` SET wheelwidth= ? WHERE id = ?", vehicle, 4, type)
				else
					setVehicleHandlingFlags(vehicle, {3, 4}, type)
                                        dbExec(exports.mysql:getConn('mta'), "UPDATE `tunnings` SET wheelwidth= ? WHERE id = ?", vehicle, {3, 4}, type)
				end
			else
				setVehicleHandlingFlags(vehicle, {3, 4}, type)
                                dbExec(exports.mysql:getConn('mta'), "UPDATE `tunnings` SET wheelwidth= ? WHERE id = ?", vehicle, {3, 4}, type)
			...
			setVehicleHandlingFlags(vehicle, {3, 4}, 0)
                        dbExec(exports.mysql:getConn('mta'), "UPDATE `tunnings` SET wheelwidth= ? WHERE id = ?", vehicle, {3, 4}, 0)
		end
	end
end)
	...
	if vehicle then
		if type then
			...
			setVehicleHandlingFlags(vehicle, 6, type)
                        dbExec(exports.mysql:getConn('mta'), "UPDATE `tunnings` SET offroadability= ? WHERE id = ?", vehicle, 6, type)
		else
			setVehicleHandlingFlags(vehicle, 6, 0)
                        dbExec(exports.mysql:getConn('mta'), "UPDATE `tunnings` SET offroadability= ? WHERE id = ?", vehicle, 6, 0)
		end
	end
end)
...
addEventHandler("tuning->Color", root, function(vehicle, color, headlightColor)
	if vehicle then
		setVehicleColor(vehicle, color[1], color[2], color[3], color[4], color[5], color[6], color[7], color[8], color[9])
                dbExec(exports.mysql:getConn('mta'), "UPDATE `tunnings` SET color= ? WHERE id = ?", color[1], color[2], color[3], color[4], color[5], color[6], color[7], color[8], color[9])
		setVehicleHeadLightColor(vehicle, headlightColor[1], headlightColor[2], headlightColor[3])
                dbExec(exports.mysql:getConn('mta'), "UPDATE `tunnings` SET color= ? WHERE id = ?", vehicle, headlightColor[1], headlightColor[2], headlightColor[3])
	end
end)
		...
		if getElementType(element) == "vehicle" then
			if getVehicleController(element) then
				local vehicleController = getVehicleController(element)
                                if getElementData(vehicleController, 'job') ~= 5 then outputChatBox('[!]#D6D6D6 Esto solo puede ser utilizado por miembros del taller', vehicleController, 255, 0 ,0, true) return end
...
addEvent("paraKes", true)
addEventHandler("paraKes", root, function(amount)
