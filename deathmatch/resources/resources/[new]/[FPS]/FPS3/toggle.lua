...
addCommandHandler(antiLag_settings.cmd,
	function(x)
		if (not TEXTURE_FPS) then
			return outputChatBox("*ERROR: Vuelva a intentarlo más tarde.", 255, 0, 0)
		end
		...
		if (antiLag_settings.status == "off") then
			...
			outputChatBox("*Se ha activado Ultra Anti-lag, sus FPS deben aumentar.", 0, 255, 0)
		else
			...
			outputChatBox("* Ultra Anti-lag está desactivado, sus FPS pueden caer.", 0, 255, 0)
		end
	end
)