...
function F1RPhelp( key, keyState )
	if not (pressed) then
		...
		if ( myWindow == nil ) then		
			...
			myWindow = guiCreateWindow ( 0.3, 0.3, 0.7, 0.7, "SantosGaming - F1 Help", true )
			guiWindowSetSizable(myWindow, false)
			local tabPanel = guiCreateTabPanel ( 0, 0.1, 1, 1, true, myWindow )
			
			local tabRules = guiCreateTab( "Reglas", tabPanel )
			...
			local tabOverview = guiCreateTab( "Vista al Roleplay", tabPanel )
