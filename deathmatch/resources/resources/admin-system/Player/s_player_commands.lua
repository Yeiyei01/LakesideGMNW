	...
	if result then
		while true do
			...
			if row["serial"] == serial then
				exports.global:sendMessageToAdmins("BAN-SYSTEM: Jugador " .. getPlayerName(thePlayer):gsub("_", " ") .. " está baneado. Kickeando al jugador")
		...
		if not hide then
			for key, value in ipairs(getElementsByType("player")) do
				if tonumber( getElementData( value, "punishment_notification_selector") ) ~= 1 or value == thePlayer or value == theAdmin then
					outputChatBox(exports.global:getPlayerAdminTitle(theAdmin) .. " " .. getPlayerName(theAdmin):gsub("_"," ") .. " baneó a " .. getPlayerName(thePlayer):gsub("_"," "), value, 255, 0, 0)
					outputChatBox("Razón: " .. reason, value, 255, 0, 0)
				end
			end
		else
			outputChatBox("Has baneado a " .. getPlayerName(thePlayer):gsub("_"," ") .. " silenciosamente.", theAdmin, 0, 255, 0)
			exports.global:sendMessageToAdmins("ADM-WARN: " .. getPlayerName(theAdmin):gsub("_"," ") .. " baneó a " .. getPlayerName(thePlayer):gsub("_"," ") .. " silently.")
			exports.global:sendMessageToAdmins("          Razón: " .. reason)
		end
...
function adminUncuff(thePlayer, commandName, targetPlayer)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if not (targetPlayer) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador / ID]", thePlayer, 255, 194, 14)
		else
			...
			if targetPlayer then
				...
				if (logged==0) then
					outputChatBox("Jugador no logueado.", thePlayer, 255, 0, 0)
				else
					local restrain = getElementData(targetPlayer, "restrain")

					if (restrain==0) then
						outputChatBox("Jugador no esposado.", thePlayer, 255, 0, 0)
					else
						local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
						if hiddenAdmin == 0 then
							outputChatBox("Has sido des-esposado por " .. username .. ".", targetPlayer)
						else
							outputChatBox("Has sido des-esposado por un Admin Oculto.", targetPlayer)
						end
						outputChatBox("Has des-esposado a " .. targetPlayerName .. ".", thePlayer)
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					if any then
						local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
						if hiddenAdmin == 0 then
							outputChatBox("Tu mascara ha sido removida por admin "..username, targetPlayer, 255, 0, 0)
						else
							outputChatBox("Tu mascara ha sido removida por un Admin Oculto", targetPlayer, 255, 0, 0)
						end
						outputChatBox("Has removido la mascara de " .. targetPlayerName .. ".", thePlayer, 255, 0, 0)
...
function infoDisplay(thePlayer)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		outputChatBox("---[        Información de Ayuda        ]---", getRootElement(), 255, 194, 15)
		outputChatBox("---[ IP: mtasa://144.217.16.123:48090", getRootElement(), 255, 194, 15)
		outputChatBox("---[ Nombre: Lakeside RP", getRootElement(), 255, 194, 15)
		outputChatBox("---[ Discord: https://discord.gg/Q9SbJeA9du", getRootElement(), 255, 194, 15)
		outputChatBox("---[ Dueños: Dreko y Ale", getRootElement(), 255, 194, 15)
		outputChatBox("---[ Version: 2.0", getRootElement(), 255, 194, 15)
	end
end
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					if (blindfolded==0) then
						outputChatBox("Jugador no vendado", thePlayer, 255, 0, 0)
					else
						exports.anticheat:changeProtectedElementDataEx(targetPlayer, "blindfold", false, false)
						fadeCamera(targetPlayer, true)
						outputChatBox("Has vendado a " .. targetPlayerName .. ".", thePlayer)
						local hiddenAdmin = getElementData(thePlayer, "hiddenadmin")
						if hiddenAdmin == 0 then
							outputChatBox("Has sido desvendado por admin " .. username .. ".", thePlayer)
						else
							outputChatBox("Has sido desvendado por un Admin Oculto.", thePlayer)
						end
			...
			if targetPlayer then
				...
				if (logged==0) then
					...
					if muted == 0 then
						...
						outputChatBox(targetPlayerName .. " está muteado del OOC.", thePlayer, 255, 0, 0)
						if hiddenAdmin == 0 then
							outputChatBox("Has sido muteado por '" .. getPlayerName(thePlayer) .. "'.", targetPlayer, 255, 0, 0)
						else
							outputChatBox("Has sido muteado por un Admin Oculto.", targetPlayer, 255, 0, 0)
						end
						...
						outputChatBox(targetPlayerName .. " está desmuteado del OOC.", thePlayer, 0, 255, 0)

						if hiddenAdmin == 0 then
							outputChatBox("Has sido desmuteado por '" .. getPlayerName(thePlayer) .. "'.", targetPlayer, 0, 255, 0)
						else
							outputChatBox("Has sido desmuteado por un Admin Oculto.", targetPlayer, 0, 255, 0)
						end
			...
			if targetPlayer then
				...
				elseif (logged==1) then
					...
					if (hiddenAdmin==0) then
						...
						outputChatBox("Has sido desarmado por "..tostring(adminTitle) .. " " .. getPlayerName(thePlayer)..".", targetPlayer, 255, 0, 0)
					else
						exports.global:sendMessageToAdmins("AdmCmd: Un admin oculto ha desarmado a " .. targetPlayerName..".")
						outputChatBox("Has sido desarmado por un Admin Oculto.", targetPlayer, 255, 0, 0)
					end
...
function forceApplication(thePlayer, commandName, targetPlayer, ...) -- Maxime
	if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) then
		if not (targetPlayer) or not (...) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador/ID] [Reason]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayer)
	...
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if not (targetPlayer) then
			...
			if targetPlayer then
				...
				if (hiddenAdmin==0) then
					exports.global:sendMessageToAdmins("AdmCmd: " .. tostring(adminTitle) .. " " .. adminName .. " reconectó " .. targetPlayerName )
				else
					adminTitle = ""
					adminName = "un admin oculto"
					exports.global:sendMessageToAdmins("AdmCmd: Un admin oculto ha reconectado a " .. targetPlayerName )
				end
				outputChatBox("El jugador '" .. targetPlayerName .. "' ha sido forzado a reconectar.", thePlayer, 255, 0, 0)

				local timer = setTimer(kickPlayer, 1000, 1, targetPlayer, getRootElement(), "Has sido forzado a reconectar por "..tostring(adminTitle) .. " " .. adminName ..".")
...
function givePlayerGun(thePlayer, commandName, targetPlayer, ...)
	if exports["integration"]:isPlayerAdmin(thePlayer) then
		...
		if not (targetPlayer) or (#args < 1) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador/ID] [Nombre del Arma/ID]", thePlayer, 255, 194, 14)
			outputChatBox("     Give player a weapon.", thePlayer, 150, 150, 150)
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador/ID] [Nombre del Arma/ID] [Cantidad]", thePlayer, 255, 194, 14)
			outputChatBox("     Dar a un jugador una cantidad de armas.", thePlayer, 150, 150, 150)
			outputChatBox("(Usa /gunlist)", thePlayer, 0, 255, 0)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayer)
			...
			if targetPlayer then
				...
				if weaponID == nil then
					...
					elseif cWeaponName == "fireextinguisher" then
						...
						if getWeaponIDFromName(cWeaponName) == false then
							outputChatBox("[MAKEGUN] Nombre del Arma inválido/ID. Usa /gunlist para ver una lista de las armas.", thePlayer, 255, 0, 0)
							return
						else
				...
				if weaponID == 40 then
					outputChatBox("[MAKEGUN] El arma/ID no está permitida.", thePlayer, 255, 0, 0)
					return
				end
				...
				if (logged==0) then
					outputChatBox("[MAKEGUN] Jugador no logueado.", thePlayer, 255, 0, 0)
				elseif (logged==1) then
					...
					if quantity > maxAmountOfWeapons then
						...
						outputChatBox("[MAKEGUN] No puedes dar más de "..maxAmountOfWeapons.." armas al mismo tiempo, prueba con "..maxAmountOfWeapons.."...", thePlayer, 150, 150, 150)
					end
					...
					if count > 0 then
						...
						if (hiddenAdmin==0) then
							--Inform Spawner
							outputChatBox("[MAKEGUN] Has dado (x"..count..") ".. getWeaponNameFromID(weaponID).." a "..targetPlayerName..".", thePlayer, 0, 255, 0)
							--Inform Player
							outputChatBox("Has recibido (x"..count..") ".. getWeaponNameFromID(weaponID).." de "..adminTitle.." "..getPlayerName(thePlayer)..".", targetPlayer, 0, 255, 0)
							--Send adm warning
							exports.global:sendMessageToAdmins("AdmCmd: " .. tostring(adminTitle) .. " " .. getPlayerName(thePlayer) .. " dió " .. targetPlayerName .. " (x"..count..") " .. getWeaponNameFromID(weaponID) .. " with serial '"..allSerials.."'")
						else -- If hidden admin
							outputChatBox("[MAKEGUN] Has dado (x"..count..") ".. getWeaponNameFromID(weaponID).." a "..targetPlayerName.." con serials '"..allSerials, thePlayer, 0, 255, 0)

							outputChatBox("Has recibido (x"..count..") ".. getWeaponNameFromID(weaponID).." de un admin oculto.", targetPlayer, 0, 255, 0)
						end
					end
					if fails > 0 then
...
function givePlayerGunAmmo( thePlayer, commandName, targetPlayer, weap_id, rounds )
	if exports["integration"]:isPlayerAdmin( thePlayer )  then
		if not targetPlayer or not weap_id or not tonumber(weap_id) or not getWeaponNameFromID( tonumber(weap_id) ) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador/id] [arma id] [cartuchos -opcional]", thePlayer, 255, 194, 14)
			outputChatBox("Info: usa el Comando /gunlist", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick( thePlayer, targetPlayer )
			if targetPlayer then
				...
				if exports.weapon:isWeapAmmoless( weap_id ) then
					outputChatBox( getWeaponNameFromID(weap_id).." no necesita municiones." , thePlayer, 255, 0, 0 )
					return
				end

				local given, ammo, why = exports.weapon:givePlayerAmmo( thePlayer, targetPlayer, weap_id, nil, rounds )
				if given then
					outputChatBox( "Has dado a  "..targetPlayerName.." un pack de "..ammo.cartridge.." ("..ammo.rounds.." cartuchos , serial: '"..why.."').", thePlayer, 0, 255, 0 )
					outputChatBox( exports.global:getPlayerFullIdentity(thePlayer) .." te dió un pack de "..ammo.cartridge.." ("..ammo.rounds.." cartuchos, serial: '"..why.."').", targetPlayer, 0, 255, 0)
					if getElementData(thePlayer, 'hiddenadmin') ~= 1 then
						exports.global:sendMessageToAdmins( exports.global:getPlayerFullIdentity(thePlayer) .." dió a "..targetPlayerName.." un pack de "..ammo.cartridge.." ("..ammo.rounds.." cartuchos, serial: '"..why.."')." )
					end
					exports.logs:dbLog(thePlayer, 4, targetPlayer, "GIVEITEM "..ammo.cartridge.." ("..ammo.rounds.." cartuchos, serial: '"..why.."').")
				else
...
function givePlayerItem(thePlayer, commandName, targetPlayer, itemID, ...)
	if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerScripter(thePlayer)) then
		...
		if not (itemID) or not (...) or not (targetPlayer) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador / ID] [Item ID] [Valor de Item]", thePlayer, 255, 194, 14)
		else
			...
			if ( itemID == 74 or itemID == 150 or itemID == 75 or itemID == 78 or itemID == 2 or itemID == 216) and not exports.integration:isPlayerLeadAdmin( thePlayer) then -- Banned Items
				exports.hud:sendBottomNotification(thePlayer, "Banned Items", "Solo un Senior+ Admin puede spawnear este item.")
				return false
			end
			...
			if itemID == 16 then
				...
				elseif itemValue == 300 and not exports.integration:isPlayerLeadAdmin(thePlayer) then
					outputChatBox("No tienes los suficientes permisos.", thePlayer, 255, 0, 0)
					return false
				end
			end

			if itemID == 114 and exports.npc:getDisabledUpgrades()[tonumber(itemValue)] then
				outputChatBox("Item desactivado.", thePlayer, 255, 0, 0)
				return false
			end
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayer)
			...
			if preventSpawn then
				exports.hud:sendBottomNotification(thePlayer, "Item no spawneable.", "Este item no se puede dar, puede ser debido a que está prohibido o que solo se consigue IC.")
				return false
			end
			...
			if targetPlayer then
				...
				elseif (itemID == 115 or itemID == 116 or itemID == 68 or itemID == 134 --[[or itemID == 137)]]) then
					outputChatBox("No puedes usar esto con /giveitem.", thePlayer, 255, 0, 0)
				...
				elseif (logged==1) then
					...
					if itemID > 0 and name and name ~= "?" then
						...
						if success then
							outputChatBox("El jugador " .. targetPlayerName .. " ha recibido un " .. name .. " con el valor de " .. itemValue .. ".", thePlayer, 0, 255, 0)
							...
							local adminTitle = exports.global:getPlayerAdminTitle(thePlayer)
							if (hiddenAdmin==0) then
								outputChatBox(tostring(adminTitle) .. " " .. getPlayerName(thePlayer) .. " te ha dado un " .. name .. " con el valor de " .. itemValue .. ".", targetPlayer, 0, 255, 0)
							else
								outputChatBox("Un administrador oculto te ha dado " .. name .. " con el valor de " .. itemValue .. ".", targetPlayer, 0, 255, 0)
							end
						else
							outputChatBox("No se pudo dar a " .. targetPlayerName .. " un " .. name .. ": " .. tostring(reason), thePlayer, 255, 0, 0)
						end
					else
						outputChatBox("Invalid Item ID.", thePlayer, 255, 0, 0)
					end
                ...
                elseif (logged==1) then
                    if exports.global:hasItem(targetPlayer, itemID, itemValue) then
                        if (commandName == "delplayeritem") then
                            outputChatBox("Has borrado el item " .. displayItemName .. " con el valor de (" .. itemValue .. ") de el inventario de " .. targetPlayerName .. ".", thePlayer, 0, 255, 0)
                        ...
                        elseif (commandName == "takeitem") then
                            outputChatBox("Has tomado " .. displayItemName .. " con el valor de (" .. itemValue .. ") de " .. targetPlayerName .. " y se transfirió a tu inventario.", thePlayer, 0, 255, 0)
                            exports.global:takeItem(targetPlayer, itemID, itemValue)
                        ...
                        outputChatBox("El jugador no tiene este item", thePlayer, 255, 0, 0)
                    end
                end
            end
        end
...
function setPlayerHealth(thePlayer, commandName, targetPlayer, health)
	if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerScripter(thePlayer)) then
		if not tonumber(health) or not (targetPlayer) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador / ID] [Vida]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayer)
			...
			if targetPlayer then
				...
				elseif not setElementHealth(targetPlayer, tonumber(health)) then
					outputChatBox("Número inválido de vida.", thePlayer, 255, 0, 0)
				else
					--mysql:query_free("UPDATE characters SET health=0 WHERE id="..getElementData(targetPlayer, 'dbid'))
					outputChatBox("El jugador " .. targetPlayerName .. " ha recibido " .. health .. " de Vida.", thePlayer, 0, 255, 0)
...
function setPlayerArmour(thePlayer, commandName, targetPlayer, armor)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if not (armor) or not (targetPlayer) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador / ID] [Armor]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayer)
...
function setPlayerArmour(thePlayer, theCommand, targetPlayer, armor)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if not (targetPlayer) or not (armor) then
			outputChatBox("SYNTAX: /" .. theCommand .. " [Nombre del Jugador / ID] [Type]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayer)
			if targetPlayer then
				local logged = getElementData(targetPlayer, "loggedin")
...
function setPlayerSkinCmd(thePlayer, commandName, targetPlayer, skinID, clothingID)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if not (skinID) or not (targetPlayer) then -- Clothing ID is a optional argument
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador / ID] [Skin ID] (Clothing ID)", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayer)
...
function asetPlayerName(thePlayer, commandName, targetPlayer, ...)
	if (exports.global:isStaffOnDuty(thePlayer)) then
		if not (...) or not (targetPlayer) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del jugador / ID] [Nuevo Nombre]", thePlayer, 255, 194, 14)
		else
			...
			if targetPlayer then
				...
				if hoursPlayed > 5 and not exports.integration:isPlayerAdmin(thePlayer) then
					outputChatBox( "Solo un admin mayor puede realizar este cambio de nombre, debido a que tiene menos de 5 horas.", thePlayer, 255, 0, 0)
					return false
				end
				if newName == targetPlayerName then
					outputChatBox( "El nombre del jugador es el que ya tiene.", thePlayer, 255, 0, 0)
				else
					...
					if (mysql:num_rows(result)>0) then
						outputChatBox("El nombre está en uso.", thePlayer, 255, 0, 0)
					else
						...
						if (name) then
							...
							if (hiddenAdmin==0) then
								exports.global:sendMessageToAdmins("AdmCmd: " .. tostring(adminTitle) .. " " .. getPlayerName(thePlayer) .. " cambio el nombre de " .. targetPlayerName .. " a " .. newName .. ".")
								outputChatBox("Tu nombre de PJ ha cambiado de '"..targetPlayerName .. "' a '" .. tostring(newName) .. "' por "..adminTitle.." "..getPlayerName(thePlayer)..".", targetPlayer, 0, 255, 0)
							else
								outputChatBox("Tu nombre de PJ ha cambiado de '"..targetPlayerName .. "' a " .. processedNewName .. "' por un Admin Oculto.", targetPlayer, 0, 255, 0)
							end
							outputChatBox("Has cambiado el nombre de " .. targetPlayerName .. " a '" .. processedNewName .. "'.", thePlayer, 0, 255, 0)
		...
		if not (targetPlayer) then
			...
			if targetPlayer then
				...
				elseif (targetPlayerPower > thePlayerPower) then -- Check the admin isn't slapping someone higher rank them him
					outputChatBox("Este jugador tiene un rango mayor al tuyo.", thePlayer, 255, 0, 0)
				else
					...
					if (hiddenAdmin==0) then
						local adminTitle = exports.global:getPlayerAdminTitle(thePlayer)
						exports.global:sendMessageToAdmins("AdmCmd: " .. tostring(adminTitle) .. " " .. getPlayerName(thePlayer) .. " golpeó a " .. targetPlayerName .. ".")
					end
...
function hiddenOOC(thePlayer, commandName, ...)
	...
	if (exports.integration:isPlayerLeadAdmin(thePlayer)) then
		if not (...) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Mensaje]", thePlayer, 255, 194, 14)
		else
...
function hiddenWhisper(thePlayer, command, who, ...)
	if (exports.integration:isPlayerLeadAdmin(thePlayer)) then
		if not (who) or not (...) then
			outputChatBox("SYNTAX: /" .. command .. " [Nombre del Jugador / ID] [Message]", thePlayer, 255, 194, 14)
		else
			...
			if (targetPlayer) then
				...
				if (logged==1) then
					...
					outputChatBox("MP de Admin Oculto: " .. message, targetPlayer, 255, 194, 14)
					outputChatBox("MP oculto enviado a " .. targetPlayerName .. ": " .. message, thePlayer, 255, 194, 14)
				elseif (logged==0) then
					outputChatBox("Jugador no logueado yet.", thePlayer, 255, 194, 14)
				end
			end
		end
	end
end
addCommandHandler("mpo", hiddenWhisper, false, false)

-- Kick
function kickAPlayer(thePlayer, commandName, targetPlayer, ...)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if not (targetPlayer) or not (...) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador] [Razón]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayer)
				...
				if (targetPlayerPower <= thePlayerPower) then
					...
					if (hiddenAdmin==0) then
						if commandName ~= "skick" then
							local adminTitle = exports.global:getPlayerAdminTitle(thePlayer)
							exports.global:sendMessageToAdmins("[PKICK]: " .. adminTitle .. " " .. playerName .. " kickeó a " .. targetPlayerName .. " del juego.")
							exports.global:sendMessageToAdmins("[PKICK]: Razón: " .. reason .. ".")

						end
						kickPlayer(targetPlayer, thePlayer, reason)
					else
						if commandName ~= "skick" then
							exports.global:sendMessageToAdmins("[PKICK]: "..targetPlayerName.." ha sido kickeado del juego.")
							exports.global:sendMessageToAdmins("[PKICK]: Razón: " .. reason .. ".")
						end
					...
					outputChatBox(" Este jugador tiene un rango mayor al tuyo.", thePlayer, 255, 0, 0)
					outputChatBox(playerName .. " intentó kickearte, pero debido a que eres mayor rango no pudo.", targetPlayer, 255, 0 ,0)
				end
			end
		end
	end
...
function setMoney(thePlayer, commandName, target, money, ...)
	if exports["integration"]:isPlayerAdmin(thePlayer) then
		if not (target) or not money or not tonumber(money) or not (...) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador] [Dinero] [Razón]", thePlayer, 255, 194, 14)
		else
			local username = getPlayerName(thePlayer)
			...
			if targetPlayer then
				...
				if money and money > 500000 then
					outputChatBox("Por razones de seguridad, no puedes poner más de 500.000.", thePlayer, 255, 0, 0)
					return false
				end
				...
				reason = table.concat({...}, " ")
				outputChatBox(targetPlayerName .. " ha recibido " .. amount .. " $.", thePlayer)
				outputChatBox("Admin " .. username .. " ha puesto tu dinero en " .. amount .. " $.", targetPlayer)
				outputChatBox("Razón: " .. reason .. ".", targetPlayer)
				...
				exports.global:sendMessageToAdmins("[SETMONEY] Admin " .. username .. " ha puesto a ("..targetUsername..") "..targetCharacterName.." con  $" .. amount.." de dinero ("..reason..")." )
			end
		end
	end
end
	...
	if exports["integration"]:isPlayerAdmin(thePlayer) then
		if not (target) or not money or not (...) then
			...
			if targetPlayer then
				money = tonumber(money) or 0
				if money and money > 500000 then
					outputChatBox("Por una razón de seguridad, no puedes dar más de $500.000.", thePlayer, 255, 0, 0)
					return false
				end

				if not exports.global:giveMoney(targetPlayer, money) then
					outputChatBox("No se pudo dar ese dinero.", thePlayer, 255, 0, 0)
					return false
				end
				...
				local amount = exports.global:formatMoney(money)
				reason = table.concat({...}, " ")
				outputChatBox("Has dado a " .. targetPlayerName .. " $" .. amount .. ".", thePlayer)
				outputChatBox("Admin " .. username .. " te dió: $" .. amount .. ".", targetPlayer)
				outputChatBox("Razón: " .. reason .. ".", targetPlayer)
				...
				exports.global:sendMessageToAdmins("[GIVEMONEY] Admin " .. username .. " dió dinero ("..targetUsername..") "..targetCharacterName.." $" .. amount .. " ("..reason..").")
			end
		end
	end
end
		...
		if not (target) then
			...
			if targetPlayer then
				...
				if (veh) then
					...
					outputChatBox(" Has sido congelado ".. textStr ..". Sigue las instrucciones.", targetPlayer)
					outputChatBox(" Has congelado " ..targetPlayerName.. ".", thePlayer)
				else
					detachElements(targetPlayer)
					toggleAllControls(targetPlayer, false, true, false)
...
function unfreezePlayer(thePlayer, commandName, target)
	if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) then
		if not (target) then
			outputChatBox("SYNTAX: /" .. commandName .. " /unfreeze [Nombre del Jugador]", thePlayer, 255, 194, 14)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, target)
			if targetPlayer then
				local textStr = "admin"
				...
				if (veh) then
					...
					if (isElement(targetPlayer)) then
						outputChatBox(" Has sido descongelado ".. textStr ..". Gracias por su cooperación.", targetPlayer)
					end

					if (isElement(thePlayer)) then
						outputChatBox(" Has descongelado " ..targetPlayerName.. ".", thePlayer)
					end
				else
					toggleAllControls(targetPlayer, true, true, true)
					...
					outputChatBox(" Has sido descongelado ".. textStr ..". Gracias por su cooperación.", targetPlayer)
					outputChatBox(" Has descongelado a " ..targetPlayerName.. ".", thePlayer)
				end
...
function gmDuty(thePlayer, commandName)
	if exports.integration:isPlayerSupporter(thePlayer) then
		...
		if gmDuty == 0 then
			...
			exports.global:sendMessageToAdmins("SDuty: " .. username .. " está de servicio.")
		elseif gmDuty == 1 then
			triggerClientEvent(thePlayer, "accounts:settings:updateAccountSettings", thePlayer, "duty_supporter", 0)
			exports.global:sendMessageToAdmins("SDuty: " .. username .. " está fuera de servicio.")
		end
	end
end
	...
	if not (target) then
		...
		if targetPlayer then
			...
			if (logged==1) then
				...
				outputChatBox("** " .. targetPlayerName .. " ID= " .. id .. ".", thePlayer, 255, 194, 14)
			else
				outputChatBox("Jugador no logueado.", thePlayer, 255, 0, 0)
			end
		end
...
function ejectPlayer(thePlayer, commandName, target)
	if not (target) then
		outputChatBox("SYNTAX: /" .. commandName .. " [Nombre Jugador]", thePlayer, 255, 194, 14)
	else
		if not (isPedInVehicle(thePlayer)) then
			outputChatBox("No estás en un vehiculo.", thePlayer, 255, 0, 0)
		else
			local vehicle = getPedOccupiedVehicle(thePlayer)
			local seat = getPedOccupiedVehicleSeat(thePlayer)

			if (seat~=0) then
				outputChatBox("Tienes que ser el conductor para realizar esto.", thePlayer, 255, 0, 0)
			else
				local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, target)

				if not (targetPlayer) then
				elseif (targetPlayer==thePlayer) then
					outputChatBox("No puedes tirarte a ti mismo.", thePlayer, 255, 0, 0)
				else
					local targetvehicle = getPedOccupiedVehicle(targetPlayer)
					...
					if targetvehicle~=vehicle and not exports.integration:isPlayerTrialAdmin(thePlayer) then
						...
						outputChatBox("Has tirado a " .. targetPlayerName .. " fuera de tu vehículo", thePlayer, 0, 255, 0)
						removePedFromVehicle(targetPlayer)
						exports.anticheat:changeProtectedElementDataEx(targetPlayer, "realinvehicle", 0, false)
...
addCommandHandler("tirar", ejectPlayer, false, false)
	...
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if not (targetPlayer) or not (...) then
			...
			if targetPlayer then
				...
				outputChatBox("Has puesto una advertencia a " .. targetPlayerName .. " (" .. warns .. "/3).", thePlayer, 255, 0, 0)
				outputChatBox("Te han dado una advertencia. Emisor:" .. adminTitle.." "..adminUsername .. ".", targetPlayer, 255, 0, 0)
				outputChatBox("Razón: " .. reason, targetPlayer, 255, 0, 0)
				...
				exports.global:sendMessageToAdmins("[WARN]: " .. adminTitle .. " " .. adminUsername .. " dio una advertencia a " .. targetPlayerName .. ". (" .. warns .. "/3)")
				exports.global:sendMessageToAdmins("[WARN]: Razón: " .. reason)

				if (warns>=3) then
					addAdminHistory(targetPlayer, thePlayer, warns .. ' Admin Warnings', 5 , 0)
					exports.bans:addToBan(accountID, getPlayerSerial(targetPlayer), getPlayerIP(targetPlayer), 0, "3 AdvAdmin")
					kickPlayer(targetPlayer, getRootElement(), "Auto-Baneo 3/3 ADV.")
					for index, player in pairs(getElementsByType("player")) do
						if tonumber( getElementData( player, "punishment_notification_selector") ) ~= 1 or player == thePlayer or player == targetPlayer then
							outputChatBox("[BAN]: " .. targetPlayerName .. " ha sido auto baneado por tener 3/3 advertencias.", player, 255, 0, 0)
						end
					end
				else
        ...
        if not (...) then
            ...
            if charid then
                ...
                if mysql:query_free("UPDATE characters SET `money`='500', `bankmoney`='1000' WHERE id = " .. mysql:escape_string(charid) ) then
					...
					exports.global:sendMessageToAdmins("[RESET-CHARACTER]: " .. tostring(adminTitle) .. " ha reiniciado el pj " .. character .. ".")
		...
		if not (...) then
			...
			if charid then
				...
				if hiddenAdmin == 0 then
					...
					exports.global:sendMessageToAdmins("AdmCmd: un Admin Oculto has reset " .. character .. "'s spawn position.")
				end
		...
		if not (targetPlayer) then
			...
			if (logged==0) then
			   outputChatBox("Jugador no logueado.", thePlayer, 255, 0, 0)
				...
				outputChatBox("Has empujado a " .. targetPlayerName .. ".", thePlayer)
				outputChatBox("Has sido empujado por " .. getPlayerName(thePlayer) .. ".", targetPlayer)
			end
		end
	end
end
...
function earthquake(thePlayer, commandName, shakeIntensity)
	if exports.integration:isPlayerLeadAdmin(thePlayer) then
		if not tonumber(shakeIntensity) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Intensidad del movimiento de pantalla]", thePlayer, 255, 194, 14)
			return
		end
		local players = exports.pool:getPoolElementsByType("player")
		for index, arrayPlayer in ipairs(players) do
...
function asetPlayerAge(thePlayer, commandName)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		outputChatBox("Usa /setdob.", thePlayer, 255, 194, 14)
	end
end
    ...
    function asetPlayerHeight(thePlayer, commandName, targetPlayer, height)
       if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
          if not (height) or not (targetPlayer) then
             outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador / ID] [Height (150 - 200)]", thePlayer, 255, 194, 14)
          else
             local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayer)
             local dbid = getElementData(targetPlayer, "dbid")
    ...
    function asetPlayerRace(thePlayer, commandName, targetPlayer, race)
       if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
          if not (race) or not (targetPlayer) then
             outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador / ID] [0= Black, 1= White, 2= Asian]", thePlayer, 255, 194, 14)
          else
             local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayer)
             local dbid = getElementData(targetPlayer, "dbid")
             race = tonumber(race)
    ...
    function asetPlayerGender(thePlayer, commandName, targetPlayer, gender)
       if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
          if not (gender) or not (targetPlayer) then
             outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador / ID] [0= Hombre, 1= Mujer]", thePlayer, 255, 194, 14)
          else
             local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayer)
             local dbid = getElementData(targetPlayer, "dbid")
             gender = tonumber(gender)
             if (gender>1) or (gender<0) then
                outputChatBox("Error: Usa 0 para hombre, o 1 para mujer.", thePlayer, 255, 0, 0)
             else
			 ...
			 exports.anticheat:setEld(targetPlayer, "gender", gender, 'all')
				if (gender==0) then
				    outputChatBox("Has cambiado el genero de " .. targetPlayerName .. " a Hombre.", thePlayer, 0, 255, 0)
				    outputChatBox("Tu género ha cambiado a Hombre.", targetPlayer, 0, 255, 0)
				elseif (gender==1) then
					outputChatBox("Has cambiado el genero de " .. targetPlayerName .. " a Mujer.", thePlayer, 0, 255, 0)
				    outputChatBox("Tu género ha cambiado a Mujer.", targetPlayer, 0, 255, 0)
				end
...
function aSetDateOfBirth(thePlayer, commandName, targetPlayer, dob, mob, year)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if not (targetPlayer) or not dob or not mob or not year then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador / ID] [Day] [Month] [Year]", thePlayer, 255, 194, 14)
		else
	...
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.factions:isInFactionType(thePlayer, 4) then
		if not (targetPlayer) then
			...
			if targetPlayer then
				...
				outputChatBox("Ya no estás en recuperatorios!", targetPlayer, 0, 255, 0) -- Let them know about it!
				outputChatBox("Has quitado a "..targetPlayerName.." de el recuperatorio!", thePlayer, 0, 255, 0)
...
function checkSkin ( thePlayer, commandName)
	outputChatBox ( "Tu ID de skin es: " .. getPedSkin ( thePlayer ), thePlayer)
end
...
function giveSuperman(thePlayer, commandName, targetPlayer)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if not targetPlayer then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador / ID] - Dar permisos de volar + freecam", thePlayer, 255, 194, 14)
			outputChatBox("Usa denuevo el comando si el jugador reconecta.", thePlayer, 200, 150, 0)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayer)
			if not targetPlayer then
				outputChatBox("Player not found.",thePlayer, 255,0,0)
			...
			if not canFly then
				if setElementData(targetPlayer, "canFly", true) then
					outputChatBox("Has dado a "..targetPlayerName.." el permiso de volar.", thePlayer, 0, 255, 0)
					if (hiddenAdmin==0) then
						outputChatBox(adminTitle.." "..getPlayerName(thePlayer):gsub("_", " ").." te ha dado el permiso de volar.", targetPlayer, 0, 255, 0)
						outputChatBox("TIP: /superman o salta para volar.", targetPlayer, 255, 255, 0)
						outputChatBox("TIP: /freecam para habilitar la cámara libre, /dropme para desactivarla.", targetPlayer, 255, 255, 0)
						...
						outputChatBox("un Admin Oculto te ha dado el permiso de volar.", targetPlayer, 0, 255, 0)
						outputChatBox("TIP: /superman o salta para volar.", targetPlayer, 255, 255, 0)
						outputChatBox("TIP: /freecam para habilitar la cámara libre, /dropme para desactivarla.", targetPlayer, 255, 255, 0)
						exports.global:sendMessageToAdmins("[ADMWARN] un Admin Oculto has given " .. getPlayerName(targetPlayer):gsub("_"," ") .. " temporary ability to fly.")
					end
					exports.logs:dbLog(thePlayer, 4, targetPlayer, "GIVESUPERMAN")
				end
			else
				if setElementData(targetPlayer, "canFly", false) then
					outputChatBox("has quitado el permiso de volar a "..targetPlayerName..".", thePlayer, 255, 0, 0)
					if (hiddenAdmin==0) then
						outputChatBox(adminTitle.." "..getPlayerName(thePlayer):gsub("_", " ").." te ha quitado el permiso de volar temporalmente.", targetPlayer, 255, 0, 0)
						...
						outputChatBox("un Admin Oculto te ha quitado el permiso de volar.", targetPlayer, 255, 0, 0)
						exports.global:sendMessageToAdmins("AdmWrn: un Admin Oculto has revoked from " .. getPlayerName(targetPlayer):gsub("_"," ") .. " temporary ability to fly.")
					end
				end
			end
		end
...
function giveTempMove(p, c, t)
	if exports.integration:isPlayerTrialAdmin(p, true) then
		if not t then
			outputChatBox("SYNTAX: /" .. c .. " [Nombre del Jugador / ID] - Give access to moveitem in the world.",p, 255, 194, 14)
		else
			...
			if targetPlayer then
				...
				if getElementData(targetPlayer, "admin:tempmove") then
					...
					outputChatBox(adminTitle.." "..getPlayerName(p):gsub("_", " ").." te ha quitado permisos de /moveitem en dimension 0.", targetPlayer, 0, 255, 0)
					...
					outputChatBox(adminTitle.." "..getPlayerName(p):gsub("_", " ").." te ha dado permisos de /moveitem en la dimension 0.", targetPlayer, 0, 255, 0)
...
function setPlayerInterior(thePlayer, commandName, targetPlayer, interiorID)
	if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerScripter(thePlayer)) then
		...
		if (not targetPlayer) or (not interiorID) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador / ID] [Interior ID]", thePlayer, 255, 194, 14, false)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayer)
			if targetPlayer then
				local logged = getElementData(targetPlayer, "loggedin")
				if (logged == 0) then
					outputChatBox("Jugador no logueado.", thePlayer, 255, 0, 0, false)
				else
...
function setPlayerDimension(thePlayer, commandName, targetPlayer, dimensionID)
	if (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerScripter(thePlayer)) then
		...
		if (not targetPlayer) or (not dimensionID) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador / ID] [Dimension ID]", thePlayer, 255, 194, 14, false)
		else
			local targetPlayer, targetPlayerName = exports.global:findPlayerByPartialNick(thePlayer, targetPlayer)
			if targetPlayer then
				local logged = getElementData(targetPlayer, "loggedin")
...
function charityGC(thePlayer, cmd, amount)
	if not amount or not tonumber(amount) or tonumber(amount) <= 0 then
		outputChatBox("SYNTAX: /" .. cmd .. " [Cantidad de Donacion]", thePlayer, 255, 194, 14)
	else
		...
		local qh = dbQuery(function(qh, thePlayer, amount, id)
				...
				if result and #result > 0 then
					...
					if currentGC < amount then
						outputChatBox("No tienes los suficentes GCs!", thePlayer, 255, 0, 0)
						return
					end
					...
					if dbExec(exports.mysql:getConn("core"), "UPDATE `accounts` SET `credits`=`credits`-? WHERE `id`=? ", amount, id) then
						...
						outputChatBox("Has donado "..amount.." GCs.", thePlayer, 0, 255, 0)
	...
	if exports.integration:isPlayerAdmin(thePlayer) then
		...
		for k, arrayPlayer in ipairs(players) do
			...
			if (playerping == 00) or (playerping>1000) then
				kickPlayer(arrayPlayer, getRootElement(), "Problemas con tu ping, reconecta.")
				kickcount = kickcount + 1
			end
		end
		exports.global:sendMessageToAdmins("AdmWrn: " .. getElementData(thePlayer, "account:username") .. " ha kickeado a "..kickcount.." por problemas con el ping ." )
	end
end
		...
		if not target then
			...
			if targetPlayer then
				if getElementData( targetPlayer, 'loggedin' ) == 1 then
					...
					dbQuery( function( qh, player, cmd, targetPlayer, targetPlayerName, id )
						...
						if res and #res==1 then
							...
							if to <= limit then
								...
								outputChatBox( "Has extendido correctamente el nombre "..exports.global:getPlayerFullIdentity( targetPlayer ).."'s character cap to "..res[1].cur.."/"..to..".", player, 0, 255, 0 )
								outputChatBox( exports.global:getPlayerFullIdentity( player ).." ha extendido tu limite de caracteres "..res[1].cur.."/"..to..".", targetPlayer, 0, 255, 0 )
							else
					...
					outputChatBox( "Jugador no logueado.", player, 255, 0, 0 )
				end
			end
		end
	end
...
function clearWhois(thePlayer, command)
	if exports.integration:isPlayerHeadAdmin(thePlayer) or exports.integration:isPlayerLeadAdmin(thePlayer) then
		...
		outputChatBox("Tu IP y Serial han sido limpiados.", thePlayer, 0, 255, 0)
	end
end
