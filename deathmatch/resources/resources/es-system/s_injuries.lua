...
function injuries(attacker, weapon, bodypart, loss)
	...
	if ( bodypart == 3 and loss >= 15 ) or bodypart == 7 or bodypart == 8 then -- damaged either left or right leg
		...
		executeCommandHandler('ame', source, "su pierna  " .. ( bodypart == 7 and "izquierda" or "derecha" ) .. " ha sido golpeada.")
	elseif bodypart == 5 or bodypart == 6 then -- damaged either arm
		...
		executeCommandHandler('ame', source, "su brazo " .. ( bodypart == 5 and "izquierdo" or "derecho" ) .. " ha sido golpeado.")
	elseif bodypart == 3 then
		if not weapon == 17 then
			executeCommandHandler('ame', source, "su torso ha sido golpeado.", source)
		end
	elseif bodypart == 9 then -- headshot
		if (weapon >= 22 and weapon <=38) then
			...
			if not tazer then
				setPedHeadless(source, true)
				executeCommandHandler('ame', source, "su cabeza ha sido golpeada.", source)
...
function doHeadHit(headHitPlayer)
	outputChatBox("Te han golpeado en la cabeza", headHitPlayer, 255, 0, 0)
...
function examine(to)
	...
	if isPedDead(source) then
		outputChatBox(name .. " esta muerto.", to, 255, 0, 0)
	elseif playerInjuries[source] and not isPedHeadless(source) then
		if playerInjuries[source]['knockout'] then
			outputChatBox(name.. " esta noqueado.", to, 255, 255, 0)
		end

		if playerInjuries[source][7] and playerInjuries[source][8] then
			outputChatBox("Las piernas de " .. name .. " están rotas.", to, 255, 255, 0)
		elseif playerInjuries[source][7] then
			outputChatBox(name .. "su pierna izquierda está rota.", to, 255, 255, 0)
		elseif playerInjuries[source][8] then
			outputChatBox(name .. "su pierna derecha está rota.", to, 255, 255, 0)
		end

		if playerInjuries[source][5] and playerInjuries[source][6] then
			outputChatBox("Los brazos de " .. name .. " están rotos.", to, 255, 255, 0)
		elseif playerInjuries[source][5] then
			outputChatBox(name .. "su brazo izquierdo está roto.", to, 255, 255, 0)
		elseif playerInjuries[source][6] then
			outputChatBox(name .. "su brazo derecho está roto.", to, 255, 255, 0)
		end
	else
...
function lvesHeal(thePlayer, commandName, targetPartialNick, price)
	if not (targetPartialNick) or not (price) then -- if missing target player arg.
		outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del Jugador / ID] [Precio de la Cura]", thePlayer, 255, 194, 14)
	else
		...
		if targetPlayer then -- is the player online?
			...
			if (logged==1) then
				if not (exports.factions:isInFactionType(thePlayer, 4)) then
					outputChatBox("No tienes las habilidades necesarias, avisa a RCES.", thePlayer, 255, 0, 0)
				...
				elseif (isPedDead(targetPlayer)) then
					outputChatBox("Ésta persona está muerta", thePlayer, 255, 0, 0)
				else
					price = tonumber(price)
					if price > 500 then
						outputChatBox("Ha tardado en responder.", thePlayer, 255, 0, 0)
					else
						local x, y, z = getElementPosition(thePlayer)
						local tx, ty, tz = getElementPosition(targetPlayer)
						
						if (getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)>3) then -- Are they standing next to each other?
							outputChatBox("Estás muy lejos de '".. targetPlayerName .."' para curarlo.", thePlayer, 255, 0, 0)
						else
							...
							if (foundkit) then
								...
								if money + bankmoney < price then
									outputChatBox("El usuario no puede pagar.", thePlayer, 255, 0, 0)
								else
									...
									exports.global:giveMoney( getTeamFromName("Riverside Municipal Government"), math.ceil(tax*price) )
									
									setElementHealth(targetPlayer, 100)
									triggerEvent("onPlayerHeal", targetPlayer, true)
									outputChatBox("Has curado a '" ..targetPlayerName.. "'.", thePlayer, 0, 255, 0)
									outputChatBox("Has sido curado por '" ..getPlayerName(thePlayer).. "' precio: $" .. price .. ".", targetPlayer, 0, 255, 0)
									...
									if itemValue > 1 then
										...
										if not exports.global:hasItem(thePlayer, 70) then
											outputChatBox("Atencion, no tienes mas kits de primeros auxilios. Re-equipate del /duty.", thePlayer, 255, 0, 0)
										end
									end
								end
							else
								outputChatBox("Necesitas un kit de primeros auxilios para curar.", thePlayer, 255, 0, 0)
							end
						end
					end
				end
			...
			if logged==1 then
				...
				if not (exports.factions:isInFactionType(thePlayer, 4)) then
					...
					if (getDistanceBetweenPoints3D(x, y, z, tx, ty, tz)>5) then -- Are they standing next to each other?
						outputChatBox("Estás muy lejos de '"..targetPlayerName.."' para examinarlo.", thePlayer, 255, 0, 0)
					else
