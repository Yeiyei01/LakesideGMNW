        ...
        if (getDistanceBetweenPoints3D(x, y, z, Mx, My, Mz) <= 20) then
           local WorldPositionX, WorldPositionY = getScreenFromWorldPosition(x, y, z +1, 0.07)
            if (WorldPositionX and WorldPositionY) then
			    dxDrawText("Trabajo Tractorista", WorldPositionX - 1, WorldPositionY + 1, WorldPositionX - 1, WorldPositionY + 1, tocolor(0, 0, 0, 255), 1.52, "default-bold", "center", "center", false, false, false, false, false)
			    dxDrawText("Trabajo Tractorista", WorldPositionX - 1, WorldPositionY + 1, WorldPositionX - 1, WorldPositionY + 1, tocolor(255, 255, 255, 255), 1.50, "default-bold", "center", "center", false, false, false, false, false)
            end
      end
end 
)
...
wndTra = guiCreateWindow(0,0,400,120,"Trabajo Tractorista",false)
centerWindow(wndTra)
guiSetVisible(wndTra,false)

labeTra = guiCreateLabel(5,25,390,50,"Bienvenido al Trabajo de Tractorero. Nuestra tarea es cosechar la plantacion en el campo.",false,wndTra)
...
buttonTra2 = guiCreateButton(210,70,180,35,"Cerrar",false,wndTra)
...
function startClick()
    ...
    if (source == buttonTra1) then
        ...
        if not state then
            ...
            outputChatBox("#1E90FF[Conductor Tractor] #FFFFFFHas comenzado el trabajo. Ande a cosechar.",255,255,255,true)
                ...
                if veh then
                    ...
                    if getElementData(player,"tractor_plus") >= #tractorMarks then
                        ...
                        if pacienty > 0 then
                            ...
                            deleteMarkerTractor(player)
                            outputChatBox("#1E90FF[Conductor Tractor] #FFFFFFEl trabajo está hecho. Ganancia total: #00FF00"..getElementData(player,"Заработано_Всего_Кон").." #FFFFFF$",255,255,255,true)
                            outputChatBox("#1E90FF[Conductor Tractor] #FFFFFFVe a donde el empleador para que te den tu paga.",255,255,255,true)
                        end
                    else
                        deleteMarkerTractor(player)
                        createMarkerTractor(player)
...
addEventHandler ( "onClientMarkerHit", getRootElement(), function(ply)
	...
	if source == markerT then
        ...
        if not veh then
            ...
            showCursor(true)
            if not state then
                guiSetText(buttonTra1,"Comenzar")
            else
                guiSetText(buttonTra1,"Completar")
            end
        end
	end
end)