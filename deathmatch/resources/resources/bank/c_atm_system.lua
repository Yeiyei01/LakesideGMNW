...
function requestATMInterfacePIN(theATM, atmLocationName)
	...
	if not (wPIN) then
		...
		wPIN = guiCreateWindow(x, y, width, height, "Cajero ATM #"..getElementData(theATM, "dbid").." at "..(atmLocationName or "Unknown Area"), false)
			guiWindowSetSizable(wPIN, false)

		local tabPanel = guiCreateTabPanel(0.05, 0.05, 0.9, 0.85, true, wPIN)
		local tabPersonal = guiCreateTab("Porfavor, ingrese el codigo PIN ", tabPanel)
		...
		Label_Error = guiCreateLabel(20,90,220,30,"Porfavor, ingrese un codigo pin de 4 dígitos",false,tabPersonal)
...
function respondToATMInterfacePIN(Label_Error_Msg, r,g,b, action, otherInfo)
	...
	if action == "cardRemoved" then
		exports.hud:sendBottomNotification(localPlayer, "Cajero ATM", "El cajero automático no funciona correctamente porque se ha retirado la tarjeta.")
		setTimer(closeATMInterfacePIN, 2000, 1)
	elseif action == "failedLessThan3" then
		exports.hud:sendBottomNotification(localPlayer, "Cajero ATM", "Acceso denegado. Si se introduce un PIN incorrecto más de 3 veces, el cajero automático se tragará la tarjeta. ("..otherInfo.."/3)")
		guiSetEnabled(bEnter, true)
	elseif action == "failedMoreThan3" then
		exports.hud:sendBottomNotification(source, "Cajero ATM", "Acceso denegado ("..otherInfo.."/3). El cajero automático se ha tragado su tarjeta. Por favor, póngase en contacto con el Banco.")
		setTimer(closeATMInterfacePIN, 2000, 1)
	elseif action == "locked" then
		exports.hud:sendBottomNotification(source, "Cajero ATM", "Tarjeta de cajero automático numero '"..otherInfo.."' está bloqueada y no se puede usar. Por favor, contactese con el banco.")
		guiSetEnabled(bEnter, true)
	elseif action == "success" then
		exports.hud:sendBottomNotification(source, "Cajero ATM", "Acceso concedido")
...
function openATMGrantedGUI(cardInfo, depositable, limit, withdrawable, theATM, atmLocationName, isPinCodeDefault)
	if not (guiATMGranted) then
		...
		guiATMGranted = guiCreateWindow(x, y, width, height, "Cajero ATM #"..getElementData(theATM, "dbid").." at "..(atmLocationName or "Unknown Area"), false)
		...
		local lBankName = guiCreateLabel(0.1, 0.05, 0.9, 0.05, "Dueño de la tarjeta:                           "..(cardInfo[1] or "Unknown"):gsub("_", " "), true, tabPersonal)
		guiSetFont(lBankName, "default-bold-small")

		local lBankNumber = guiCreateLabel(0.1, 0.1, 0.9, 0.05, "Numero de la tarjeta bancaria:       "..(cardInfo[2] or "Unknown"), true, tabPersonal)
		...
		if withdrawable then
			...
			addEventHandler("onClientGUIClick", bWithdrawP, function()
				...
				if not amount or amount <= 0 or math.ceil( amount ) ~= amount then
					exports.hud:sendBottomNotification(localPlayer, "Cajero ATM", "Por favor, ingrese una cantidad positiva")
				elseif (amount>money) then
					exports.hud:sendBottomNotification(localPlayer, "Cajero ATM", "No tienes los fondos suficientes.")
				elseif not _depositable and limitedwithdraw ~= 0 and oldamount + amount > limitedwithdraw then
					exports.hud:sendBottomNotification(localPlayer, "Cajero ATM", "Este cajero automático sólo permite retirar $" .. exports.global:formatMoney( limitedwithdraw - oldamount ) .. ".")
				else
			...
			lWithdrawP = guiCreateLabel(0.1, 0.15+posYOffset, 0.5, 0.05, "Este cajero automático no admite la función de retirar.", true, tabPersonal)
			guiSetFont(lWithdrawP, "default-bold-small")
		end
		...
		if (depositable) then
			...
			addEventHandler("onClientGUIClick", bDepositP, function()
				...
				elseif not exports.global:hasMoney(localPlayer, amount) then
					exports.hud:sendBottomNotification(localPlayer, "Cajero ATM", "No tienes los fondos necesarios.")
				else
					hideATMGrantedGUI()
			...
			lDepositP = guiCreateLabel(0.1, 0.25+posYOffset, 0.5, 0.05, "Este cajero no permite la funcion de depositar.", true, tabPersonal)
			lCharge = guiCreateLabel(0.1, 0.29+posYOffset, 0.5, 0.05, "Este cajero automático tiene una tarifa de 2 dólares por su uso.", true, tabPersonal)
			guiSetFont(lDepositP, "default-bold-small")
		...
		if hoursplayed >= 12 then
			...
			addEventHandler("onClientGUIClick", bTransferP, function()
				...
				elseif (amount>money) then
					exports.hud:sendBottomNotification(localPlayer, "Cajero ATM", "No tienes los fondos necesarios.")
				elseif reason == "" then
					exports.hud:sendBottomNotification(localPlayer, "Cajero ATM", "Por favor, ingrese una razón para la transacción")
				elseif bankAccNo == "" then
					exports.hud:sendBottomNotification(localPlayer, "Cajero ATM", "Por favor, introduzca el número de cuenta bancaria de destino para la transferencia")
				else
			...
			eTransferP = guiCreateEdit(0.66, 0.43+posYOffset, 0.3, 0.075, "<Nombre de cuenta/Facción>", true, tabPersonal)
...
function respondToPendingTransfer(msg, closeATMGUI)
	...
	exports.hud:sendBottomNotification(localPlayer, "Cajero ATM", msg)
end
...
function createTheRestOf(parent, isPinCodeDefault)
	...
	bEnter = guiCreateButton(20,344+posYOffset,225,66,"Ingresar",false,parent)
	addEventHandler( "onClientGUIClick", bEnter, function ()
		guiSetEnabled(bEnter, false)
		...
		if not cooldown and element and getElementType(element) =="object" and state=="up" and getElementParent(getElementParent(element)) == getResourceRootElement() then
			...
			if getDistanceBetweenPoints3D( px, py, pz, ax, ay, az ) < 1.3 then
				...
				if isAnyOneElseAround(element) then
					exports.hud:sendBottomNotification(localPlayer,"Este Cajero ya está en uso", "Por favor, póngase en la cola, un cajero sólo puede atender a una persona a la vez.")
				else
...
function atmCardExisted()
	...
	if not (wGui) then
		showCursor(true)
		--NEW CARD
		wGui = guiCreateWindow(X, Y, Width, Height, "'Ya tiene uno en nuestro sistema. ¿Qué debo hacer?'", false )
		option[1] = guiCreateButton( 0.05, verticalPos, 0.9, 1/numberOfButtons, "Dar de baja mi tarjeta anterior, hacer una nueva por favor.", true, wGui )
...
function chooseAtm()
	...
	if not (wGui) then
		showCursor(true)
		--NEW CARD
		wGui = guiCreateWindow(X, Y, Width, Height, "'¿Qué tipo de tarjeta de cajero automático quiere?'", false )
		option[1] = guiCreateButton( 0.05, verticalPos, 0.9, 1/numberOfButtons, "Tarjeta de Cajero Automático - Basica ($50)\nPuede utilizarse para realizar transacciones con una cantidad muy limitada al día. (($10,000 por 5 horas))", true, wGui )
		...
		option[3] = guiCreateButton( 0.05, verticalPos, 0.9, 1/numberOfButtons, "Tarjeta de Cajero Automático - Ultimate ($500)\nSe puede utilizar para realizar transacciones con una cantidad ilimitada por día.", true, wGui )
