...
function createATM(thePlayer, commandName, withdraw, deposit)
	if exports.integration:isPlayerTrialAdmin(thePlayer) then
		...
		elseif not (withdraw) or not (deposit) then
			outputChatBox("SYNTAX: /" .. commandName .. " [Límite de retirada / 0 = Infinito] [Depositar - 1 = Permitido, 0 = Desactivado]", thePlayer, 255, 194, 14)
			outputChatBox("Coloca /atmfast para poner un ATM default rapidamente.", thePlayer, 255, 194, 14)
		elseif (tonumber(withdraw)) and (tonumber(deposit)) then
			...
			if (tonumber(deposit) > 1) or (tonumber(deposit) < 0) then
				outputChatBox("SYNTAX: /" .. commandName .. " [Límite de retirada / 0 = Infinito] [Depositar - 1 = Pemitido, 0 = Desactivado]", thePlayer, 255, 194, 14)
			else
