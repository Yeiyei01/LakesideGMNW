...
function withdrawMoneyPersonal(amount)
	...
	if money >= 0 then
		...
		outputChatBox("Haz retirado $" .. exports.global:formatMoney(amount) .. " de tu cuenta personal.", client, 255, 194, 14)
...
function depositMoneyPersonal(amount)
	...
	if exports.global:takeMoney(client, amount, nil, true) then
			...
			outputChatBox("Ha depositado $" .. exports.global:formatMoney(amount) .. " en su cuenta personal.", client, 255, 194, 14)
			exports.logs:dbLog(client, 25, client, "DEPOSIT " .. amount.. " - REMAINING $"..money+amount)
	else
		outputChatBox("No tienes esa cantidad en una partida de dinero ordenada.", client, 255, 194, 14)
	end
end
...
function withdrawMoneyBusiness(amount, factionID)
	...
	if exports.global:takeMoney(theTeam, amount) then
		if exports.global:giveMoney(client, amount) then
			...
			outputChatBox("Haz retirado $" .. exports.global:formatMoney(amount) .. " de su cuenta de negocios.", client, 255, 194, 14)
...
function depositMoneyBusiness(amount, factionID)
	...
	if exports.global:takeMoney(client, amount) then
		...
		if exports.global:giveMoney(theTeam, amount) then
			...
			outputChatBox("Ha depositado $" .. exports.global:formatMoney(amount) .. " en su cuenta de negocios.", client, 255, 194, 14)
...
function transferMoneyToPersonal( fromFactionId, name, amount, reason )
	if getElementData( client, "loggedin" ) == 1 then
		...
		if not amount or not tonumber(amount) or tonumber(amount) <=0 then
			return not outputChatBox( "Cantidad invalida para transferir.", client, 255, 0, 0 )
		else
		...
		if fromFactionId then
			...
			if not sender then 
				return not outputChatBox( "Transacción fallida. Error Code: 147", client, 255, 0, 0 )
			end
			...
			if res then
				if nums > 0 then
					...
					if tonumber(res[1]['account']) == tonumber(senderAccountID) then -- Uh oh... bad boy!
						...
						return not outputChatBox( "No puedes transferir dinero a uno de tus personajes alternativos.", client, 255, 0, 0 )
					end
				else
					return not outputChatBox( "Destinatarios no encontrados con '"..name.."'. Prueba con un nombre de facción o de personaje.", client, 255, 0, 0 )
				end
			else
				dbFree( qh )
		...
		if sender == receiver then
			return not outputChatBox( "No te puedes transferir dinero a ti mismo. ", client, 255, 0, 0 )
		end
		...
		if hasBankMoney( sender, amount ) then
			if takeBankMoney( sender, amount ) then
				...
				return outputChatBox("Haz transferido $" .. famount .. " de tu cuenta "..(fromFactionId and "de negocios" or "personal").." a "..receiver_name..".", client, 255, 194, 14)
			else
				return not outputChatBox("Interal Error! Code 197", client, 255, 0, 0 )
			end
		else
			return not outputChatBox("Transacción fallida. Balance insuficiente", client, 255,0, 0)
		end
	end
end
