version = "Banco Nine Nine RP v3.0"
...
function startFactionSelect(fTable, selectiontype)
    ...
    factionSelectGUI.window[1] = guiCreateWindow(766, 385, 399, 121, "Seleccione la facción que desea ver", false)
    ...
    factionSelectGUI.combobox[1] = guiCreateComboBox(13, 35, 366, 113, "Seleccione la facción que desea ver", false, factionSelectGUI.window[1])
    factionSelectGUI.button[2] = guiCreateButton(268, 64, 111, 42, "Aceptar", false, factionSelectGUI.window[1])
    ...
    addEventHandler("onClientGUIClick", factionSelectGUI.button[2], function()
    	...
    	if name ~= "Seleccione la facción que desea ver" then
    		...
    		outputChatBox("Por favor, seleccione una facción.", 255, 0, 0)
    	end
    end, false)
...
function showBankUI(fTable, depositable, limit, withdrawable, ped)
	if not (wBank) then
		...
		wBank = guiCreateWindow(x, y, width, height, "Banco de Riverside | "..version.."", false)
		...
		if (exports.global:countTable(fTable) > 0) then
			tabBusiness = guiCreateTab("Banca de empresas", tabPanel)
			...
			-- WITHDRAWAL BUSINESS
				...
				lWithdrawB = guiCreateLabel(0.1, 0.15, 0.5, 0.05, "Este cajero ATM no permite la función de retirar.", true, tabBusiness)
				guiSetFont(lWithdrawB, "default-bold-small")
			end

			if (depositable) then
				-- DEPOSIT BUSINESS
				lDepositB = guiCreateLabel(0.1, 0.25, 0.2, 0.05, "Depositar:", true, tabBusiness)
				...
				lDepositB = guiCreateLabel(0.1, 0.25, 0.5, 0.05, "Este cajero ATM no permite la función de depositar.", true, tabBusiness)
				guiSetFont(lDepositB, "default-bold-small")
			...
			if hoursplayed >= 12 then
				...
				eTransferB = guiCreateEdit(0.66, 0.43, 0.3, 0.075, "<Numero de tarjeta>", true, tabBusiness)
				addEventHandler("onClientGUIClick", eTransferB, function()
					if guiGetText(eTransferB) == "<Numero de tarjeta>" then
			...
			-- TRANSACTION HISTORY
			tabBusinessTransactions = guiCreateTab("Transacciones de negocio", tabPanel)
		...
		if withdrawable then
			...
			lWithdrawP = guiCreateLabel(0.1, 0.15, 0.5, 0.05, "Este cajero ATM no permite la función de retirar.", true, tabPersonal)
			guiSetFont(lWithdrawP, "default-bold-small")
		end

		if (depositable) then
			-- DEPOSIT PERSONAL
			lDepositP = guiCreateLabel(0.1, 0.25, 0.2, 0.05, "Depositar:", true, tabPersonal)
			...
			bDepositP = guiCreateButton(0.44, 0.23, 0.2, 0.075, "Depositar", true, tabPersonal)
			addEventHandler("onClientGUIClick", bDepositP, depositMoneyPersonal, false)
		else
			lDepositP = guiCreateLabel(0.1, 0.25, 0.5, 0.05, "Este cajero ATM no permite la función de depositar.", true, tabPersonal)
			guiSetFont(lDepositP, "default-bold-small")
		...
		if hoursplayed >= 12 then
			...
			eTransferP = guiCreateEdit(0.66, 0.43, 0.3, 0.075, "<Jugador/Nombre de facción>", true, tabPersonal)
			addEventHandler("onClientGUIClick", eTransferP, function()
				if guiGetText(eTransferP) == "<Jugador/Nombre de facción>" then
					guiSetText(eTransferP, "")
				end
			end, false)

			lTransferPReason = guiCreateLabel(0.1, 0.55, 0.2, 0.05, "Razón:", true, tabPersonal)
...
function withdrawMoneyPersonal(button)
	if (button=="left") then
		...
		elseif (amount>money) then
			outputChatBox("No tienes los fondos suficientes.", 255, 0, 0)
		elseif not _depositable and limitedwithdraw ~= 0 and oldamount + amount > limitedwithdraw then
			outputChatBox("Este cajero automático sólo permite retirar $" .. exports.global:formatMoney( limitedwithdraw - oldamount ) .. ".")
		else
...
function transferMoneyPersonal(button)
	if (button=="left") then
		...
		elseif (amount>money) then
			outputChatBox("No tienes los fondos suficientes.", 255, 0, 0)
		elseif reason == "" then
			outputChatBox("Por favor, ingrese una razón de transacción", 255, 0, 0)
		elseif playername == "" then
			outputChatBox("Por favor, ingrese el nombre completo del recibidor", 255, 0, 0)
		else
...
function withdrawMoneyBusiness(button)
	if (button=="left") then
		...
		elseif (amount>gfactionBalance) then
			outputChatBox("No tienes los fondos suficientes.", 255, 0, 0)
		elseif not _depositable and limitedwithdraw ~= 0 and oldamount + amount > limitedwithdraw then
			outputChatBox("Este ATM solo permite retirar $" .. exports.global:formatMoney( limitedwithdraw - oldamount ) .. ".")
		else
			setElementData( lastUsedATM, "withdrawn", oldamount + amount, false )
			setTimer(
				function( atm, amount )
...
function transferMoneyBusiness(button)
	if (button=="left") then
		...
		elseif (amount>gfactionBalance) then
			outputChatBox("No tienes los fondos suficientes.", 255, 0, 0)
		elseif reason == "" then
			outputChatBox("Por favor, ingrese una razón para la transacción", 255, 0, 0)
		elseif playername == "" then
			outputChatBox("Por favor, ingrese el nombre completo del recibidor", 255, 0, 0)
		else
...
function getTransactionReason(type, reason, from)
	...
	elseif type == 7 then
		return "Salario (Negocio+Intereses+Donador)"
	elseif type == 8 then
		return "Presupuesto"
	elseif type == 9 then
		outputDebugString(tostring(reason) .. " " .. tostring(type))
		return tostring("Gasolina: "..math.ceil(tonumber(reason)).."L")
	elseif type == 10 then
		return "Reparación"
	else
		return "Transferencia: " .. tostring(reason or "")
	end
end
