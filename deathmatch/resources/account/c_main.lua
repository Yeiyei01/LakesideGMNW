...
local label = guiCreateLabel( 0, 0, screenX, 15, "Discord: ¡Click aquí! LsRPv"..exports.global:getScriptVersion().."", false )
...
addEventHandler('onClientGUIClick', label, function()
	local url = "https://discord.gg/Fb3RhgFFJS"
...
function updateSoundLabel(setting)
	if setting == 0 then 
		guiSetText(toggleSoundLabel, "[X] Musica")
	else 
		guiSetText(toggleSoundLabel, "[V] Musica")
	end
end
...
addEventHandler("account:showMusicLabel", root, function()
	toggleSoundLabel = guiCreateLabel( 0, 0, screenX, 15, "[V] Musica", false )
