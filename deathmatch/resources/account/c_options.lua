...
function options_showmenu()
	...
	bChangeCharacter = guiCreateButton(margin, margin, 230, 30, "Cambiar PJ", false, wOptions)
	...
	bStatistics = guiCreateButton(margin, wHeight, 230, 30, "Estadísticas del PJ", false, wOptions)
	...
	bGraphicsSettings = guiCreateButton(margin, wHeight, 230, 30, "Configuraciones", false, wOptions)
	...
	if getResourceFromName("donators") then
		bStore = guiCreateButton(margin, wHeight, 230, 30, "Opciones Premium", false, wOptions)
		--addEventHandler("onClientGUIClick", bGraphicsSettings, options_opengraphicsmenu, false)
	...
	if getResourceFromName("admin-system") and exports['admin-system']:canPlayerAccessStaffManager(localPlayer) then
		bStaffManager = guiCreateButton(margin, wHeight, 230, 30, "Control STAFF", false, wOptions)
	...
	if getResourceFromName("factions") and exports.factions:canAccessFactionManager( localPlayer ) then
		bFactionManager = guiCreateButton(margin, wHeight, 230, 30, "Control FACCIONES", false, wOptions)
	...
	if getResourceFromName("interior_system") and getResourceFromName("interior-manager") and exports.integration:isPlayerAdmin( localPlayer ) then
		bInteriorManager = guiCreateButton(margin, wHeight, 230, 30, "Control INTERIORES", false, wOptions)
	...
	if getResourceFromName("vehicle") and getResourceFromName("vehicle_manager") and exports.vehicle_manager:canAccessVehicleManager( localPlayer ) then
		bVehicleManager = guiCreateButton(margin, wHeight, 230, 30, "Control VEHICULOS", false, wOptions)
	...
	if getResourceFromName('vehicle') and getResourceFromName("vehicle_manager") then
		...
		if exports.integration:isPlayerVCTMember(thePlayer) or exports.integration:isPlayerSupporter(thePlayer) or exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerScripter(thePlayer) or exports.integration:isPlayerVehicleConsultant(thePlayer) then
			bVehicleLib = guiCreateButton(margin, wHeight, 230, 30, "Librería de Vehiculos", false, wOptions)
	...
	if getResourceFromName("apps") and exports.integration:isPlayerTrialAdmin(localPlayer) or exports.integration:isPlayerSupporter(localPlayer) then
		bApplicationManager = guiCreateButton(margin, wHeight, 230, 30, "Control de APLICACIONES", false, wOptions)
	...
	if getResourceFromName("carradio") then
		bRadioManager = guiCreateButton(margin, wHeight, 230, 30, "Control de ESTACIONES DE RADIO", false, wOptions)
	...
	if getResourceFromName("announcement") and exports.announcement:canPlayerAccessMotdManager(localPlayer) then
		bmotd = guiCreateButton(margin, wHeight, 230, 30, "Mensaje del Día", false, wOptions)
	...
	if getResourceFromName("map_manager") then
		bHelp = guiCreateButton(margin, wHeight, 230, 30, "Control de MAPAS", false, wOptions)
	...
	bLogout = guiCreateButton(margin, wHeight, 230, 30, "Cerrar sesion", false, wOptions)
...
function options_opengraphicsmenu()
	...
	cMotionBlur = guiCreateCheckBox(10, 25, 180, 17, "Activar BLUR", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cMotionBlur, options_updateGameConfig)
	-----------
	cSkyClouds = guiCreateCheckBox(10, 45, 180, 17, "Activar NUBES Sky Cloud", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cSkyClouds, options_updateGameConfig)
	------------
	cStreamingAudio = guiCreateCheckBox(10, 65, 180, 17, "Activar transmision de audio", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cStreamingAudio, options_updateGameConfig)

	bOverlayDescription = guiCreateButton ( 10, 85, 180, 17*2, "Overlay Descripción de Ajustes", false, wGraphicsMenu )
	...
	cLogsEnabled = guiCreateCheckBox(10, 160, 180, 17, "Registro del chat", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cLogsEnabled, options_updateGameConfig)

	cBubblesEnabled = guiCreateCheckBox(10, 180, 180, 17, "Activar Burbujas de Chat", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cBubblesEnabled, options_updateGameConfig)

	cIconsEnabled = guiCreateCheckBox(10, 200, 180, 17, "Activar Iconos (escribiendo)", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cIconsEnabled, options_updateGameConfig)

	cEnableNametags = guiCreateCheckBox(10, 220, 180, 17, "Activar etiquetas de nombre", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cEnableNametags, options_updateGameConfig)

	cEnableRShaders = guiCreateCheckBox(10, 240, 180, 17, "Activar shader de radar", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cEnableRShaders, options_updateGameConfig)

	cEnableWShaders = guiCreateCheckBox(10, 260, 180, 17, "Activar shader de agua", false, false, wGraphicsMenu)
	addEventHandler("onClientGUIClick", cEnableWShaders, options_updateGameConfig)

	cEnableVShaders = guiCreateCheckBox(10, 280, 180, 17, "Activar shader de vehiculos", false, false, wGraphicsMenu)
...
function overlayDescSettings(button, state)
	if source == bOverlayDescription then
		if wOverlayDescSettings then
			...
			wOverlayDescSettings = guiCreateWindow(left, top, windowWidth, windowHeight, "Overlay - Opciones de Descripciones", false)
			...
			cEnableDescription = guiCreateCheckBox(10, 25+y, lane1w, 17, "Activar todas las descripciones", true, false, wOverlayDescSettings)
			...
			cEnableDescriptionVeh = guiCreateCheckBox(10, 25+y, lane1w, 17, "Activar descripciones de vehiculos", true, false, wOverlayDescSettings)
			...
			cEnableDescriptionVehPin = guiCreateCheckBox(lane2x, 25+y, lane2w, 17, "ID", false, false, wOverlayDescSettings)
			...
			lFontVeh = guiCreateLabel ( 10, 25+y+3, 40, 20,  "Fuente:", false, wOverlayDescSettings )
			...
			bgVeh = guiCreateCheckBox ( 10, 25+y+3, 150, 17,  "Activar fondo", true, false, wOverlayDescSettings )
			...
			cEnableOverlayDescriptionPro = guiCreateCheckBox(10, 25+y, lane1w, 17, "Activar Descripcion de Propiedades", true, false, wOverlayDescSettings)
			...
			cEnableOverlayDescriptionProPin = guiCreateCheckBox(lane2x, 25+y, lane2w, 17, "ID", false, false, wOverlayDescSettings)
...
function options_logOut( message )
	...
	triggerServerEvent("accounts:characters:change", localPlayer, "Cambiar de PJ")
...
function options_logOutToLoginPanel( message )
	triggerServerEvent("accounts:characters:logout", localPlayer, "Cambiar de PJ")
	open_log_reg_pannel()
end
