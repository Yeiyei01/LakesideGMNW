...
local function testSort(a, b, a_more_recent)
	if byTime({lastOnline = a}, {lastOnline = b}) ~= a_more_recent then
		outputDebugString( "La clasificación falló en " .. tostring( a ) .. " " .. tostring( b ) )
	end
end
testSort( 20, 10, true )
testSort( 20, "5 dias", true )
testSort( "5 dias", 10, false )
testSort( "14 dias", "5 dias", false )
...
addEventHandler( "social:friends", localPlayer,
	function( friendsList, ownMessage, currentTimestamp )
		...
		outputDebugString( "Recibiendo " .. #friendsList .. " amigos.")
	end
)
...
function drawFriends( )
	...
	local line = scroll_y
	for _, data in ipairs( { { title = "Conectado", items = online }, { title = "Desconectado", items = offline } }) do
		-- render title
		...
		for key, value in ipairs( data.items ) do
			...
			if inBox and justClicked then
				...
				elseif not value.editable then
					...
					friends_message_window = guiCreateWindow( screen_x / 2 - 200, screen_y / 2 - 30, 400, 60, "Actualizar Mensaje", false )
					local e = guiCreateEdit( 5, 25, 290, 30, value.message, false, friends_message_window )
					local c = guiCreateButton( 300, 25, 45, 30, "Salvar", false, friends_message_window )
					local d = guiCreateButton( 350, 25, 45, 30, "Cancelar", false, friends_message_window )
			...
			if value.player then
				text = "jugando con " .. getPlayerName( value.player ):gsub( "_", " " ) .. " (" .. getElementData( value.player, "playerid" ) .. ")."
			else
				text = ( formatTimeInterval( value.lastOnline ) .. " fue visto la ultima vez." ):gsub( "  ", " " )
			end
			...
			if value.message then
				...
				dxDrawText( "¡Hola!", x + 10, y, w - 10, h, white, 1, small, "left", "top", true, true, true )
			end
