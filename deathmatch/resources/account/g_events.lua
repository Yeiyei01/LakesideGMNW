	...
	for i = 1, #theText do
		...
		if char == ' ' then -- it's a space
			...
			if #current < 2 then -- check if name's part is at least 2 chars
				valid = false
				return false, "Tu nombre no puede ser tan corto"
			end
			current = ''
		elseif lastChar == ' ' then -- this char follows a space, we need a capital letter
			if char < 'A' or char > 'Z' then
				valid = false
				return false, "No estás usando mayúsculas al principio."
			end
		...
		elseif ( char >= 'a' and char <= 'z' ) or ( char >= 'A' and char <= 'Z' ) or (char == "'") then -- can have letters anywhere in the name
			...
			valid = false
			return false, "Tu nombre tiene carácteres invalidos \nen el texto. Solo puedes usar letras de\n A-Z, a-z y espacios"
		end
		lastChar = char
	end
	
	if valid and foundSpace and #theText <= 22 and #theText >= 2 then
		return true, "Nombre disponible" -- passed
	else
		return false, "El nombre es muy largo o muy corto \n(min 2, max 22)" -- failed for the checks
	end
end