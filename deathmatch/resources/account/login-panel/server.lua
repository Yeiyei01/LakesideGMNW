...
function validateCredentials(username,password,checksave)
	if not (username == "") then
		if not (password == "") then
			...
			triggerClientEvent(client,"set_warning_text",client,"Login","Coloca tu contraseña")
		end
	else
		triggerClientEvent(client,"set_warning_text",client,"Login","Coloca tu usuario")
	end
	return false
end
...
function getAccountDetails(id)
	...
	elseif #result_mta == 0 then
		if dbExec(exports.mysql:getConn("mta", "INSERT INTO account_details SET `account_id`=?", id)) then
			...
			outputDebugString("La creación de datos mágicos ha fallado.")
		end
	else
		data = result_mta[1]
	end
			...
			if result and client then
				if #result > 0 then
					...
					if exports.bans:checkAccountBan(accountData["id"]) then
						triggerClientEvent(client,"set_warning_text",client,"Login","La cuenta está baneada.")
						exports.logs:dbLog("ac"..tostring(accountData["id"]), 27, "ac"..tostring(accountData["id"]), "Conexión rechazada de " .. getPlayerIP(client) .. " - ".. getPlayerSerial(client) .. " ya que la cuenta está baneada.")
						return false
					end

					--Now check if passwords are matched or the account is activated, this is to prevent user with fake emails.
					triggerClientEvent(client,"set_authen_text",client,"Login","¡Contraseña aceptada! Autentificando..")
					-- Check if old method
					if not string.find(accountData["password"], "$", 1, true) then -- Plain search, not regex
						...
						if not dbExec(exports.mysql:getConn("core"), "UPDATE `accounts` SET `password`=?, `salt`=NULL WHERE id=?", new_pass, accountData["id"]) then
							triggerClientEvent(client,"set_warning_text",client,"Login","¡Fallo en la conversión de la contraseña para la cuenta '".. username .."'!")
							return false
						end
					else -- Else if new
						...
						if not verified then
							triggerClientEvent(client,"set_warning_text",client,"Login","La contraseña es incorrecta para la cuenta '".. username .."'!")
							return false
						end
					end
...
--						triggerClientEvent(client,"set_warning_text",client,"Login","La cuenta '".. username .."' no está activa.")
--						return false
--					end
]]
					--Validation is done, fetching some more details
					triggerClientEvent(client,"set_authen_text",client,"Login","¡Cuenta autentificada!")
					...
					for _, thePlayer in ipairs(exports.pool:getPoolElementsByType("player")) do
						...
						if (playerAccountID) then
							if (playerAccountID == tonumber(accountData["id"])) and (thePlayer ~= client) then
								kickPlayer(thePlayer, thePlayer, "Otra persona ha entrado en su cuenta.")
								triggerClientEvent(client,"set_authen_text",client,"Login","La cuenta está actualmente en línea. Desconectando al otro usuario..")
								break
							end
						end
					end
					...
					local result_mta = dbPoll(qb, -1)
					if not result_mta then
						triggerClientEvent(client,"set_authen_text",client,"Login","¡La conexión de Magic Data ha fallado!")
						return
					elseif #result_mta == 0 then
					...
					if not exports.serialwhitelist:check(client) then
						triggerClientEvent(client,"set_warning_text",client,"Login","No tienes permiso para conectarte al servidor desde ese PC, comprueba el UCP.")
					...
					triggerClientEvent(client,"set_warning_text",client,"Login","La cuenta '".. username .."' no existe.")
				end
			else
				triggerClientEvent(client,"set_warning_text",client,"Login","No se ha podido conectar con el servidor. Error en la base de datos.")
				dbFree(qh)
			end
	...
	dbQuery(function(qh, username, password, confirmPassword, email, client)
		...
		if result then
			if #result == 0 then
				...
				if not Q2 then
					triggerClientEvent(client,"set_warning_text",client,"Register","Se ha producido el código de error 0003.")
					return false
				end

				local usernameExisted = mysql:fetch_assoc(Q2)
				if (mysql:num_rows(Q2) > 0) and usernameExisted["id"] ~= "1" then
					triggerClientEvent(client,"set_warning_text",client,"Register","No se permiten las cuentas múltiples (Existe: "..tostring(usernameExisted["username"])..")")
					return false
				end
				...
				if userid then
					...
					triggerClientEvent(client,"set_warning_text",client,"Register","No se ha podido crear una nueva cuenta.")
					return false
				end
			else
				triggerClientEvent(client,"set_warning_text",client,"Register","El nombre o el correo electrónico existen.")
			end
		else
