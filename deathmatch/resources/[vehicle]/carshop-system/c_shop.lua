...
function carshop_showInfo(carPrice, taxPrice)
	...
	if isOverlayDisabled then
		outputChatBox("")
		outputChatBox("Consecionaria de vehículos")
		...
		if getVehicleType(source) ~= 'BMX' then
			outputChatBox("   - Kilómetros: "..exports.global:formatMoney(getElementData(source, 'odometer') or 0) .. " miles"  )
		end
		outputChatBox("   - Precio: $"..exports.global:formatMoney(carPrice)  )
		outputChatBox("   - Impuesto: $"..exports.global:formatMoney(taxPrice)  )
		outputChatBox("   (( Modelo de MTA: "..getVehicleNameFromModel( getElementModel( source ) ).."))"  )
		outputChatBox("Presiona 'F' o 'Enter' para comprarlo")
	else
		local content = {}
		...
		if getVehicleType(source) ~= 'BMX' then
			table.insert(content, {"   - Kilómetros: "..exports.global:formatMoney(getElementData(source, 'odometer') or 0) .. " miles"})
		end
		table.insert(content, {"   - Precio: $"..exports.global:formatMoney(carPrice)  } )
		table.insert(content, {"   - Impuesto: $"..exports.global:formatMoney(taxPrice) } )
		table.insert(content, {"   (( Modelo de MTA: "..getVehicleNameFromModel( getElementModel( source ) ).."))" } )
		table.insert(content, {"Presiona 'F' o 'Enter' para comprarlo" } )
...
function carshop_buyCar(carPrice, cashEnabled, bankEnabled)
	...
	gui["lblText1"] = guiCreateLabel(20, 25, windowWidth-40, 16, "Está a punto de comprar el siguiente vehículo:", false, gui["_root"])
	...
	gui["lblText2"] = guiCreateLabel(20, 45+15*2, windowWidth-40, 70, "Al hacer clic en el botón de pago, usted acepta que no es posible un reembolso. Gracias por elegirnos.", false, gui["_root"])
	guiLabelSetHorizontalAlign(gui["lblText2"], "left", true)
	guiLabelSetVerticalAlign(gui["lblText2"], "center", true)

	gui["btnCash"] = guiCreateButton(10, 140, 105, 41, "Pagar con el dinero en mano", false, gui["_root"])
	...
	if exports.global:hasItem(localPlayer, 263) and carPrice <= 35000 then
		guiSetText(gui["btnCash"], "Reclamar ticket")
		guiSetEnabled(gui["btnCash"], true)
	end

	gui["btnBank"] = guiCreateButton(120, 140, 105, 41, "Pagar con el banco", false, gui["_root"])
...
function carshop_buyCar_click()
	if exports.global:hasSpaceForItem(getLocalPlayer(), 3, 1) then
		...
		if (source == gui["btnBank"]) then
			sourcestr = "bank"
		elseif guiGetText(gui["btnCash"]) == "Reclamar ticket" then
			sourcestr = "token"
		end
		triggerServerEvent("carshop:buyCar", theVehicle, sourcestr)
	else
		outputChatBox("No tienes espacio en el inventario para una llave", 0, 255, 0)
	end
