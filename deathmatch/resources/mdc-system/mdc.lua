...
function main ( )
	...
	if ( warrantResult ) then
		...
		while true do
			...
			local account = nil
			local accountResult = exports.mysql:query( "SELECT `user` FROM mdc_users WHERE `id` = '".. exports.mysql:escape_string( row.wanted_by ) .."'" )
	...
	local callsResult = exports.mysql:query( "SELECT * FROM `mdc_calls` WHERE `timestamp` > CURDATE() - INTERVAL 2 DAY")
	if ( callsResult ) then
		local count = 1
		while true do
			...
			--Fetch mdc account name from ID
			local account = nil
			local accountResult = exports.mysql:query( "SELECT `charactername` FROM characters WHERE `id` = '".. exports.mysql:escape_string( row.caller ) .."'" )
...
function search( query, queryType )
	...
	elseif queryType == 0 then --Person
		...
		if exports.mysql:num_rows( result ) > 0 then
			...
			local vehicles = { }
			local result4 = exports.mysql:query( "SELECT `id`, `model`, `plate` FROM `vehicles` WHERE `owner` = '".. character.id .."'" )
			if ( result4 ) then
				local count = 1
				while true do
	...
	elseif queryType == 1 then --Vehicle
		...
		local result = exports.mysql:query( "SELECT * FROM vehicles WHERE `plate` = '".. exports.mysql:escape_string( query ) .."'" ) --Fetch the information from the database 
		if exports.mysql:num_rows( result ) > 0 then
	...
	elseif queryType == 3 then --Vehicle by ID
		...
		local result = exports.mysql:query( "SELECT * FROM vehicles WHERE `id` = '".. exports.mysql:escape_string( query ) .."'" ) --Fetch the information from the database 
		if exports.mysql:num_rows( result ) > 0 then
			vehicle = exports.mysql:fetch_assoc( result )
...
function add_crime( charid, charactername, crime, punishment )
	...
	local query = exports.mysql:query_insert_free( "INSERT INTO `mdc_crimes` ( `crime`, `punishment`, `character`, `officer`, `timestamp` ) VALUES ( '"..exports.mysql:escape_string( crime ).."','"..exports.mysql:escape_string( punishment ).."','"..charid.."','"..officer.."', '"..timestamp.."' )" )
	if query then
...
function add_apb( description, person )
	...
	local time = getRealTime( )
	local timestamp = time.timestamp
	local query = exports.mysql:query_insert_free( "INSERT INTO `mdc_apb` ( `person_involved`, `description`, `doneby`, `time` ) VALUES ( '"..exports.mysql:escape_string( person ).."','"..exports.mysql:escape_string( description ).."','"..officer.."', '"..timestamp.."' )" )
	if query then
		main(  )
	end
end
...
function system_admin( )
	local query = exports.mysql:query( "SELECT * FROM mdc_users ORDER BY id ASC" )
	if ( query ) then
		local rows = { }
		local count = 1
...
function create_account( user, pass, level )
	...
	local query = "INSERT INTO `mdc_users` ( `user`, `pass`, `level` ) VALUES ( '"..exports.mysql:escape_string( user ).."','"..exports.mysql:escape_string( pass ).."','"..exports.mysql:escape_string( level + 1 ).."' )"
