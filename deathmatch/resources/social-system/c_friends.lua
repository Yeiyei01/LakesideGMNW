...
testSort( 20, 10, true )
testSort( 20, "5 Días", true )
testSort( "5 Días", 10, false )
testSort( "14 Días", "5 Días", false )
		...
		for key, value in ipairs( data.items ) do
			...
			if inBox and justClicked then
				...
				elseif not value.editable then
					...
					friends_message_window = guiCreateWindow( screen_x / 2 - 200, screen_y / 2 - 30, 400, 60, "Actualiza los mensajes de tus amigos", false )
					local e = guiCreateEdit( 5, 25, 290, 30, value.message, false, friends_message_window )
					local c = guiCreateButton( 300, 25, 45, 30, "Guardar", false, friends_message_window )
			...
			if value.player then
				text = "Jugando Como " .. getPlayerName( value.player ):gsub( "_", " " ) .. " (" .. getElementData( value.player, "playerid" ) .. ")."
			else
				text = ( formatTimeInterval( value.lastOnline ) .. " Atrás." ):gsub( "  ", " " )
			end
