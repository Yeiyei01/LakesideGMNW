...
function sendFriends( player, accountID )
	...
	for _, otherAccount in ipairs( friends[ accountID ] ) do
		...
		if friend.name then
			...
			outputDebugString( "social:sendFriends: Account " .. otherAccount .. " no existe?" )
...
addEventHandler( "social:remove", root,
	function( otherAccount )
		...
		if accountID and friends[ accountID ] and friends[ otherAccount ] then
			...
			if not found then
				outputChatBox( "no es tu amigo.", client, 255, 0, 0 )
			else
				...
				outputChatBox( "You removed " .. friends[ otherAccount ].name .. " de tu lista de amigos.", client, 0, 255, 0 )
