		...
		if vehicle and isElement(vehicle) then
			...
			if(isElement(patient)) then --and getElementAttachedTo(stretcherArray[playerElement]) == patient
				...
				if not warpResult then
					outputChatBox("La/El "..tostring(exports.global:getVehicleName(vehicle)).." no tiene mas espacios para pacientes.", playerElement, 255, 0, 0)
				else
...
function createStretcher( playerElement, vehicle )
	...
	if hasPlayerStretcherSpawned( playerElement ) then
		if vehicle then
			exports.global:sendLocalMeAction(playerElement, "coloca la camilla dentro de "..tostring(exports.global:getVehicleName(vehicle))..".")
		else
			exports.global:sendLocalMeAction(playerElement, "coloca la camilla dentro.")
		end
		destroyStretcher( playerElement, vehicle )
	else
		if vehicle then
			exports.global:sendLocalMeAction(playerElement, "saca una camilla de dentro de "..tostring(exports.global:getVehicleName(vehicle))..".")
		else
