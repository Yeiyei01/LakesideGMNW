...
function playerDeath(totalAmmo, killer, killerWeapon)
	if getElementData(source, "dbid") then
		if getElementData(source, "adminjailed") then
			...
			spawnPlayer(source, 246.64765930176, -225.82168579102, 1.5678124427795, 90.394104003906) --, team)
			...
			exports.logs:dbLog(source, 34, source, "murió en jail")
		elseif getElementData(source, "jailed") then
			...
			exports.logs:dbLog(source, 34, source, "murió en prision")
		else
			local affected = { }
			...
			if (killer) then
				if getElementType(killer) == "player" then
					if (killerWeapon) then
						killstr = ' ha muerto por '..getPlayerName(killer):gsub("_", " ").. ' ('..getWeaponNameFromID ( killerWeapon )..')'
					else
						killstr = ' murio'
					end
					table.insert(affected, killer)
				else
				killstr = ' murió por un bug'
			...
			outputChatBox("Si fue asesinado por DM o algo similar, informe para que un administrador lo reviva.", source)
			outputChatBox("Si acepta su muerte, puede perder algunos de sus artículos, a menos que lo revivan.", source)
...
function acceptDeath(victimDropItem)
	if isPedDead(client) then
		...
		outputChatBox("Reapareciendo...", client)
		...
		outputChatBox("No estás muerto", client, 255, 0, 0)
	end
end
...
function readLog(thePlayer)
	if exports.integration:isPlayerTrialAdmin(thePlayer) then
		local logMeBuffer = getElementData(getRootElement(), "killog") or { }
		outputChatBox("Muertes recientes:", thePlayer, 205, 201, 165)
		for a, b in ipairs(logMeBuffer) do
			outputChatBox("- "..b, thePlayer, 205, 201, 165, true)
		end
		outputChatBox("  FIN", thePlayer, 205, 201, 165)
	end
end
addCommandHandler("vermuertes", readLog)
...
function respawnPlayer(thePlayer, victimDropItem)
	if (isElement(thePlayer)) then
		...
		exports.global:takeMoney( getTeamFromName("Riverside Municipal Government"), math.ceil((1-tax)*cost) )
		...
		outputChatBox("Has sido tratado en el hospital.", thePlayer, 255, 255, 0)
		...
		if count > 0 then
			outputChatBox("Empleado de RCES: Te hemos quitado las drogas que llevabas.", thePlayer, 255, 194, 14)
		end
				...
				if itemCheck[1] == 115 then -- Weapon
					...
					if (((weapon >= 16 and weapon <= 40 and (gunlicense == 0 and gunlicense2 == 0) or not exports.weapon:isWeaponCCWP(thePlayer, itemCheck[1], itemCheck[2])) or (weapon == 29 or weapon == 30 or weapon == 32 or weapon ==31 or weapon == 34) and (gunlicense2 == 0)) and (not exports.factions:isInFactionType(thePlayer, 2))) or (weapon >= 35 and weapon <= 38) then
						if exports['item-system']:takeItemFromSlot(thePlayer, itemSlot-cor) then
							cor = cor + 1
							exports.logs:dbLog(thePlayer, 34, thePlayer, "perdio el arma (" ..  itemCheck[2] .. ")")
							for k = 1, 12 do
				...
				elseif itemCheck[1] == 116 then
					...
					if cart_id and gunlicense == 0 and gunlicense2 == 0 then
						if exports['item-system']:takeItemFromSlot(thePlayer, itemSlot-cor) then
							...
							exports.logs:dbLog(thePlayer, 34, thePlayer, "perdio un pack de "..pack.cartridge.." municiones. ("..ammountOfAmmo.." balas)")

							if (removedWeapons == nil) then
								removedWeapons = ammountOfAmmo .. " " .. pack.cartridge
								formatedWeapons = ammountOfAmmo .. " " .. pack.cartridge
							else
								removedWeapons = removedWeapons .. ", " .. ammountOfAmmo .. " balas de " .. pack.cartridge
								formatedWeapons = formatedWeapons .. "\n" .. ammountOfAmmo .. " balas de " .. pack.cartridge
							end
						end
					end
				end
		...
		if (removedWeapons~=nil) then
			if gunlicense == 0 and factiontype ~= 2 then
				outputChatBox("Empleado de RCES: Te hemos quitado el arma que llevabas ya que no tenias licencia. (" .. removedWeapons .. ").", thePlayer, 255, 194, 14)
			else
				outputChatBox("Empleado de RCES: Te hemos quitado el arma debido a que no tienes permitido llevarla. (" .. removedWeapons .. ").", thePlayer, 255, 194, 14)
			end
		end
		...
		if removedWeapons ~= nil then
			logMe(death)
			exports.global:sendMessageToAdmins("/vermuertes para ver las armas perdidas.")
		...
		spawnPlayer(thePlayer, 246.64765930176, -225.82168579102, 1.5678124427795, 90.394104003906)--, theTeam)
...
function recoveryPlayer(thePlayer, commandName, targetPlayer, duration)
	if not (targetPlayer) or not tonumber(duration) or (tonumber(duration) or 0) <= 0 then
		outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del jugador / ID] [Horas]", thePlayer, 255, 194, 14)
	else
		...
		if targetPlayer then
			...
			if (logged==1) then
				if exports.factions:isInFactionType(thePlayer, 4) or (exports.integration:isPlayerTrialAdmin(thePlayer) == true) then
					--if (targetPlayer == thePlayer) then
						...
						--if (dimesion==9) then
							...
							if totaltime < 12 then
								...
								outputChatBox("Has puesto en sala de recuperatorio " .. targetPlayerName .. " en sala de recuperatorio por " .. duration .. " hora(s) y le costo $".. 100*totaltime ..".", thePlayer, 255, 0, 0)
								exports.global:sendMessageToAdmins("AdmWrn: " .. targetPlayerName .. " ha sido puesto en sala de recuperatorio tiempo " .. duration .. " hora(s) por " .. getPlayerName(thePlayer):gsub("_"," ") .. ".")
								outputChatBox("Has sido puesto por " .. getPlayerName(thePlayer) .. " en sala de recuperatorio tiempo " .. duration .. " hora(s) y costó $".. 100*totaltime ..".", targetPlayer, 255, 0, 0)
								exports.logs:dbLog(thePlayer, 4, targetPlayer, "RECOVERY " .. duration)
							else
								outputChatBox("No puedes poner a alguien por tanto tiempo.", thePlayer, 255, 0, 0)
							end
						--[[else
					...
					outputChatBox("No tienes las habilidades básicas, llama a emergencias RCES.", thePlayer, 255, 0, 0)
				end
			else
...
addCommandHandler("recuperatorio", recoveryPlayer)
		...
		if (logged==1) then -- Check all logged in players.
			...
			if (inRecovery == 1) then
				...
				if (recoveryEndsAt <= currentTime) then -- Is the time up? If yes:
					...
					outputChatBox("Ya no estás en recuperatorio", value, 0, 255, 0) -- Let them know about it!
				else
					...
					if (player==value) and (eventname=="accounts:characters:spawn") then
						outputChatBox("Aun estás en sala de recuperatorio.", value, 255,0,0)
					end
				end
			end
		end
	...
	if (logged==1) then
        ...
        if (inRecovery == 1) then
			...
			if (recoveryEndsAt <= currentTime) then -- Is the time up? If yes:
				...
				mysql:query_free("UPDATE characters SET recovery='0', recoverytime=NULL WHERE id = " .. dbid) -- Allow them to move, and revert back to recovery type set to 0.
				outputChatBox("Ya no estás en sala de recuperatorios", thePlayer, 0, 255, 0) -- Let them know about it!
			else
				...
				local minutesLeft = math.floor(recoveryEndsAt / 60)
           		outputChatBox("Aun te quedan " .. hoursLeft .. " hora(s) y " .. minutesLeft .. " minuto(s) de recuperatorio restante.", thePlayer)
           	end
        end
    end
end
addCommandHandler("recuperatoriotiempo", checktime)
	...
	if (logged==1) then
		...
		if (inRecovery == 1) then
			...
			if (recoveryEndsAt <= currentTime) then -- Is the time up? If yes:
				...
				mysql:query_free("UPDATE characters SET recovery='0', recoverytime=NULL WHERE id = " .. dbid) -- Allow them to move, and revert back to recovery type set to 0.
				outputChatBox("Ya no estás en recuperatorios", player, 0, 255, 0) -- Let them know about it!
			else
				...
				outputChatBox("Aun estás en recuperatorio.", player, 255,0,0)
			end
		end
	end
end
...
function prescribe(thePlayer, commandName, ...)
	if exports.factions:isInFactionType(thePlayer, 4) then
		if not (...) then
			outputChatBox("SYNTAX /" .. commandName .. " [costo receta]", thePlayer, 255, 184, 22)
		else
			local itemValue = table.concat({...}, " ")
			itemValue = tonumber(itemValue) or itemValue
			if not(itemValue=="") then
				exports.global:giveItem( thePlayer, 132, itemValue )
				outputChatBox("La receta '" .. itemValue .. "' ha sido procesada.", thePlayer, 0, 255, 0)
		...
		if not (targetPlayer) then
			...
			if targetPlayer then
				if isPedDead  ( targetPlayer ) then
					...
					outputChatBox("Has sido revivido por "..tostring(exports.global:getPlayerAdminTitle(thePlayer)).." "..tostring(getPlayerName(thePlayer):gsub("_"," "))..".", targetPlayer, 0, 255, 0)
					outputChatBox("Has revivido a "..tostring(getPlayerName(targetPlayer):gsub("_"," "))..".", thePlayer, 0, 255, 0)
					...
					outputChatBox(tostring(getPlayerName(targetPlayer):gsub("_"," ")).." no está muerto.", thePlayer, 255, 0, 0)
				end
			end
		end
	end
...
function medicalBill(thePlayer, commandName, targetPlayer, amount)
	...
	if medicalBillFactions[dutyFaction] then
		if not (targetPlayer) or not tonumber(amount) or (tonumber(amount) or 0) <= 0 then
			outputChatBox("SYNTAX: /" .. commandName .. " [Nombre del jugador / ID] [Cantidad]", thePlayer, 255, 194, 14)
		else
			...
			if targetPlayer then
				...
				if (logged==1) then
					...
					outputChatBox("El costo de la atencion medica ha sido enviado a "..tostring(targetPlayerName)..".", thePlayer, 245, 217, 7)
				else
					outputChatBox("El jugador no está logueado.", thePlayer, 255,0,0)
				end
			else
...
addCommandHandler("atencionmed", medicalBill)
	...
	if action == 1 then --pay by cash
		if exports.global:takeMoney(client, toPay) then
			...
			if exports.global:giveMoney(theTeam, amount) then
				...
				outputChatBox("Te han pagado por la atención médica ("..tostring(exports.global:formatMoney(amount)).."$).", client, 0, 250, 0)
				outputChatBox(name.." pagó su atencion medica ("..tostring(exports.global:formatMoney(amount)).."$).", doctor, 0, 250, 0)
			end
		else
			outputChatBox("No tienes dinero en mano para pagar, se tomará del banco.", client, 255, 0, 0)
			payMedicalBill(client, doctor, faction, amount, insurance, 2)

			local name = exports.global:getPlayerName(client):gsub("_", " ")
			outputChatBox(name.." no puede pagar la atencion medica.", doctor, 255, 0, 0)
		end
	elseif action == 2 then --pay by bank
		if exports.bank:hasBankMoney(client, toPay) then
			if exports.bank:takeBankMoney(client, toPay) then
				...
				outputChatBox("Hubo un error en la transaccción. No se ha pagado la atencion.", client, 255, 0, 0)
				local name = exports.global:getPlayerName(client):gsub("_", " ")
				outputChatBox(name.." no puede pagar debido a un error de transaccion.", doctor, 255, 0, 0)
			end
		else
			outputChatBox("No puedes pagar esta atencion medica.", client, 255, 0, 0)
			local name = exports.global:getPlayerName(client):gsub("_", " ")
			outputChatBox(name.." no puede pagar la atencion medica.", doctor, 255, 0, 0)
		end
	elseif action == 3 then --refuse to pay
		local name = exports.global:getPlayerName(client):gsub("_", " ")
		outputChatBox(name.." rechazó pagar la atencion medica.", doctor, 255, 0, 0)
	end
end
