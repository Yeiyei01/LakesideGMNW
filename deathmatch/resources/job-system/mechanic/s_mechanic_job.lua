...
function serviceVehicle(veh)
	if (veh) then
		...
		if not (exports.global:hasMoney(source, mechcost) or exports.integration:isPlayerTrialAdmin(source, true)) then
			outputChatBox("No puedes pagar las piezas para este vehiculo.", source, 255, 0, 0)
		else
			...
			exports.global:sendLocalMeAction(source, "arregla el vehiculo.")
			exports.logs:dbLog(source, 31, {  veh }, "REPAIR QUICK-SERVICE")
		end
	else
		outputChatBox("Tienes que estar en un vehiculo para repararlo.", source, 255, 0, 0)
	end
end
...
function changePaintjob( veh, paintjob )
	if (veh) then
		...
		if not exports.global:takeMoney(source, mechcost) then
			outputChatBox("No puedes permitirte pintar este vehiculo.", source, 255, 0, 0)
		else
			...
			if setVehiclePaintjob( veh, paintjob ) then
				...
				exports.global:sendLocalMeAction(source, "pinta el vehiculo.")
				exports.logs:dbLog(source, 6, {  veh }, "MODDING PAINTJOB ".. paintjob)
				exports.vehicle:saveVehicleMods(veh)
			else
				outputChatBox("Este auto ya tiene este vinilo.", source, 255, 0, 0)
			end
		end
	end
end
...
function editVehicleHeadlights( veh, color1, color2, color3 )
	if (veh) then
		...
		if not (exports.global:hasMoney(source, mechcost) or exports.integration:isPlayerTrialAdmin(source, true)) then
			outputChatBox("No puedes permitirte modificar el vehiculo.", source, 255, 0, 0)
		else
			...
			if setVehicleHeadLightColor ( veh, color1, color2, color3) then
				...
				exports.global:sendLocalMeAction(source, "remplaza las luces del vehículo.")
				...
				outputChatBox("Este auto ya tiene estas luces.", source, 255, 0, 0)
			end
		end
	end
end
		...
		if not item and not exports.global:hasMoney( source, mechcost ) then
			...
			for i = 0, 16 do
				if upgrade == getVehicleUpgradeOnSlot( veh, i ) then
					outputChatBox("Este vehiculo ya tiene esta mejora.", source, 255, 0, 0)
					return
				end
			end
