...
local function forceUpdate( )
	...
	if getElementType(element)=="interior" then
		...
		if not hasKey(getElementData(element, "dbid"), element) and interiorType~=2 then
			...
			guiGridListSetItemText(gElementItems, row, VIColName, "Necesitas la llave de esta casa", false, false)
			...
			if interiorType==2 then
				if (not exports.factions:isInFactionType(getLocalPlayer(), 2) and not exports.factions:isInFactionType(getLocalPlayer(), 3) and getElementData(getLocalPlayer(), "duty_admin") ~= 1) then
					local row = guiGridListAddRow(gElementItems)
					guiGridListSetItemText(gElementItems, row, VIColName, "Miembros del gobierno solamente", false, false)
					triggerEvent("item:updateclient", localPlayer)
					return
				end
			end
	...
	if getElementType( element ) == "vehicle" then
			...
			for key, value in pairs( data[ element ] ) do
				for k, v in ipairs( mods ) do
					if key == v then
						...
						guiGridListSetItemText(gElementItems, row, VIColName, "Mejora " .. getItemDescription( 114, v ), false, false)
		...
		if exports.integration:isPlayerTrialAdmin(getLocalPlayer()) then
			for key, value in ipairs(mods) do
				...
				guiGridListSetItemText(gElementItems, row, VIColName, "Mejora " .. getItemDescription( 114, value ), false, false)
			...
			if item then
				...
				if (hasKey(getElementData(element, "dbid"), element) and getElementType(element)=="interior") or getElementType(element)~="interior" then
					if itemID < 0 and itemID ~= -100 then -- weapon
						...
						if totalfree == 0 then
							outputChatBox( "Tienes todas las armas que puedes llevar.", 255, 0, 0 )
						...
						if slot and slot ~= 0 and getPedTotalAmmo( getLocalPlayer(), slot ) > 0 then
							...
							outputChatBox( "Usted no lleva esa arma, por favor tire su " .. weaponName .. " primero.", 255, 0, 0 )
						elseif free == 0 then
							outputChatBox( "No puedes llevar más de esa arma.", 255, 0, 0 )
						else
							--guiSetVisible( wWait, true )
							--guiSetEnabled( wInventory, false )
					...
					outputChatBox("Debes tener las llaves de su casa para coger objetos del Buzón.", getLocalPlayer(), 255, 0, 0)
				end
			end
		end
	end
...
local function openElementInventory( ax, ay )
	if not getElementData(localPlayer, "exclusiveGUI") then
		...
		bCloseInventory = guiCreateButton(0.05, 0.9, 0.9, 0.075, "Cerrar inventario", true, wInventory)
		...
		outputChatBox("No puedes acceder a tu inventario en este momento.", 255, 0, 0)
	end
end
