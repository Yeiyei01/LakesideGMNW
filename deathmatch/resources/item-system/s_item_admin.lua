...
function getNearbyItems(thePlayer, commandName)
	if exports.integration:isPlayerTrialAdmin(thePlayer) or exports.global:isStaffOnDuty(thePlayer) or exports.global:isPlayerScripter(thePlayer) then
		...
		outputChatBox("Items alrededor:", thePlayer, 255, 126, 0)
		local count = 0
	...
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		if not (targetID) then
			...
			if object and getElementData(object, "itemID") ~= 169 then
				...
				outputChatBox("Item #" .. id .. " borrado.", thePlayer)
				destroyElement(object)
			else
				outputChatBox("ID de item inválido.", thePlayer, 255, 0, 0)
			end
		end
	end
end
...
function delNearbyItems(thePlayer, commandName)
	if (exports.integration:isPlayerTrialAdmin(thePlayer)) then
		...
		outputChatBox( count .. " Items borrados.", thePlayer, 255, 126, 0)
	end
end
...
function delAllItemInstances(thePlayer,commandName, itemID, itemValue )
	if exports.integration:isPlayerLeadAdmin( thePlayer ) then
		if not tonumber(itemID) or tonumber(itemID)%1 ~=0 then
			...
			outputChatBox( "Elimina todas las instancias de objetos de cualquier lugar del juego.", thePlayer, 150, 150, 50 )
		else
...
function deleteAllItemsFromAnInterior(thePlayer, commandName, intID , dayOld, restartRes)
	if exports.integration:isPlayerTrialAdmin( thePlayer ) then
		if not tonumber(intID) or tonumber(intID) < 0 or tonumber(intID)%1 ~=0 or  not tonumber(dayOld) or tonumber(dayOld) < 0 or tonumber(dayOld)%1 ~=0 then
			...
			outputChatBox( "Elimina todos los elementos de un interior especificado que sean más antiguos que un intervalo de días de antigüedad del elemento.", thePlayer, 150, 150, 50 )
			if exports.integration:isPlayerLeadAdmin( thePlayer ) then
				outputChatBox( "SYNTAX: /" .. commandName .. " [Int ID] [Day old of Items]", thePlayer, 255, 194, 14 )
				outputChatBox( "Elimina todos los elementos dentro de un mapa interior o mundial especificado que sean más antiguos que un intervalo de días del elemento.", thePlayer, 150, 150, 50 )
			end
		else
			if tonumber(intID) == 0 and not exports.integration:isPlayerLeadAdmin( thePlayer ) then
				outputChatBox("Solo Head+ Admins pueden borrar items de todo el mapa.", thePlayer, 255, 0, 0)
				return false
			end
			...
			if deleteAllItemsWithinInt(intID, dayOld) then
				...
				outputChatBox( "Todas las instancias de items que son más antiguas que "..dayOld.." days wthin interior ID #"..intID.." have been deleted.", thePlayer, 0, 255, 0)
				...
				if restartRes == 1 and getResourceFromName("item-world") then
					...
					setTimer(function () 
						outputChatBox( " [!] El servidor está limpiando los items del mundo, por favor, espere.", root)
