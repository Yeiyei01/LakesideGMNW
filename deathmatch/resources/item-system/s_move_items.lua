...
local function output(from, to, itemID, itemValue, evenIfSamePlayer, metadata)
	...
	if getElementType(from) == "player" and getElementType(to) == "player" then
		exports.global:sendLocalMeAction( from, "le da a " .. getPlayerName( to ):gsub("_", " ") .. " un " .. getItemName( itemID, itemValue, metadata ) .. "." )
	-- player to item
	elseif getElementType(from) == "player" then
		local name = getName(to)
		if itemID == 134 then
			triggerEvent('sendAme', from, "coloca $" .. exports.global:formatMoney(itemValue) .. " dentro de ".. name .."." )
		elseif itemID == 150 then --ATM card / MAXIME
			triggerEvent('sendAme',  from, "pone una tarjeta de cajero automático en el "..name.."." )
		else
			triggerEvent('sendAme',  from, "coloca " .. getItemName( itemID, itemValue, metadata ) .. " dentro de ".. name .."." )
		end
	-- item to player
	elseif getElementType(to) == "player" then
		local name = getName(from)
		if itemID == 134 then
			triggerEvent('sendAme',  to, "toma $" .. exports.global:formatMoney(itemValue) .. " de la ".. name .."." )
		elseif itemID == 150 then --ATM card / MAXIME
			triggerEvent('sendAme',  to, "toma la tarjeta de "..name.."." )
		else
			triggerEvent('sendAme',  to, "toma un " .. getItemName( itemID, itemValue, metadata ) .. " de la ".. name .."." )
		end
	end
...
local function moveToElement( element, slot, ammo, event )
	if not canAccessElement( source, element ) then
		outputChatBox("No puedes acceder a este inventario en este momento.", source, 255, 0, 0)
	...
	if not ammo then
		...
		if item then
			...
			if ( (item[1] >= 31 and item[1] <= 43) or itemBannedByAltAltChecker[item[1]]) and not (getElementModel(element) == 2942 and item[1] == 150) then
				...
				if not exports.global:isStaffOnDuty(source) and not exports.global:isStaffOnDuty(element) then
					if hoursPlayedFrom < 10 then
						outputChatBox("Necesitas 10 horas jugadas para mover este "..getItemName( item[1] ).." a un "..name..".", source, 255, 0, 0)
						triggerClientEvent( source, event or "finishItemMove", source )
						return false
					end

					if hoursPlayedTo < 10 then
						outputChatBox(string.gsub(getPlayerName(element), "_", " ").." requiere 10 horas de juego para recibir un "..getItemName( item[1] )..".", source, 255, 0, 0)
						outputChatBox("Se necesitan 10 horas de juego para recibir un "..getItemName( item[1] ).." de "..string.gsub(getPlayerName(source), "_", " ")..".", element, 255, 0, 0)
						triggerClientEvent( source, event or "finishItemMove", source )
						return false
					end
				end
			...
			if getElementType(element) == "vehicle" and getElementData(element, "faction") < 0 then 
				...
				if vehicleAccountID and accountID.id == vehicleAccountID.id and getElementData(source, "account:character:id") ~= getElementData(element, "owner") then
					...
					outputChatBox(" No puedes poner objetos en el vehículo de tus personajes alternativos", source, 255, 0, 0)
					triggerClientEvent( source, event or "finishItemMove", source )
					return false
				end
			end
			...
			if (getElementType(element) == "ped") and getElementData(element,"shopkeeper") then
				...
				if getElementData(element,"customshop") then
					...
					if restricted[item[1]] then
						outputChatBox('.', source, 255, 100, 100)
						triggerClientEvent( source, event or "finishItemMove", source )
						return false
					end
			...
			if not (getElementModel( element ) == 2942) and not hasSpaceForItem( element, item[1], item[2] ) then --Except for ATM Machine
				outputChatBox( "El inventario está lleno.", source, 255, 0, 0 )
			else
				if (item[1] == 115) then -- Weapons
					...
					if tonumber(weaponDetails[2]) then
						...
						if gun_source == 2  then
							outputChatBox("No puedes colocar tu arma en un " .. name .. ".", source, 255, 0, 0)
							triggerClientEvent( source, event or "finishItemMove", source )
							return
				...
				if (item[1] == 137 or item[1] == 162 or item[1] == 219 or item[1] == 220 or item[1] == 221 or item[1] == 262 or item[1] == 263) then -- Snake cam and armors
					outputChatBox("No puedes mover este item.", source, 255, 0, 0)
					triggerClientEvent( source, event or "finishItemMove", source )
					return
				elseif item[1] == 138 then
					if not exports.integration:isPlayerTrialAdmin(source) and getElementType(element) == "Vehicle" then
						outputChatBox("Sólo un administrador puede instalar este elemento.", source, 255, 0, 0)
						triggerClientEvent( source, event or "finishItemMove", source )
						return
					end
				elseif item[1] == 139 then
					if not exports.integration:isPlayerTrialAdmin(source) then
						outputChatBox("Se requiere un administrador de la prueba para mover este elemento.", source, 255, 0, 0)
						triggerClientEvent(source, event or "finishItemMove", source)
						return
					end
				end
				...
				if (item[1] == 134) then -- Money
					if not exports.global:isStaffOnDuty(source) and not exports.global:isStaffOnDuty(element) then
						...
						if (getElementType(element) == "player") and (getElementType(source) == "player") then
							if hoursPlayedFrom < 5 or hoursPlayedTo < 5 then
								outputChatBox("Necesitas 5 horas de juego para pagar a otro jugador.", source, 255, 0, 0)
								outputChatBox(exports.global:getPlayerName(source).." requiere de 5 horas de juego para pagar.", element, 255, 0, 0)
								triggerClientEvent( source, event or "finishItemMove", source )

								outputChatBox("Necesitas 5 hora de juego para recibir el pago.", element, 255, 0, 0)
								outputChatBox(exports.global:getPlayerName(element).." requiere de 5 hora de juego para recibir el pago.", source, 255, 0, 0)
								triggerClientEvent( source, event or "finishItemMove", source )
								return false
							end
						elseif (getElementType(element) == "vehicle") and (getElementType(source) == "player") then
							if hoursPlayedFrom < 10 then
								outputChatBox("Se necesitan 10 horas de juego para guardar dinero en un vehículo.", source, 255, 0, 0)
								triggerClientEvent( source, event or "finishItemMove", source )
								return false
							end
						elseif (getElementType(element) == "object") and (getElementType(source) == "player") then
							if hoursPlayedFrom < 10 then
								outputChatBox("Se requieren 10 horas de juego para almacenar dinero en ese.", source, 255, 0, 0)
								triggerClientEvent( source, event or "finishItemMove", source )
								return false
							end
						end
					...
					if exports.global:takeMoney(source, tonumber(item[2])) then
						if getElementType(element) == "player" then
							if exports.global:giveMoney(element, tonumber(item[2])) then
								triggerEvent('sendAme', source, "le da $" .. exports.global:formatMoney(item[2]) .. " a ".. exports.global:getPlayerName(element) .."." )
							end
						else
							if exports.global:giveItem(element, 134, tonumber(item[2])) then
								triggerEvent('sendAme', source, "pone $" .. exports.global:formatMoney(item[2]) .. " dentro de "..  name .."." )
							end
						end
					end
				else -- not money
					if getElementType( element ) == "object" then
						...
						if elementItemID then
							if elementItemID == 166 then --video player
								if item[1] ~= 165 then --if item being moved to video player is not a valid video item
									exports.hud:sendBottomNotification(source, "Reproductor de Video", "Este no es un disco válido.")
									triggerClientEvent( source, event or "finishItemMove", source )
									return
								end
							end
						...
						if ( getElementDimension( element ) < 19000 and ( item[1] == 4 or item[1] == 5 ) and getElementDimension( element ) == item[2] ) or ( getElementDimension( element ) >= 20000 and item[1] == 3 and getElementDimension( element ) - 20000 == item[2] ) then -- keys to that safe as well
							if countItems( source, item[1], item[2] ) < 2 then
								outputChatBox("No puedes colocar tu única llave de esa caja fuerte en la caja fuerte.", source, 255, 0, 0)
								triggerClientEvent( source, event or "finishItemMove", source )
								return
							end
						end
					...
					if not success then
						...
						if elementItemID then
							if elementItemID == 166 then --video system
								exports.hud:sendBottomNotification(source, "Reproductor de Video", "Ya hay un disco dentro. Expulse primero el disco antiguo.")
								fakeReturned = true
							end
						end
						if not fakeReturned then --only check by model IDs if we didnt already find a match on itemID
							if getElementModel(element) == 2942 then
								exports.hud:sendBottomNotification(source, "Cajero ATM", "Hay otro cajero atascado dentro de la ranura del cajero. Haz clic con el botón derecho para interactuar.")
							end
						end
						outputDebugString( "Error al movimiento del item: " .. tostring( reason ))
					else
						if getElementModel(element) == 2942 then
		...
		if not ( ( slot == -100 and hasSpaceForItem( element, slot ) ) or ( slot > 0 and hasSpaceForItem( element, -slot ) ) ) then
			...
			if tonumber(getElementData(source, "duty")) > 0 then
				outputChatBox("No puedes poner tus armas en un " .. name .. " estando en servicio.", source, 255, 0, 0)
			elseif tonumber(getElementData(source, "job")) == 4 and slot == 41 then
				outputChatBox("No se puede poner este bote de spray en un " .. name .. ".", source, 255, 0, 0)
			else
				if slot == -100 then
					...
					if ammo > 0 then
						...
						exports.logs:dbLog(source, 39, source, getPlayerName(source) .. " movió " .. getItemName(slot) " - " .. ammo .. " #" .. getElementID(element) )
						output(source, element, -100)
					end
				else
					local getCurrentMaxAmmo = exports.global:getWeaponCount(source, slot)
					if ammo > getCurrentMaxAmmo then
						exports.global:sendMessageToAdmins("[items\moveToElement] Posible duplicación de arma '"..getPlayerName(source).."' // " .. getItemName( -slot ) )
						exports.logs:dbLog(source, 39, source, getPlayerName(source) .. " movió " .. getItemName(-slot) " -  #" .. getElementID(element) .. " - BLOQUEADO POR POSIBLE DUPLICACIÓN" )
						triggerClientEvent( source, event or "finishItemMove", source )
						return
					end
...
local function moveWorldItemToElement( item, element )
	...
	if not canPickup(source, item) then
		outputChatBox("No puedes mover este item. Contacta a un admin via F1.", source, 255, 0, 0)
		return
	end

	if itemID == 138 then
		if not exports.integration:isPlayerTrialAdmin(source) and getElementType(element) == "Vehicle" then
			outputChatBox("Solo admins pueden colocar este item.", source, 255, 0, 0)
			return
		end
	end
...
local function moveFromElement( element, slot, ammo, index )
	...
	if item and tonumber(item[3]) == tonumber(index) then
			...
			if not exports.global:isStaffOnDuty(source) and not exports.global:isStaffOnDuty(element) then
				if hoursPlayedTo < 10 then
					if not (item[1] == 3 and getElementData(exports.pool:getElement("vehicle", item[2]), "owner") == getElementData(source, "dbid")) then -- Checks if they own the vehicle for the key
						outputChatBox("Necesitas 10 horas para recibir un/a "..getItemName( item[1] ).." de "..name..".", source, 255, 0, 0)
		...
		elseif not exports.integration:isPlayerTrialAdmin( source ) and getElementType( element ) == "vehicle" and ( item[1] == 61 or item[1] == 85  or item[1] == 117 or item[1] == 140) then
			outputChatBox( "Porfavor contacta con un admin para mover este item.", source, 255, 0, 0 )
		elseif not exports.integration:isPlayerAdmin(source) and (item[1] == 138) then
			outputChatBox("Este artículo requiere un administrador regular para ser movido.", source, 255, 0, 0)
		elseif not exports.integration:isPlayerTrialAdmin(source) and (item[1] == 139) then
			outputChatBox("Este item requiere un admin para ser movido.", source, 255, 0, 0)
		...
		elseif item[1] == -100 then
			...
			if armor == 0 then
				outputChatBox( "No te puedes poner mas armadura.", source, 255, 0, 0 )
			else
