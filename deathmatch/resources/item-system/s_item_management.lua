...
local drugList = {[30]=" gramo(s)", [31]=" gramo(s)", [32]=" gramo(s)", [33]=" gramo(s)", [34]=" gramo(s)", [35]=" ml(s)", [36]=" tableta(s)", [37]=" gramo(s)", [38]=" gramo(s)", [39]=" gramo(s)", [40]=" ml(s)", [41]=" ficha(s)", [42]=" setas(s)", [43]=" tableta(s)"}
		...
		if isThisFromSplittingOrAdminCmd then
			if drugList[itemID] then
				if not tonumber(itemValue) or tonumber(itemValue) < 1 then
					return false, "El valor de la droga debe ser numérico y debe estar en gramos."
				else
								...
								if ( (itemID >= 31) and (itemID <= 43) ) or itemBannedByAltAltChecker[itemID] then
									...
									if not exports.global:isStaffOnDuty(to) and not exports.global:isStaffOnDuty(from) then
										if hoursPlayedFrom < 10 then
											outputChatBox("Necesitas 10 horas para mover un "..getItemName( itemID ).." a "..getName(to)..".", from, 255, 0, 0)
											return false, "Movimiento del item cancelado, < 10 horas"
										end
										...
										if hoursPlayedTo < 10 then
											if not (itemID == 3 and getElementData(exports.pool:getElement("vehicle", itemValue), "owner") == getElementData(source, "dbid")) then -- Checks if they own the vehicle for the key
												outputChatBox("Necesitas 10 horas para recibir un "..getItemName( itemID ).." de "..getName(from)..".", to, 255, 0, 0)
												return false, "Movimiento del item cancelado, < 10 horas"
											end
										end
									end
