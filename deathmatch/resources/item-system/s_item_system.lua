...
armor = {
	[219]  = {49, "type2", "coge su chaleco y se lo coloca debajo de la ropa", "agarra su chaleco y se lo quita"},
	[220]  = {74, "type3", "coge su chaleco blindado y se lo pone", "agarra su chaleco blindado y se lo quita"},
	[221]  = {100, "type4", "coge su chaleco blindado y se lo pone", "agarra su chaleco blindado y se lo quita"},
}
...
function useItem(itemSlot, additional)
	...
	if itemID then
		if (itemID==1) then -- hotdog
			...
			triggerEvent('sendAme', source, "come un hotdog.")
		...
		elseif (itemID==4) or (itemID==5) then -- house key or business key
			...
			if found then
				...
				if locked == 0 then
					triggerEvent('sendAme', source, "pone la llave en la puerta para abrirla.")
				else
					newRealLockedValue = true
					triggerEvent('sendAme', source, "pone la llave en la puerta para cerrarla.")
				end
				...
				outputChatBox("Estás muy lejos de la puerta.", source, 255, 194, 14)
			end
		...
		elseif (itemID==9) then -- sprunk
			...
			toggleAllControls(source, true, true, true)
			triggerEvent('sendAme', source, "bebe un refresco.")
			takeItemFromSlot(source, itemSlot)
		...
		elseif (itemID==11) then -- taco
			...
			giveHealth(source, 10)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "come un taco.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==12) then -- cheeseburger
			...
			triggerEvent('sendAme', source, "come una hamburguesa.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==13) then -- donut
			...
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "come una dona.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==14) then -- cookie
			...
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "come una galletita.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==15) then -- water
			giveHealth(source, 90)
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			triggerEvent('sendAme', source, "bebe una botella de agua.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==16) then -- clothes
			...
			if clothingid or fits then
				...
				triggerEvent('sendAme', source, "se cambia de ropa.")
			else
		...
		elseif (itemID==17) then -- watch
			...
			triggerEvent('sendAme', source, "mira su " ..metaName .. ".")
			outputChatBox("Hora: " .. string.format("%02d:%02d", getTime()) .. ".", source, 255, 194, 14)
		...
		elseif (itemID==20) then -- STANDARD FIGHTING
			...
			outputChatBox("Lees un libro sobre pelea básica.", source, 255, 194, 14)
		...
		elseif (itemID==21) then -- BOXING
			...
			outputChatBox("Lees un libro sobre Boxeo.", source, 255, 194, 14)
		...
		elseif (itemID==22) then -- KUNG FU
			...
			outputChatBox("Lees un libro sobre Kung Fu.", source, 255, 194, 14)
		...
		elseif (itemID==23) then -- KNEE HEAD
			...
			outputChatBox("Abres el libro, y notas que el libro está escrito en griego antiguo.", source, 255, 194, 14)
		...
		elseif (itemID==24) then -- GRAB KICK
			...
			outputChatBox("Lees un libro sobre Kick Fighting.", source, 255, 194, 14)
		...
		elseif (itemID==25) then -- ELBOWS
			...
			outputChatBox("Lees un libro sobre Elbow Fighting.", source, 255, 194, 14)
		...
		elseif (itemID==27) then -- FLASHBANG
			...
			triggerEvent('sendAme', source, "tira una cegadora.")
		...
		elseif (itemID==28) then -- GLOWSTICK
			...
			triggerEvent('sendAme', source, "tira una bara luminosa.")
			setTimer(destroyElement, 600000, 1, marker)
		elseif (itemID==29) then -- RAM
			outputChatBox("Reporta para usar este item.", source, 255, 0, 0)
		elseif (itemID==34) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "snifa algo de Cocaina.")
		elseif (itemID==35) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "se traga una pastila de Morphine.")
		elseif (itemID==36) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "se traga una pastilla de Ecstasy.")
		elseif (itemID==37) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "se injecta algo de Heroina.")
                ...
                if sucess then
                    if (tonumber(itemvalue) >= 1) then
                        if (hasSpaceForItem(source, 182, 1)) then
                            ...
                            exports.global:sendLocalMeAction(source, "saca un cogollo de marihuana de la bolsa, empezando a liar el porro después.")
                            giveItem(source, 181, itemvalue - 1)
                        else
                            outputChatBox( "Tu inventario está lleno.", source, 255, 0, 0 )
                        end
                    else
                        outputChatBox("El paquete de papeles de cigarros se ha acabado", source, 255, 0, 0)
                    end
                else
                    outputChatBox("Necesitas un paquete de papeles de cigarros para usar esto.", source, 255, 0, 0)
                end
            else
                triggerEvent('sendAme', source, "mira dentro de su bolsa de marihuana. Está vacía.")
            end
		elseif (itemID==39) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "aspira algo de Methamphetamine.")
		elseif (itemID==40) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "se injecta una Epinephrine.")
		elseif (itemID==41) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "vierte una gota de LSD en su boca.")
		elseif (itemID==42) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "come unas setas")
		elseif (itemID==43) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "traga unas pastillas de PCP")
		...
		elseif (itemID==50) then -- highway code book
			local bookTitle = "Código de Conducción de Riverside"
			local bookName = "LSHighwayCode"
			triggerEvent('sendAme', source, "lee ".. bookTitle ..".")
		...
		elseif (itemID==51) then -- chemistry book
			...
			triggerEvent('sendAme', source, "lee un ".. bookTitle ..".")
			triggerClientEvent( source, "showBook", source, bookName, bookTitle )
		...
		elseif (itemID==54) then -- GHETTOBLASTER
			triggerEvent('sendAme', source, "coloca un ghettoblaster en el piso.")
		...
		elseif (itemID==57) then -- FUEL CAN
			...
			if found then
				...
				outputChatBox("Está demasiado lejos de un vehículo.", source, 255, 194, 14)
			end
		elseif (itemID==58) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "bebe una buena cerveza Ziebrand.")
			setElementHealth(source,getElementHealth(source)-5)
		elseif (itemID==59) then -- MUDKIP
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "come un mudkip.")
			--killPed(source)
		...
		elseif (itemID==62) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "bebe un Vodka puro.")
			setElementHealth(source,getElementHealth(source)-10)
		elseif (itemID==63) then
			takeItemFromSlot(source, itemSlot)
			triggerEvent('sendAme', source, "bebe un Whiskey.")
			setElementHealth(source,getElementHealth(source)-10)
		elseif (itemID==68) then
			triggerEvent('sendAme', source, "mira su billete de lotería.")
			outputChatBox("Miras el billete de lotería y lees el número: " .. itemValue, source, 0, 255, 0)
		elseif (itemID==69) then -- Dictionary
			...
			if (learned) then
				...
				outputChatBox("Has aprendido lo básico de " .. lang .. ", Presiona F6 para ver tus idiomas.", source, 0, 255, 0)
			end
		elseif (itemID==72) then -- Note
			triggerEvent('sendAme', source, "lee una nota.")
		...
		elseif (itemID==83) then -- Coffee
			...
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			triggerEvent('sendAme', source, "bebe una taza de café.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==88) then -- earpiece
			outputChatBox("Puedes utilizar este auricular con una radio.", source, 255, 194, 14)
		elseif (itemID==89) then -- Generic Food
			...
			triggerEvent('sendAme', source, "come un " .. itemExploded[1] .. ".")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==91) then
			takeItemFromSlot(source, itemSlot)
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			triggerEvent('sendAme', source, "bebe algo de buen ponche de huevo.")
			setElementHealth(source, 100)
		elseif (itemID==92) then
			setElementHealth(source, 100)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "come algo de pavo.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==93) then
			...
			takeThirst(source, 5)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "come un pudín de navidad.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==94) then
			...
			triggerEvent('sendAme', source, "abre un regalo de Navidad")
		elseif (itemID==95) then -- Generic Drink
			giveHealth(source, 50)
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 2000, false, true, true)
			local itemExploded = explode(":", itemValue)
			triggerEvent('sendAme', source, "bebe algo de " .. itemExploded[1] .. ".")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==96) then -- PDA
			triggerEvent('sendAme', source, "enciende su " .. itemName .. ".")
		...
		elseif (itemID==97) then -- SFES Procedures Manual (book)
			...
			triggerClientEvent( source, "showBook", source, bookName, bookTitle )]]
			outputChatBox("Este artículo ha sido desactivado y ya no está en uso.", source, 255, 0, 0)
		...
		elseif (itemID==99) then --[ADDED: 2/22/10 by herbjr] tray
			...
			triggerEvent('sendAme', source, "se come la comida de la bandeja.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==100) then --[ADDED: 2/22/10 by herbjr] milk
			...
			triggerEvent('sendAme', source, "bebe una caja de leche.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==101) then --[ADDED: 2/22/10 by herbjr] juice
			...
			triggerEvent('sendAme', source, "bebe una caja de jugo.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==102) then --[ADDED: 2/23/10 by herbjr] Cabbage (Asked for by Misha)
			...
			triggerEvent('sendAme', source, "come un cabeza de repollo.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==104) then -- portable TV
			...
			if isTvUsed == nil or isTvUsed == false then
				triggerEvent('sendAme', source, "prende una pequeña television portable.")
		...
		elseif (itemID == 105) then -- Pack of cigarettes. Withdraw one if it still has one
			if (itemValue > 0) then
				if (hasSpaceForItem(source, 106, 1)) then
					...
					triggerEvent('sendAme', source, "mira dentro de su paquete de cigarrillos, y saca uno.")
				else
					outputChatBox( "Tu inventario está lleno.", source, 255, 0, 0 )
				end
			else
				triggerEvent('sendAme', source, "mira dentro de su paquete de cigarrillos. Está vacío.")
			end
		elseif (itemID == 106) then -- Cigarette
			if hasItem( source, 107 ) then
				triggerEvent('sendAme', source, "prende un cigarro.")
				outputChatBox( "(( /throwaway para tirarlo, /switchhand para cambiar de mano ))", source )
				triggerEvent("realism:startsmoking", source, 0) -- 0 = left hand -- 1 = right hand
				takeItemFromSlot(source, itemSlot)
			else
				triggerEvent('sendAme', source, "muestra a todos un cigarrillo.")
				outputChatBox( "Necesitas un encendedor para prenderlo.", source, 255, 0, 0 )
			end
		elseif (itemID == 107) then
			triggerEvent('sendAme', source, "muestra a todos un encendedor.")
		elseif (itemID==108) then -- donut
			setElementHealth(source, 100)
			giveHunger(source, 35)
			takeThirst(source, 7)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "come un pancake.")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID==109) or (itemID==110) then -- Fruit
			...
			takeThirst(source, 7)
			exports.global:applyAnimation(source, "food", "eat_burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "come un " .. itemName .. ".")
			takeItemFromSlot(source, itemSlot)
		elseif (itemID == 113) then -- Pack of glowsticks, Withdraw one if it still has one
			if tonumber(itemValue) and tonumber(itemValue) > 0 then
				if (hasSpaceForItem(source, 28, 1)) then
					...
					triggerEvent('sendAme', source, "mira dentro de su paquete de glowsticks, deslizando uno hacia fuera.")
				else
					outputChatBox( "Tu inventario está lleno.", source, 255, 0, 0 )
				end
			else
		...
		elseif itemID == 134 then
			outputChatBox("$" .. exports.global:formatMoney(itemValue) .. " de dinero.", source)
		elseif itemID == 137 then -- New snake cam, Adams
			triggerEvent('snakecam:toggleSnakeCam', root, source)
		elseif itemID == 138 then
			outputChatBox("Coloque este dispositivo en un inventario de vehículos para instalarlo.", source, 0, 255, 0)
		...
		elseif masks[itemID] then
			...
			elseif getPedOccupiedVehicle(source) and getElementData(getPedOccupiedVehicle(source), "job") > 0 and getVehicleOccupant(getPedOccupiedVehicle(source)) == source then
				outputChatBox("No puedes usar esto en autos civiles.", source, 255, 0, 0)
			else
		...
		elseif itemID == 139 then
			outputChatBox("((Necesitas un admin para colocar esto en tu auto.))", source, 255, 0, 0)
		...
		elseif itemID == 173 then
			...
			if distance < 25 then
				...
				outputChatBox("Para vender su vehículo, debe estar en el estacionamiento del DMV.", source, 0, 255, 0)
			end
        ...
        elseif itemID == 181 then
        	triggerEvent("sendAme", source, "muestra a todos un Paquete de Cigarros.")
    	elseif (itemID == 182) then
            if hasItem( source, 107 ) then -- If da player has a lighter, how else would he smoke it dawg
                exports.global:sendLocalMeAction(source, "enciende un porro enrollado.")
                outputChatBox( "(( /throwaway para tirarlo, /switchhand para cambiar de mano, /passjoint para pasarlo ))", source )
                ...
                exports.global:sendLocalMeAction(source, "muestra a todos un porro enrollado.")
                outputChatBox("Necesitarás un encendedor para usarlo correctamente.", source, 255, 0, 0)
            end
		...
		elseif (itemID==210) then -- Coca-Cola Christmas Edition
			...
			--exports.global:sendLocalMeAction(thePlayer, "bebe una coke.")
			triggerEvent('sendAme', thePlayer, "bebe una coca.")
		...
		elseif (itemID==213) then --pinnekjott
			giveHealth(source, 10)
			exports.global:applyAnimation(source, "FOOD", "EAT_Burger", 4000, false, true, true)
			triggerEvent('sendAme', source, "come un pinnekjott.")
			takeItemFromSlot(source, itemSlot)
		...
		elseif itemID == 232 then -- car battery charger
			...
			if batt == 100 then
				exports.hud:sendBottomNotification( source, itemName, "La batería de este "..exports.global:getVehicleName(veh).." está al máximo." )
			else
				if takeItemFromSlot( source, itemSlot) then
					exports.anticheat:setEld( veh, 'battery', math.min( 100, batt + 20 ) )
					exports.hud:sendBottomNotification( source, itemName, "La batería de este "..exports.global:getVehicleName(veh).." se ha cargado un 20%.." )
					triggerEvent( 'fuel:sync', veh, source )
				end
			end
		elseif itemID == 260 then
			exports.global:applyAnimation(source, "VENDING", "VEND_Drink_P", 4000, false, true, true)
			exports.global:sendLocalMeAction(source, "bebe algo de Ammonia.")
		...
		elseif itemID == 270 then
			...
			if getElementData(source, "gloves") then
				triggerEvent('sendAme', source, "se coloca un par de " .. metaName .. " de sus manos.")
				...
				triggerEvent('sendAme', source, "se quita un par de " .. metaName .. " de sus manos.")
...
function npcUseItem(element, itemID)
	...
	if itemID then
		...
		if(hasItem) then
			...
			if badges[itemID] then
				if ( getElementData( source, badges[itemID][1] ) ) then
					...
					if pedname then
						...
						exports.global:sendLocalText(source, "* "..tostring(pedname).." quita "..badges[itemID][2]..".", 255, 51, 102, 5)
					end
				else
						...
						if key ~= itemID then
							if ( getElementData ( source, badge[1] ) ) then
								...
								if pedname then
									...
									exports.global:sendLocalText(source, "* "..tostring(pedname).." quita "..badge[2]..".", 255, 51, 102, 5)
								end
							end
						end
					end
					...
					if pedname then
						...
						exports.global:sendLocalText(source, "* "..tostring(pedname).." pone "..badges[itemID][2]..".", 255, 51, 102, 5)
					end
					return true
				end
			...
			outputDebugString("El ped no tiene ese elemento")
			return false
		end
	end
end
...
function destroyItem(itemID)
	...
	outputChatBox("Haz destruido un " .. itemName .. ".", source, 255, 194, 14)
	triggerEvent('sendAme', source, "Destruye un "..itemName..".")
end
...
function sortMoney(thePlayer, commandName, moneyAmountCurrent, moneyAmountSort)
	if not (moneyAmountCurrent) or not (moneyAmountSort) or tonumber(moneyAmountCurrent) < 0 or tonumber(moneyAmountSort) < 0 then
		outputChatBox("SYNTAX: /" .. commandName .. " [Dinero que tienes] [Dinero a sortear]", thePlayer, 255, 194, 14)
	else
		...
		if tonumber(moneyAmountCurrent) <= tonumber(playerMoney) and tonumber(moneyAmountSort) <= tonumber(playerMoney) then
			if hasItem(thePlayer, 134, tonumber(moneyAmountCurrent)) then
				outputChatBox("Haz sorteado $" .. exports.global:formatMoney(moneyAmountSort) .. " de $" .. exports.global:formatMoney(moneyAmountCurrent) .. ".", thePlayer)
				...
				outputChatBox("No tienes esa cantidad de dinero en un artículo ordenado.", thePlayer, 255, 0, 0)
			end
		else
			outputChatBox("No tienes esa cantidad de dinero.", thePlayer, 255, 0, 0)
		end
	end
end
...
function combineMoney(thePlayer, commandName)
		...
		if tonumber(playerMoney) > 0 then
			...
			outputChatBox("Combinas $" .. exports.global:formatMoney(playerMoney) .. ".", thePlayer)
		else
			outputChatBox("No puedes combinar dinero que no tienes", thePlayer, 255, 0, 0)
		end
end
...
function toggleBadge(source, badges, itemID , itemValue)
	if ( getElementData( source, badges[itemID][1] ) ) then
		...
		if itemID == 122 or itemID == 123 or itemID == 124 or itemID == 125 or itemID == 135 or itemID == 136 or itemID == 158 or itemID == 168 then
			triggerEvent('sendAme',  source, "se quita una " .. badges[itemID][1] .. " de su cabeza." )
		else
			triggerEvent('sendAme', source, "se quita una " .. badges[itemID][1] .. ".")
		end
	else
		for key, badge in pairs ( badges ) do
			if key ~= itemID then
				if ( getElementData ( source, badge[1] ) ) then
					exports.anticheat:changeProtectedElementDataEx( source, badge[1], false, true )
					if itemID == 122 or itemID == 123 or itemID == 124 or itemID == 125 or itemID == 135 or itemID == 136 or itemID == 158 or itemID == 168 then
					triggerEvent('sendAme',  source, "se quita una " .. badge[1] .. " de su cabeza." )
					else
					triggerEvent('sendAme',  source, "se quita una " .. badge[1] .. "." )
					end
				end
			end
		end
		...
		triggerEvent('sendAme',  source, "envuelve un " .. badges[itemID][1] .. " alrededor de su cabeza." )
		else
			exports.anticheat:changeProtectedElementDataEx( source, badges[itemID][1], removeOOC(itemValue), true )
			triggerEvent('sendAme',  source, "se pone un " .. badges[itemID][1] .. "." )
		end
	end
	exports.global:updateNametagColor(source)
end
