...
local opensans = dxCreateFont("butcher/files/opensans.ttf",14)
...
addEventHandler("onClientResourceStart",resourceRoot,function()
	...
	meatMarker = createMarker(942.20703125, 2135.830078125, 1011.0234375,"cylinder",1,65, 105, 225,60)
	setElementDimension(meatMarker,dim)
	setElementInterior(meatMarker,int)
	setElementData(meatMarker,"boxingMarker",true)
	
	dropMarker = createMarker(949.71875, 2169.3369140625, 1011.0302734375,"cylinder",1,65, 105, 225,60)
					...
					if showMeat then
						if progress >= 0 then
							...
							if progress+(loss/rand) >= 156.2 then
								...
								outputChatBox("#4169E1[Nine Nine Roleplay] #ffffffLleva la carne a la maquina para empaquetarla.",255,255,255,true)
							end
						end
					end
				end
		...
		if getElementData(source, "boxingMarker") then
			if getElementData(localPlayer, "char.job") == 1 then
				if getElementData(localPlayer,"isMeatInHand") then
					...
					if not isElement(boxing) then
						boxing = createObject(2806,942.3203125, 2154.3154296875, 1011.0234375)
						...
						moveObject ( boxing, 20000,942.3203125, 2154.3154296875, 1011.0234375)
					end
...
function()
	txd = engineLoadTXD( "butcher/machete/machete.txd" )
	dff = engineLoadDFF( "butcher/machete/machete.dff" )
