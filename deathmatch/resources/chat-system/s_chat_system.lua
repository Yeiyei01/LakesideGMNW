...
function localIC(source, message, language)
	...
	if playerVehicle then
		if (exports.vehicle:isVehicleWindowUp(playerVehicle)) then
			...
			outputChatBox( " [" .. languagename .. "] " .. playerName .. " ((En vehículo)) dice: " .. message, source, unpack(color))
		else
						...
						if (exports.vehicle:isVehicleWindowUp(pveh)) then
							for i = 0, getVehicleMaxPassengers(pveh) do
								...
								if (lp) and (lp~=source) then
									...
									outputChatBox(" [" .. languagename .. "] " .. playerName .. " ((En vehículo)) dice: " .. message2, lp, unpack(color))
...
function meEmote(source, cmd, ...)
	...
	if logged == 1 then
		...
		if not (...) then
			outputChatBox("SYNTAX: /me [Accion]", source, 255, 194, 14)
		else
...
function outputChatBoxCar( vehicle, target, text1, text2, color )
	if vehicle and exports.vehicle:isVehicleWindowUp( vehicle ) then
		if getPedOccupiedVehicle( target ) == vehicle then
			outputChatBox( text1 .. " ((En vehículo))" .. text2, target, unpack(color))
			return true
		else
			return false
		end
...
function radio(source, radioID, message, cmd)
	...
	if jailed > 0 then
		outputChatBox('No puedes usar la radio en jail.', source, 255, 0, 0)
		return
	end
	...
	if isPedDead(source) then
		outputChatBox(" No puedes usar la radio mientras estás muerto.", source, 255, 0, 0)
		return
	end
	...
	if hasRadio or getElementType(source) == "ped" or radioID == -2 then
		...
		if theChannel == 1 or theChannel == 0 then
			outputChatBox("Por favor, cambia la frecuencia de radio con /hz [frecuencia]", source, 255, 194, 14)
		elseif not hasChannelAccess(source, theChannel) then
			outputChatBox("No tienes permitido acceder a esta frecuencia. Por favor, reajusta la frecuencia.", source, 255, 194, 14)
		elseif (theChannel > 1 or radioID < 0) and itemValue > 0 then
			...
			elseif channelName == "DEPARTMENT" then
				...
				if cmd == "rlow" then
					...
					triggerEvent("sendAme", source, "susurra a su radio.")
				else
			...
			outputChatBox("Tu radio está apagada. ((/tradio))", source, 255, 0, 0)
		end
	else
		...
		if not (targetPlayerNick) then
			...
			for k, v in ipairs(ignoreList[thePlayer] or {}) do
				if v == targetPlayer then
					...
					outputChatBox("You're no longer ignoring susurra from " .. targetPlayerName .. ".", thePlayer, 0, 255, 0)
					existed = true
					break
				end
			end
			if not existed then
				...
				outputChatBox("You're ignoring susurra from " .. targetPlayerName .. ".", thePlayer, 0, 255, 0)
...
function localShout(thePlayer, commandName, ...)
	...
	if not (isPedDead(thePlayer)) and (logged==1) then
		if not (...) then
			...
			outputChatBox("[" .. languagename .. "] " .. playerName .. " grita:¡" .. message .. "!", thePlayer, r, g, b)
...
addLocalizedCommandHandler("g", localShout, false, false)
addLocalizedCommandHandler("gritar", localShout, false, false)
...
function megaphoneShout(thePlayer, commandName, ...)
	...
	if not (isPedDead(thePlayer)) and (logged==1) then
		...
		if types[2] or types[3] or types[4] or (exports.global:hasItem(thePlayer, 141)) or ( isElement(vehicle) and exports.global:hasItem(vehicle, 141) and (seat==1 or seat==0)) then
					...
					if getElementDistance( thePlayer, nearbyPlayer ) < 40 then
						...
						if (nearbyPlayerDimension==dimension) and (nearbyPlayerInterior==interior) then
							...
							if (logged==1) and not (isPedDead(nearbyPlayer)) then
								...
								table.insert(affectedElements, nearbyPlayer)
								outputChatBox(" [" .. langname .. "] ((" .. playerName .. ")) Megáfono <O: " .. trunklateText(nearbyPlayer, message2), nearbyPlayer, 255, 255, 0)
							end
						end
					end
				end
			...
			outputChatBox("Creas o no, es dificil gritar por un megafono si no tienes uno.", thePlayer, 255, 0 , 0)
		end
	end
end
...
function toggleFaction(thePlayer, commandName)
	...
	if fL then
		if togState[pF] == false or not togState[pF] then
			togState[pF] = true
			outputChatBox("Chat de facción desactivado.", thePlayer)
			for index, arrayPlayer in ipairs( exports.pool:getPoolElementsByType( "player" ) ) do
				if isElement( arrayPlayer ) then
					if exports.factions:isPlayerInFaction(arrayPlayer, pF) and getElementData(thePlayer, "loggedin") == 1 then
						outputChatBox("((".. theTeamName .. ")) Chat OOC de facción desactivado", arrayPlayer, 255, 0, 0)
					end
				end
			end
		else
			togState[pF] = false
			outputChatBox("Chat de facción activado.", thePlayer)
			for index, arrayPlayer in ipairs( exports.pool:getPoolElementsByType( "player" ) ) do
				if isElement( arrayPlayer ) then
					if exports.factions:isPlayerInFaction(arrayPlayer, pF) and getElementData(thePlayer, "loggedin") == 1 then
						outputChatBox("((".. theTeamName .. ")) Chat de facción OOC activado", arrayPlayer, 0, 255, 0)
					end
				end
			end
		end
...
function toggleFactionSelf(thePlayer, commandName)
	...
	if(logged==1) then
		...
		if (factionBlocked==1) then
			...
			outputChatBox("((".. teamName ..")) Chat de facción activado para tí.", thePlayer, 0, 255, 0)
		else
			exports.anticheat:changeProtectedElementDataEx(thePlayer, "chat-system:blockF"..pF, 1, false)
			outputChatBox("((".. teamName ..")) Chat de facción desactivado para tí.", thePlayer, 255, 0, 0)
		end
	end
end
	...
	if (logged==1) then
		if not (...) then
			...
			if not playerFaction then
				outputChatBox("No estás en esta facción.", thePlayer)
				return
			end
	...
	if (logged==1) then
		if not (...) then
			...
			if not exports.factions:hasMemberPermissionTo(thePlayer, playerFaction, "use_fl") then
				outputChatBox("No eres lider de facción.", thePlayer, 255, 0, 0)
			else
				local theTeam = exports.factions:getFactionFromID(playerFaction)
				local theTeamName = getTeamName(theTeam)
				...
				for index, arrayPlayer in ipairs( getElementsByType( "player" ) ) do
					if isElement( arrayPlayer ) then
						...
						elseif isIn and getElementData(arrayPlayer, "loggedin") == 1 and getElementData(arrayPlayer, "chat-system:blockF"..playerFaction) ~= 1 and exports.factions:hasMemberPermissionTo(arrayPlayer, playerFaction, "use_fl") then
							table.insert(affectedElements, arrayPlayer)
							outputChatBox("((" .. theTeamName .. " Lider)) " .. playerName .. ": " .. message, arrayPlayer, 3, 180, 200)
						end
					end
				end
				...
				if v[1] == 6 then
					...
					if count == slot then
						if tonumber(v[2]) > 0 then
							...
							outputChatBox("Your radio is off. ((/tradio))", thePlayer, 255, 0, 0)
						end
						return
					end
				end
...
addCommandHandler("hz", setRadioChannel, false, false)
...
function toggleRadio(thePlayer, commandName, slot)
	if (exports.global:hasItem(thePlayer, 6)) then
		...
		local genderm = getElementData(thePlayer, "gender") == 1 and "su" or "su"

		if titemValue < 0 then
			outputChatBox("Haz encendido tu radio.", thePlayer, 255, 194, 14)
			triggerEvent('sendAme', thePlayer, "enciende " .. genderm .. " radio.")
		else
			outputChatBox("Haz apagado tu radio.", thePlayer, 255, 194, 14)
			triggerEvent('sendAme', thePlayer, "apaga " .. genderm .. " radio.")
		end
		...
		outputChatBox("No tienes una radio", thePlayer, 255, 0, 0)
	end
end
addCommandHandler("tradio", toggleRadio, false, false)
		...
		if not (...) then
			...
			for k, arrayPlayer in ipairs(players) do
				...
				if(exports.integration:isPlayerTrialAdmin(arrayPlayer)) and (logged==1) and (hiddena ~= "true") then
					table.insert(affectedElements, arrayPlayer)
					outputChatBox("[Admin] ("..playerid..") ".. adminTitle .." " .. username .. " (".. dude .."): " .. message, arrayPlayer, 51, 255, 102)
				end
			end
		...
		if not (...) then
			...
			for k, arrayPlayer in ipairs(players) do
				...
				if (exports.integration:isPlayerLeadAdmin(arrayPlayer)) and (logged==1) then
					table.insert(affectedElements, arrayPlayer)
					outputChatBox("[LeadAdm] ("..playerid..") " ..adminTitle .. " "..accountName.. ": " .. message, arrayPlayer, 204, 102, 255)
				end
			end
...
addCommandHandler("lead", leadAdminChat, false, false)
addCommandHandler("ld", leadAdminChat, false, false)
				...
				if logged==1 and (exports.integration:isPlayerTrialAdmin(arrayPlayer) or exports.integration:isPlayerSupporter(arrayPlayer)) then
					...
					if hideg then
						...
						if string.find(string, account) then
							table.insert(affectedElements, arrayPlayer)
							triggerClientEvent ( "playNudgeSound", arrayPlayer, "Mencionado en /g ", "info")
							outputChatBox("Mencionado en el chat /g  - "..accountName..": "..message, arrayPlayer)
						end
					else
						table.insert(affectedElements, arrayPlayer)
						outputChatBox("[MOD] ("..playerid..") "..adminTitle .. " " .. accountName..": " .. message, arrayPlayer,  255, 100, 150)
					end
				end
			end
...
addCommandHandler("mod", gmChat, false, false)
...
function toggleAdminChat(thePlayer, commandName)
	...
	if logged==1 and exports.integration:isPlayerLeadAdmin(thePlayer) then
		...
		if not hidea or hidea == "false" then
			...
			outputChatBox("Chat de admin ocultado, /toga denuevo para activarlo.",thePlayer, 0,255,0)
		elseif hidea=="true" then
			setElementData(thePlayer, "hidea", "false")
			outputChatBox("Chat de admin revelado, /toga denuevo para desactivarlo.",thePlayer, 0,255,0)
		end
	end
end
...
function toggleGMChat(thePlayer, commandName)
	...
	if logged==1 and (exports["integration"]:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerSupporter(thePlayer)) then
		...
		outputChatBox("Chat de Moderador - "..(hideg and "Mostrandose" or "Oculto").." /togm para cambiarlo.",thePlayer)
	end
end
addCommandHandler("togmod", toggleGMChat, false, false)
addCommandHandler("togm", toggleGMChat, false, false)
		...
		if (commandName == "togooc") then
			if (oocEnabled==0) then
				...
				for k, arrayPlayer in ipairs(players) do
					...
					if	(logged==1) then
						outputChatBox("Chat OOC activado por un admin.", arrayPlayer, 0, 255, 0)
					end
				end
			elseif (oocEnabled==1) then
				...
				for k, arrayPlayer in ipairs(players) do
					...
					if	(logged==1) then
						outputChatBox("Chat OOC desactivado por un admin.", arrayPlayer, 255, 0, 0)
					end
				end
			end
			...
			if (oocEnabled==0) then
				...
				for k, arrayPlayer in ipairs(players) do
					...
					if	(logged==1) and (tonumber(admin)>0)then
						outputChatBox("Chat OOC activado silenciosamente por el admin " .. getPlayerName(thePlayer) .. ".", arrayPlayer, 0, 255, 0)
					end
				end
			elseif (oocEnabled==1) then
				exports.global:setOOCState(0)
				...
				for k, arrayPlayer in ipairs(players) do
					...
					if	(logged==1) and (tonumber(admin)>0)then
						outputChatBox("Chat OOC desactivado silenciosamente por el admin " .. getPlayerName(thePlayer) .. ".", arrayPlayer, 255, 0, 0)
					end
				end
			end
		end
...
function togglePM(thePlayer, commandName)
	...
	if hasPerk or (exports.integration:isPlayerTrialAdmin(thePlayer) or exports.integration:isPlayerFMTLeader(thePlayer)) then
		...
		outputChatBox("No tienes activado este Perk. Contacta con un admin, normalmente se les da a los donadores.", thePlayer)
	end
end
		...
		if not (targetPlayerNick) or not (amount) or not tonumber(amount) then
			...
			if targetPlayer then
				...
				if (distance<=10 and not getElementData(targetPlayer, "reconx")) then
					...
					if (targetPlayer==thePlayer) then
						outputChatBox("No puedes pagarte a ti mismo.", thePlayer, 255, 0, 0)
					elseif amount == 0 then
						outputChatBox("Tienes que poner una cantidad mayor a 0.", thePlayer, 255, 0, 0)
					elseif (hoursplayed<5) and (amount>150) and not exports.integration:isPlayerTrialAdmin(thePlayer) and not exports.integration:isPlayerTrialAdmin(targetPlayer) and not exports.integration:isPlayerSupporter(thePlayer) and not exports.integration:isPlayerSupporter(targetPlayer) then
						outputChatBox("No has jugado 5 horas, por lo tanto solo puedes pagar 150.", thePlayer, 255, 0, 0)
					elseif exports.global:hasMoney(thePlayer, amount) then
						if hoursplayed < 5 and not exports.integration:isPlayerTrialAdmin(targetPlayer) and not exports.integration:isPlayerTrialAdmin(thePlayer) and not exports.integration:isPlayerSupporter(targetPlayer) and not exports.integration:isPlayerSupporter(thePlayer) then
							...
							if totalAmount > 1000 then
								outputChatBox( "Tu solo puedes pagar /pay $1000 por 5 minutos. /report para pagar una cantidad mas grande.", thePlayer, 255, 0, 0 )
								return
							end
						...
						if (hoursplayed<5) then
							exports.global:sendMessageToAdmins("AdmWarn: Jugador nuevo '" .. getPlayerName(thePlayer) .. "' transfirió $" .. exports.global:formatMoney(amount) .. " a '" .. targetPlayerName .. "'.")
						end
						...
						triggerEvent('sendAme', thePlayer, "toma unos dolares de " .. genderm .. " billetera y se los da a " .. targetPlayerName .. ".")
						outputChatBox("Pagaste $" .. exports.global:formatMoney(amount) .. " a " .. targetPlayerName .. ".", thePlayer)
						...
						outputChatBox("No tienes dinero.", thePlayer, 255, 0, 0)
					end
				else
					outputChatBox("Estás muy lejos de " .. targetPlayerName .. ".", thePlayer, 255, 0, 0)
				end
			end
		end
	end
end
addCommandHandler("pagar", payPlayer, false, false)
...
addLocalizedCommandHandler("s", localWhisper, false, false)
				...
				if targetPlayer then
					...
					if (targetLogged==1) then
						if(getElementData(targetPlayer,"interview"))then
							...
							local playerName = getPlayerName(thePlayer)
							outputChatBox(playerName .." le ha ofrecido una entrevista.", targetPlayer, 0, 255, 0)
							outputChatBox("((Usa /enot para hablar en la entrevista.))", targetPlayer, 0, 255, 0)
							local NewsFaction = exports.factions:getPlayersInFaction(20)
							for key, value in ipairs(NewsFaction) do
								outputChatBox("((".. playerName .." ha invitado a " .. targetPlayerName .. " a una entrevista.))", value, 0, 255, 0)
							end
						end
					end
				end
				...
				if targetPlayer then
					...
					if (targetLogged==1) then
						if not(getElementData(targetPlayer,"interview"))then
							...
							local playerName = getPlayerName(thePlayer)
							outputChatBox(playerName .." terminó tu entrevista.", targetPlayer, 255, 0, 0)

							local NewsFaction = exports.factions:getPlayersInFaction(20)
							for key, value in ipairs(NewsFaction) do
								outputChatBox("((".. playerName .." ha terminado la entrevista de " .. targetPlayerName .. "))", value, 255, 0, 0)
							end
						end
					end
				end
...
addCommandHandler("tentrevista", endInterview, false, false)
	...
	if (logged==1) then
		if(getElementData(thePlayer, "interview"))then
			if not(...)then
				...
				local finalmessage = "[RSTV] Entrevistado " .. name .." dice: ".. message
				if exports.factions:isInFactionType(thePlayer, 6)then -- news faction
					finalmessage = "[RSTV] " .. name .." dice: ".. message
				end
...
addCommandHandler("enot", interviewChat, false, false)
	...
	if not (amount) then
		...
		if (donation<=0) then
			...
			if not exports.global:takeMoney(thePlayer, donation) then
				outputChatBox("No tienes tanto dinero para donar.", thePlayer, 255, 0, 0)
			else
				outputChatBox("Has donado $".. exports.global:formatMoney(donation) .." a caridad.", thePlayer, 0, 255, 0)
				exports.global:sendMessageToAdmins("AdmWrn: " ..getPlayerName(thePlayer).. " donó a caridad $" ..exports.global:formatMoney(donation))
				exports.logs:dbLog(thePlayer, 25, thePlayer, "CHARITY $" .. amount)
			end
		end
	end
end
addCommandHandler("dcaridad", charityCash, false, false)
			...
			for k, arrayPlayer in ipairs(players) do
				...
				if logged==1 and isPlayerStaff(arrayPlayer) then
					...
					if hideStaffChat then
						...
						if string.find(string, account) then
							table.insert(affectedElements, arrayPlayer)
							triggerClientEvent ( "playNudgeSound", arrayPlayer, "Mencionado en /st ", "info")
							outputChatBox("Mencionado en /st  - "..accountName..": "..message, arrayPlayer)
						end
					else
						table.insert(affectedElements, arrayPlayer)
...
function toggleStaffChat(thePlayer, commandName)
	...
	if logged==1 and isPlayerStaff(thePlayer) then
		...
		outputChatBox("Staff Chat - "..(hideStaffChat and "Mostrandose" or "Oculto").." /"..commandName.." para cambiar.",thePlayer)
	end
end
