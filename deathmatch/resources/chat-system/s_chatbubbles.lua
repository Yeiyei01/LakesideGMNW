...
function sendStatus(thePlayer, commandName, ...)
	...
	outputChatBox("Ha habilitado un estado permanente. Para eliminarlo escriba /status.", thePlayer, 255, 194, 14)
...
function writeChatbubbleText( thePlayer, commandName, ... )
	...
	if getElementData(thePlayer, "statusSpam") == message then
		outputChatBox("SPAM: Tu /" .. commandName .. " no se ha demostrado que evite el spam. Inténtelo de nuevo en unos segundos.", thePlayer, 255, 0, 0)
		return nil
	else
...
function sendChatBubble(source, command, message, inChatBox)
	...
	elseif command == "ame" then
		...
		outputDebugString("No se puede añadir un mensaje de estado " .. command .. "?")
		return
	end
