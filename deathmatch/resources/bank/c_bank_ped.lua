...
function bankerInteraction(ped) 
	...
	if not (wGui) then
		showCursor(true)
		--NEW CARD
		wGui = guiCreateWindow(X, Y, Width, Height, "'¿Qué puedo hacer por usted, señor?'", false )
		option[1] = guiCreateButton( 0.05, verticalPos, 0.9, 1/numberOfButtons, "Quiero solicitar una nueva tarjeta ATM, por favor.", true, wGui )
		...
		verticalPos = verticalPos + 1/numberOfButtons
		--UNLOCK CARD
		option[3] = guiCreateButton( 0.05, verticalPos, 0.9, 1/numberOfButtons, "He encontrado mi tarjeta de cajero automático\n ¿Podría volver a activarlo, por favor? ($0)", true, wGui )
		...
		verticalPos = verticalPos + 1/numberOfButtons
		--RECOVER CARD
		option[4] = guiCreateButton( 0.05, verticalPos, 0.9, 1/numberOfButtons, "Necesito recuperar mi tarjeta ATM\nY si también pudieras decirme el código PIN, sería genial ($50)", true, wGui )
		...
		verticalPos = verticalPos + 1/numberOfButtons
		--DELETE CARD
		option[5] = guiCreateButton( 0.05, verticalPos, 0.9, 1/numberOfButtons, "Quiero cancelar mi tarjeta ATM\nPorque ya no la uso ($0)", true, wGui )
