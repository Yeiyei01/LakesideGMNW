...
function jobAdverts()
	...
	for _, ourPlayer in pairs(exports.pool:getPoolElementsByType( 'player' )) do
		if (getElementData(ourPlayer, 'loggedin') == 1) then
			if taxicount > 0 then
				text = "ANUNCIO: Yellow Cab Co está aceptando tarifas. ¡Llámenos hoy mismo! - #8294"
				if exports.integration:isPlayerTrialAdmin(ourPlayer) then 
					text = text .. " (( Anuncio Empleo. ))"
				end

				outputChatBox(text, ourPlayer, 0, 255, 0)
			end
			if buscount > 0 then
				text = "ANUNCIO: ¡Riverside Bus ya tiene rutas! Si necesitas que te lleven, ¡Espera en una de nuestras paradas!"
				if exports.integration:isPlayerTrialAdmin(ourPlayer) then 
					text = text .. " (( Anuncio Empleo. ))"
				end
