...
local JOB_ID = 1
local JOB_DATA_NAME = "char >> job"

setElementData(localPlayer, JOB_DATA_NAME, JOB_ID)
...
local selectedHotdog = {} -- {Kifli, Kolbász}
local selectedDrink = {} -- {Pohár, Ital}
...
addEventHandler("onClientResourceStart", resourceRoot, function()
    ...
    fonts.Roboto11 = dxCreateFont("hot-dog/files/Roboto-Regular.ttf", respc(11))

    if isElement(fonts.Roboto13) then
        destroyElement(fonts.Roboto13)
        fonts.Roboto13 = nil
    end
    fonts.Roboto13 = dxCreateFont("hot-dog/files/Roboto-Regular.ttf", respc(13))

    if isElement(fonts.Roboto14) then
        destroyElement(fonts.Roboto14)
        fonts.Roboto14 = nil
    end
    fonts.Roboto14 = dxCreateFont("hot-dog/files/Roboto-Regular.ttf", respc(14))

    if isElement(fonts.RobotoB15) then
        destroyElement(fonts.RobotoB15)
        fonts.RobotoB15 = nil
    end
    fonts.RobotoB15 = dxCreateFont("hot-dog/files/Roboto-Bold.ttf", respc(15))

    if isElement(fonts.lunabar) then
        destroyElement(fonts.lunabar)
        fonts.lunabar = nil
    end
    fonts.lunabar = dxCreateFont("hot-dog/files/lunabar.ttf", respc(30), false, "antialiased")

    if getElementData(localPlayer, JOB_DATA_NAME) == JOB_ID then
        createStallBlips()
        jobInfo()
    end
end)
...
addEventHandler("onClientElementDataChange", root, function(dataName, oldValue, newValue)
    if getElementType(source) == "player" and source == localPlayer then
        if dataName == JOB_DATA_NAME then 
...
function createStallBlips()
    for k, v in pairs(hotdogStalls) do
        ...
        setElementData(stallBlips[k], "blip >> name", "Sosisli Tezgahı")    
    end
end
        ...
        if jobActive then
                ...
                if clickedElement and clickedElement == currentBuyer then
                    if #selectedHotdog == 2 and #selectedDrink == 2 then
                        ...
                        if currentOrder.hotdog[1] ~= selectedHotdog[1] then 
                            print("This isn't the right hot dog bun the customer asked for")
                            showInfobox("error", "¡Este no es el pedido correcto!")
                            return
                        end

                        if currentOrder.hotdog[2] ~= selectedHotdog[2] then
                            print("The customer did not ask for such sausage") 
                            showInfobox("error", "¡El cliente no quería la salchicha así!")
                            return
                        end
                        
                        if progress < 1 then 
                            print("You did not fill the cup fully") 
                            showInfobox("error", "¡No llenó completamente el vaso!")
                            return
                        end

                        if currentOrder.drink[1] ~= selectedDrink[1] then 
                            print("The customer did not ask for such cup") 
                            showInfobox("error", "¡El cliente no quería este vaso!")
                            return
                        end

                        if currentOrder.drink[2] ~= selectedDrink[2] then 
                            print("The customer did not ask for such drink") 
                            showInfobox("error", "¡El cliente no quería esta bebida!")
                            return
                        end
                        ...
                        print("Pedido completado con éxito, entregado. Has ganado " .. totalIncome .. "$")
                        showInfobox("info", "¡Pedido completado con éxito, entregado! Has ganado. " .. totalIncome .. "$")

                        --GiveMoney
                        triggerServerEvent("giveHotdogMoney", localPlayer, totalIncome)
                        ...
                        print("Your order is not completed!")
                        showInfobox("error", "¡Tu pedido no está completo!")
                    end
                end
            end
        else
            if clickedElement and getElementModel(clickedElement) == 1340 and getElementData(clickedElement, "stall.id") then
                ...
                if getDistanceBetweenPoints3D(oX, oY, oZ, pX, pY, pZ) <= 5 then
                    ...
                    if getElementData(localPlayer, JOB_DATA_NAME) ~= JOB_ID then
                        showInfobox("error", "No eres un vendedor de perritos calientes")
                        return
                    end

                    if isElement(getElementData(clickedElement, "stall.inUse")) then
                        showInfobox("error", "¡Este puesto está ocupado! ¡Encuentra otro!")
                    else    
                        if not isPedInVehicle(localPlayer) then
                            showInfobox("info", "Aquí hay un nuevo cliente.")
...
function renderHotdogStall()
    ...
    if currentPage == "hotdogs" then
        ...
        for k, v in pairs(hotdogs) do
            local crescentSize = resp(256 * 0.60) -- kifli kép mérete
...
function drawHotdog(typ, x, y, sausage, sizeMultipler)
	...
	local crescentSize = resp(256 * sizeMultipler) -- kifli kép mérete
	local sausageSize = resp(264 * sizeMultipler) -- kolbász kép mérete

	dxDrawImage(x, y, crescentSize, crescentSize, "hot-dog/files/" .. hotdogs[typ].images[1]) -- kifli hátsó része

	if sausages[sausage] then -- kolbász
		dxDrawImage(x + (crescentSize - sausageSize) / 2, y + (crescentSize - sausageSize) / 2 - resp(15), sausageSize, sausageSize, "hot-dog/files/" .. sausages[sausage].image)
	end

	dxDrawImage(x + resp(5), y + resp(10), crescentSize, crescentSize, "hot-dog/files/" .. hotdogs[typ].images[2]) -- kifli elülső része
end
...
function drawCup(typ, x, y, liquid, sizeMultipler)
	...
	local imageSize = resp(256 * sizeMultipler)
	dxDrawImage(x, y, imageSize, imageSize, "hot-dog/files/" .. cups[typ].images[1]) -- pohár külseje

	if drinks[liquid] then
		dxDrawImageSection(x, y + imageSize, imageSize, -imageSize * progress, 0, 0, imageSize / responsiveMultipler, -imageSize / responsiveMultipler * progress, "hot-dog/files/cup_" .. cups[typ].mark .. "_liquid.png", 0, 0, 0, tocolor(unpack(drinks[liquid][2])))
	end

	dxDrawImage(x, y, imageSize, imageSize, "hot-dog/files/" .. cups[typ].images[2]) -- üveg effekt
end
...
function drawOrder(x, y, data, center)
    ...
    dxDrawImage(x, y, orderW, orderH, "hot-dog/files/order.png")
    dxDrawText("ORDEN", x, y + respc(43), x + orderW, y + orderH, tocolor(0, 0, 0), 1, fonts.RobotoB15, "center")

    local textY = y + respc(90)
    
    if data.hotdog then
        dxDrawText("Pan:", x + respc(10), textY, x, y, tocolor(50, 50, 50, 180), 1, fonts.Roboto14)
        textY = textY + respc(31)
        dxDrawText("- " .. hotdogs[data.hotdog[1]].name .. " pan", x + respc(10), textY, x, y, tocolor(50, 50, 50, 180), 1, fonts.Roboto14)
        textY = textY + respc(31)
    ...
    if data.drink then
        textY = textY + respc(31)
        dxDrawText("Bebida:", x + respc(10), textY, x, y, tocolor(50, 50, 50, 180), 1, fonts.Roboto14)
        textY = textY + respc(31)
        dxDrawText("- " .. cups[data.drink[1]].name .. " bebida", x + respc(10), textY, x, y, tocolor(50, 50, 50, 180), 1, fonts.Roboto14)
        textY = textY + respc(31)
        dxDrawText("- " .. drinks[data.drink[2]][1] .. "", x + respc(10), textY, x, y, tocolor(50, 50, 50, 180), 1, fonts.Roboto14)
    end
end
