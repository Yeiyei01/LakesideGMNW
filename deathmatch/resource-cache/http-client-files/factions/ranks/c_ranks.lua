...
function btEditRanks(button, state)
	if (source==gButtonEditRanks) and (button=="left") and (state=="up") then
		...
		wRanks = guiCreateWindow(sX, sY, wX, wY, "Gestión de Cargos", false)
		guiWindowSetSizable(wRanks, false)
		guiSetAlpha(wRanks, 1.0)
		-- Labels
		ranksLabel1 = guiCreateLabel(9, 27, 172, 15, "Cargos", false, wRanks)
		...
		ranksLabel2 = guiCreateLabel(288, 27, 142, 15, "Permisos", false, wRanks)
		...
		ranksLabel3 = guiCreateLabel(193, 48, 335, 32, "Los permisos son a los que tiene acceso este rango. Para cambiar el permiso, simplemente marque/desmarque la casilla.", false, wRanks)
		guiLabelSetHorizontalAlign(ranksLabel3, "center", true)
		-- Gridlist
		ranksGridlist = guiCreateGridList(9, 76, 175, 211, false, wRanks)
		guiGridListAddColumn(ranksGridlist, "Cargos", 0.9)
		guiGridListSetSortingEnabled(ranksGridlist, false)
		-- Buttons
		btRanksClose = guiCreateButton(489, 28, 38, 15, "Cerrar", false, wRanks)
		btRanksMvUp = guiCreateButton(9, 292, 85, 22, "Subirlo", false, wRanks)
		btRanksMvDn = guiCreateButton(99, 292, 85, 22, "Bajarlo", false, wRanks)
		btRanksUpdOrd = guiCreateButton(9, 48, 175, 22, "Actualizar Orden", false, wRanks)
		btRanksAdd = guiCreateButton(9, 320, 55, 22, "Añadir", false, wRanks)
		btRanksRem = guiCreateButton(129, 320, 55, 22, "Eliminar", false, wRanks)
		btRanksRen = guiCreateButton(69, 320, 55, 22, "Renombrar", false, wRanks)
		btRanksApply = guiCreateButton(284, 313, 158, 30, "Actualizar Ajustes", false, wRanks)
...
function saveRanks(button, state)
	if (source==bRanksSave) and (button=="left") and (state=="up") then
		...
		if (found) then
			outputChatBox("Sus rangos contienen caracteres no válidos, por favor asegúrese de que no contiene caracteres como '@;", 255, 0, 0)
		elseif not (isNumber) then
			outputChatBox("Sus salarios no son números, por favor, asegúrese de que ha introducido un número y ningún símbolo de moneda.", 255, 0, 0)
		else
			local sendRanks = { }
			local sendWages = { }
...
function btAddRanks()
	...
	wAddRank = guiCreateWindow(sX, sY, wX, wY, "Añadir Cargo...", false)
	guiWindowSetSizable(wAddRank, false)
	guiSetAlpha(wAddRank, 1.00)
	-- Labels
	lAddRank1 = guiCreateLabel(9, 74, 223, 15, "Nombre", false, wAddRank)
	guiLabelSetHorizontalAlign(lAddRank1, "center", false)
	lAddRank2 = guiCreateLabel(9, 26, 224, 15, "Permisos...", false, wAddRank)
	...
	-- Buttons
	bAddRankCreate = guiCreateButton(250, 35, 85, 28, "Añadir", false, wAddRank)
	guiSetFont(bAddRankCreate, "default-bold-small")
	guiSetProperty(bAddRankCreate, "NormalTextColour", "FFAAAAAA")
	bAddRankClose = guiCreateButton(250, 78, 85, 28, "Cancelar", false, wAddRank)
...
function addRank()
	...
	if (rank == "") then
		outputChatBox("Introduzca un nombre al cargo.", 255, 100, 100)
		return
	end
...
function btRenameRank()
	...
	wRenameRank = guiCreateWindow(sX, sY, wX, wY, "Renombrar Cargo", false)
	guiWindowSetSizable(wRenameRank, false)
	-- Label
	lRenameRank1 = guiCreateLabel(-18, 28, 277, 15, "Cambiar '"..currentRank.."' a:", false, wRenameRank)
	...
	-- Button
	btRenameRankChange = guiCreateButton(53, 77, 63, 26, "Cambiar", false, wRenameRank)
...
function btRemoveRank()
	...
	wRemoveRank = guiCreateWindow(660, 366, 285, 138, "Eliminar Cargo", false)
	guiWindowSetSizable(wRemoveRank, false)
	-- Labels
	local row = guiGridListGetSelectedItem(ranksGridlist)
	local rankName = guiGridListGetItemText(ranksGridlist, row, 1)
	lRemoveRank1 = guiCreateLabel(26, 24, 225, 30, "¿Esta seguro de eliminar \n'"..rankName.."'?", false, wRemoveRank)
	...
	lRemoveRank2 = guiCreateLabel(10, 58, 257, 31, "Todos los miembros con este rango recibirán \n'"..def_rank.."'.", false, wRemoveRank)
	guiLabelSetHorizontalAlign(lRemoveRank2, "center", false)
	-- Buttons
	btRemoveRankDel = guiCreateButton(68, 96, 67, 29, "Eliminar", false, wRemoveRank)
...
function setRankInfo(rankTbl, rankperms, perms, wageTable)
	...
	if not isElement(ranksLabel5) then
		ranksLabel5 = guiCreateLabel(0, 0, 337, 15, "Seleccione un rango para personalizar sus permisos", false, ranksScrollPane)
	...
	for i,perm in ipairs(perms) do
		...
		if #perms == i  then
			ranks_gui.label[1] = guiCreateLabel(0, (i*25+25), 39, 15, "Salario:", false, ranksScrollPane)
...
function selectRankOffList()
	...
	guiSetText(ranksLabel5, "Permisos de edición de '"..ranks[row+1][2].."'")
end
...
function applyPermissions()
	...
	if (not row or row == -1) then 
		outputChatBox("Seleccione primero un rango.", 255, 100, 100)
	return end
	local rankID = ranks[row+1][1]
...
function renameRank()
	...
	if (rankName == "") then
		outputChatBox("Introduzca un nombre de rango.", 255, 100, 100)
		return
	end
