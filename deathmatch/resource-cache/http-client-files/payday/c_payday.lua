function cPayDay(faction, pay, profit, interest, donatormoney, tax, incomeTax, vtax, ptax, rent, totalInsFee, grossincome, Perc)
	...
	if not (faction) then
		if (pay + tax > 0) then
			...
			table.insert(info, {"  Beneficios del Estado: $" .. exports.global:formatMoney(pay+tax)})	
		end
	else
		if (pay + tax > 0) then
			--outputChatBox(, 255, 194, 14, true)
			table.insert(info, {"  Salario: $" .. exports.global:formatMoney(pay+tax)})
		end
	end
	...
	if (profit > 0) then
		--outputChatBox(, 255, 194, 14, true)
		table.insert(info, {"  Beneficios de negocio: $" .. exports.global:formatMoney(profit)})
	end
	
	-- bank interest
	if (interest > 0) then
		--outputChatBox(,255, 194, 14, true)
		table.insert(info, {"  Intereses del banco: $" .. exports.global:formatMoney(interest) .. " (≈" ..("%.2f"):format(Perc) .. "%)"})
	end
	
	-- donator money (nonRP)
	if (donatormoney > 0) then
		--outputChatBox(, 255, 194, 14, true)
		table.insert(info, {"  Dinero de donador: $" .. exports.global:formatMoney(donatormoney)})
	end
	...
	if (tax > 0) then
		--outputChatBox(, 255, 194, 14, true)
		table.insert(info, {"  Impuesto " .. (math.ceil(incomeTax*100)) .. "%: $" .. exports.global:formatMoney(tax)})
	end
	
	if (vtax > 0) then
		--outputChatBox(, 255, 194, 14, true)
		table.insert(info, {"  Impuesto de vehículo: $" .. exports.global:formatMoney(vtax)})
	end
	...
	if (ptax > 0) then
		--outputChatBox(, 255, 194, 14, true )
		table.insert(info, {"  Expensas de propiedad: $" .. exports.global:formatMoney(ptax)})
	end
	
	if (rent > 0) then
		--outputChatBox(, 255, 194, 14, true)
		table.insert(info, {"  Renta de apartamento: $" .. exports.global:formatMoney(rent)})
	end
	...
	elseif (grossincome > 0) then
		...
		table.insert(info, {"  Reajuste(s): Transferido a tu cuenta bancaria."})
	else
		--outputChatBox(, 255, 194, 14, true)
		--outputChatBox(, 255, 194, 14)
		table.insert(info, {"  Ingresos brutos: $" .. exports.global:formatMoney(grossincome)})
		table.insert(info, {"  Reajuste(s): Tomado de tu cuenta bancaria."})
	end
