...
function dispatchGUI()
	...
	if count > 1 then
		outputChatBox("Sólo puedes estar en una facción de la ley a la vez para usar /despacho.", 255, 0, 0)
		return 
	end
	...
	DispatchUI.window[1] = guiCreateWindow(x, y, width, height, "Dispositivo Despacho a Distancia", false)
	...
	guiGridListAddColumn(DispatchUI.gridlist[1], "Indicativo", 0.3)
	guiGridListAddColumn(DispatchUI.gridlist[1], "Nombre", 0.3)
	...
	DispatchUI.label[2] = guiCreateLabel(11, 234, 47, 19, "Indicativo:", false, DispatchUI.window[1])
	...
	guiComboBoxAddItem(DispatchUI.combobox[1], "Fuera de Servicio")
	...
	addEventHandler("onClientGUIClick", DispatchUI.button[2], function ()
			if cooldownDispatch then outputChatBox("Por favor, espere.", 255, 0 ,0) return end
...
function canUseDispatch()
    if exports.global:hasItem(localPlayer, 177) and (exports.factions:isPlayerInFaction(localPlayer, 1) or exports.factions:isPlayerInFaction(localPlayer, 50)) then
		...
		outputChatBox("Despacho en el aire no parece funcionar bien...", 255, 0, 0)
	end
end
addCommandHandler("despacho", canUseDispatch)
	...
	if getElementData(getLocalPlayer(), "dispatch:onDuty") then
	    ...
	    --[[for k, vehicle in ipairs( getElementsByType("vehicle") ) do
	    	if isSameFaction(vehicle) and not dispatchBlips[vehicle] and tablelength(getVehicleOccupants(vehicle) or {}) > 0 and allowedFactions[tonumber(getElementData(vehicle, "faction"))] then
	    		dispatchBlips[vehicle] = exports.customblips:createCustomBlip ( 0, 0, 8, 8, "vehicle.png", 999999, true, vehicle, "El Vehículo Contiene al Sospechoso" )
        			...
        			if veh then
        				...
        				if isSameFaction(veh) and t > 1 then
							...
							if not found then
								...
								--[[dispatchBlips[veh] = exports.customblips:createCustomBlip ( 0, 0, 12, 12, "vehicle.png", 99999, true, veh, "El Vehículo Contiene al Sospechoso" )
								if getElementData(veh, "lspd:siren") then -- Begin flash
			           		    	exports.customblips:setBlipImageColor(dispatchBlips[veh], 255, 0, 0)
			           			end]]
							elseif found then
		...
		elseif dispatchBlips[source] and t ~= 0 then
			...
			if not found then
				...
				--[[if isSameFaction(source) and getElementData(getLocalPlayer(), "dispatch:onDuty") then
					dispatchBlips[source] = exports.customblips:createCustomBlip ( 0, 0, 12, 12, "vehicle.png", 99999, true, source, "El Vehículo Contiene al Sospechoso" )
