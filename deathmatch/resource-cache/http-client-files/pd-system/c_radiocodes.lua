...
function displayPdCodes(contentFromServer)
	...
	myWindow = guiCreateWindow( 0.25, 0.25, 0.5, 0.5, "Gobierno de Riverside - Códigos Radio y Procedimientos - V2.0",true)
	local tabPanel = guiCreateTabPanel ( 0, 0.12, 1, 1, true, myWindow ) 
	
	--Response & Ten Codes
	local tabCodes = guiCreateTab( "Códigos 10", tabPanel )
	
	memoCodes = guiCreateMemo ( 0.02, 0.02, 0.96, 0.96, content.codes or "Está solo aquí o el contenido está desactualizado. \n\nPor favor, actualice..", true, tabCodes )
	guiMemoSetReadOnly(memoCodes, true)
	
	--Radio Procedures
	local tabProcedure = guiCreateTab( "Procedimientos Radio", tabPanel )
	memoProcedure = guiCreateMemo ( 0.02, 0.02, 0.96, 0.96, content.procedures or "Está solo aquí o el contenido está desactualizado. \n\nPor favor, actualice..", true, tabProcedure )
	...
	local tlRefreshButton = guiCreateButton(0.65, 0.05, 0.1, 0.07, "Actualizar", true, myWindow) 
...
addCommandHandler("codigossd", displayPdCodes)
