...
function start_GUI(thePlayer, commandName)
	if exports.integration:isPlayerLeadAdmin(getLocalPlayer()) or exports.integration:isPlayerScripter(getLocalPlayer()) then
		...
		GUIEditor.window[1] = guiCreateWindow(659, 293, 254, 309, "Gestion Perros K9", false)
		...
		GUIEditor.column[1] = guiGridListAddColumn(GUIEditor.gridlist[1], "Nombre", 0.5)
		...
		GUIEditor.button[1] = guiCreateButton(0.36, 0.82, 0.29, 0.13, "Revocar", true, GUIEditor.window[1])
		addEventHandler("onClientGUIClick", GUIEditor.button[1], function ()
				...
				if rindex ~= -1 then
					...
					outputChatBox("Debe seleccionar una persona para revocar.")
				end
			end, false)

		GUIEditor.button[2] = guiCreateButton(0.67, 0.82, 0.29, 0.13, "Cerrar", true, GUIEditor.window[1])
...
addCommandHandler("dogadd", function (commandName, character, attack)
	if exports.integration:isPlayerLeadAdmin(getLocalPlayer()) or exports.integration:isPlayerScripter(getLocalPlayer()) then
		if character and tonumber(attack) then
			...
			outputChatBox("¡El usuario ha sido añadido! "..character.." - Attack: "..attack)
		else
			outputChatBox("SYNTAX: /"..commandName.." [Nombre] [attack: 1 o 0]")
		end
	end
end)