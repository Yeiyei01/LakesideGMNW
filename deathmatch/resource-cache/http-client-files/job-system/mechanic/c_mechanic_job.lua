	...
	if factionType == 7 then -- Mechanic Faction / Adams]]
		if not vehicle then
			...
			if not (wMechanic) then
				-- Create the window
				wMechanic = guiCreateWindow(X, Y, Width, Height, "Opciones Mecánicas", false )
				...
				if not isPedInVehicle( localPlayer ) and getPedSimplestTask(localPlayer) ~= "TASK_SIMPLE_CAR_DRIVE" then
					...
					if getElementHealth(vehicle) > 950 then
						bMechanicTwo = guiCreateButton( 0.05, y, 0.9, 0.1, "Reparar - $110", true, wMechanic )
					...
					if not noTyres[getVehicleType(vehicle)] then
						bMechanicThree = guiCreateButton( 0.05, y, 0.9, 0.1, "Cambiar rueda - $10", true, wMechanic )
					...
					if faction[4] or ( tonumber( getElementData(vehicle, "job") or 0 ) == 0 and ( getElementData(vehicle, "faction") == -1 or faction[getElementData(vehicle, "faction")] ) ) then
						bMechanicFour = guiCreateButton( 0.05, y, 0.9, 0.1, "Repintar vehiculo - $100", true, wMechanic )
					...
					if vehicleWithPaintjob[getElementModel(vehicle)] then
						bMechanicSix = guiCreateButton( 0.05, y, 0.9, 0.1, "Vinilo - $7,500", true, wMechanic )
					...
					-- Replace lights
					--if vehicleWithPaintjob[getElementModel(vehicle)] then
						bMechanicNine = guiCreateButton( 0.05, y, 0.9, 0.1, "Reemplazar luces - $1,250", true, wMechanic )
					...
					if getVehicleUpgradeOnSlot(vehicle, 8) ~= 0 and faction[4] then
						bMechanicSeven = guiCreateButton( 0.05, y, 0.9, 0.1, "Remover NOS", true, wMechanic )
				...
				-- Add/Remove Tint
				bMechanicEight = guiCreateButton( 0.05, y, 0.9, 0.1, "Adjustar Tinte", true, wMechanic )
				y = y + 0.1
				...
				-- Close
				bMechanicClose = guiCreateButton( 0.05, 0.85, 0.9, 0.1, "Cerrar", true, wMechanic )
...
function tyreWindow()
	...
	if not (wTyre) then
		-- Create the window
		wTyre = guiCreateWindow(X+100, Y, Width, Height, "Selecciona una rueda para cambiar.", false )

		if getVehicleType(currentVehicle) ~= "Bike" then
			-- Front left
			bTyreOne = guiCreateButton( 0.05, 0.1, 0.45, 0.35, "Frente izquierda", true, wTyre )
			...
			-- Back left
			bTyreTwo = guiCreateButton( 0.05, 0.5, 0.45, 0.35, "Atras izquierda", true, wTyre )
			...
			-- front right
			bTyreThree = guiCreateButton( 0.5, 0.1, 0.45, 0.35, "Frente derecha", true, wTyre )
			...
			-- back right
			bTyreFour = guiCreateButton( 0.5, 0.5, 0.45, 0.35, "Atras derecha", true, wTyre )
			...
			-- Front
			bTyreOne = guiCreateButton( 0.05, 0.1, 0.9, 0.35, "Frente", true, wTyre )
			addEventHandler( "onClientGUIClick", bTyreOne, function(button, state)
				if(button == "left" and state == "up") then
			...
			-- back right
			bTyreThree = guiCreateButton( 0.05, 0.5, 0.9, 0.35, "Atras", true, wTyre )
			addEventHandler( "onClientGUIClick", bTyreThree, function(button, state)
				if(button == "left" and state == "up") then
		...
		-- Close
		bTyreClose = guiCreateButton( 0.05, 0.9, 0.9, 0.1, "Cerrar", true, wTyre )
