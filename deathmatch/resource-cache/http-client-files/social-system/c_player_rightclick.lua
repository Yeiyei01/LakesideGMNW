...
function showPlayerSelfMenu()
	...
	if getElementData(getLocalPlayer(), "realism:stretcher:hasStretcher") then
		...
		row.stretcher = exports['rightclick']:addRow("Dejar camilla")
...
function showPlayerMenu(targetPlayer, friend)
	...
	if (blindfold) and (blindfold == 1) then
		bBlindfold = exports['rightclick']:addRow("Remover Cegador")
		addEventHandler("onClientGUIClick", bBlindfold, cremoveBlindfold, false)
	else
		bBlindfold = exports['rightclick']:addRow("Cegar")
	...
	if button == "left" and state == "up" then
		if (exports.global:hasItem(getLocalPlayer(), 70)) then -- Has First Aid Kit?
			...
			if not knockedout then
				outputChatBox("Este jugador no esta inconsciente.", 255, 0, 0)
				hidePlayerMenu()
			else
			...
			outputChatBox("No tienes un kit de primeros auxilios.", 255, 0, 0)
		end
	end
end
	...
	if (button=="left") then
		if (exports.global:hasItem(getLocalPlayer(), 66)) then -- Has blindfold?
			...
			if (blindfolded==1) then
				outputChatBox("Este jugador ya esta cegado.", 255, 0, 0)
				hidePlayerMenu()
			elseif (restrained==0) then
				outputChatBox("Este jugador debe de estar esposado para cegarlo.", 255, 0, 0)
				hidePlayerMenu()
			else
			...
			outputChatBox("No tienes con que cegar.", 255, 0, 0)
		end
	end
end
...
function cremoveBlindfold(button, state, x, y)
	if (button=="left") then
		...
		if (blindfolded==1) then
			...
			outputChatBox("Este jugador no esta cegado.", 255, 0, 0)
			hidePlayerMenu()
		end
	end
end
	...
	if (button=="left") then
		if (exports.global:hasItem(getLocalPlayer(), 45) or exports.global:hasItem(getLocalPlayer(), 46)) then
			...
			if (restrained==1) then
				outputChatBox("Este jugador ya esta esposado.", 255, 0, 0)
				hidePlayerMenu()
			else
				local restrainedObj
			...
			outputChatBox("No tienes esposas.", 255, 0, 0)
			hidePlayerMenu()
		end
	end
end
...
function cunrestrainPlayer(button, state, x, y)
	if (button=="left") then
		...
		if (restrained==0) then
			outputChatBox("Este jugador no esta esposado.", 255, 0, 0)
			hidePlayerMenu()
		else
			...
			if (exports.global:hasItem(getLocalPlayer(), 47, dbid)) or (restrainedObj==46) then -- has the keys, or its a rope
				...
				outputChatBox("No tienes las llaves para esas esposas.", 255, 0, 0)
			end
		end
	end
end
...
gx, gy, bFriskTakeItem, bFriskClose, gFriskItems, FriskColName = nil
function cfriskPlayer(button, state, x, y)
	if (button=="left") then
		...
		if restrained ~= 0 and not injured then
			outputChatBox("Este jugador no esta esposado o herido, esposalo para revisarlo.", 255, 0, 0)
			hidePlayerMenu()
		--[[elseif getElementHealth(getLocalPlayer()) < 50 then
			outputChatBox("Necesitas almenos que tenga la mitad de la vida para cachearlo.", 255, 0, 0)
			hidePlayerMenu()]]--
		else
			gx = x
			gy = y
...
function friskShowItems(items)
	...
	triggerServerEvent("sendLocalMeAction", getLocalPlayer(), getLocalPlayer(), " Cachea a " .. playerName .. ".")
	--local width, height = 300, 200
	
	--wFriskItems = guiCreateWindow(gx, gy, width, height, "Cachear: " .. playerName, false)
	--guiSetText(wFriskItems, "Cachear: " .. playerName)
	--guiWindowSetSizable(wFriskItems, false)
	
	--gFriskItems = guiCreateGridList(0.05, 0.1, 0.9, 0.7, true)
	--FriskColName = guiGridListAddColumn(gFriskItems, "Nombre", 0.9)
	...
	bFriskClose = guiCreateButton(0.05, 0.85, 0.9, 0.1, "Cerrar", true)
