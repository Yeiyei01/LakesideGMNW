...
function formatTimeInterval( seconds )
	...
	if seconds < 60 then
		return formatTimeString( seconds, "segundo" )
	end
	
	local minutes = math.floor( seconds / 60 )
	if minutes < 60 then
		return formatTimeString( minutes, "minuto" ) .. " y " .. formatTimeString( seconds - minutes * 60, "segundo" )
	end
	
	local hours = math.floor( minutes / 60 )
	if hours < 48 then
		return formatTimeString( hours, "hora" ) .. " y " .. formatTimeString( minutes - hours * 60, "minuto" )
	end
	
	local days = math.floor( hours / 24 )
	return formatTimeString( days, "día" )
end
