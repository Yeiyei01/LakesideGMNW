...
function getItemName(id, value, metadata)
	...
	elseif (id == 150) and value then --ATM card
		local itemExploded = explode(";", value )
		local text = "Tarjeta de crédito ATM"
...
function getItemValue(id, value, metadata)
	...
	elseif id == 214 then
		return ""
	elseif id == 223 then
		return "Capacidad: "..tostring(split(value, ":")[3]).."kg"
...
function getItemDescription(id, value, metadata)
	...
	if i then
		...
		elseif id == 115 then
			...
			if exports.weapon:isWeapAmmoless( tonumber(values[1]) ) then
				...
				if values[4] and tonumber(values[4]) then
					return values[4]..' municiones cargadas.'
				else
					return '0 municiones cargadas.'
				end
			end
		-- ammopack
		elseif id == 116 then
		...
		elseif id == 150 then--ATM card
			...
			if itemExploded and itemExploded[2] and tonumber(itemExploded[2]) then
				if tonumber( itemExploded[2] ) > 0 then
					return "Número de tarjeta: '"..itemExploded[1].."', Dueño: '"..tostring(exports.cache:getCharacterNameFromID(itemExploded[2])):gsub("_", " ").."'"
				else
					return "Número de tarjeta: '"..itemExploded[1].."', Dueño: '"..tostring(exports.cache:getBusinessNameFromID(math.abs(itemExploded[2]))).."'"
				end
			else
				return "La tarjeta parece estar rota y ya no es utilizable."
			end
		elseif id == 152 then--ID card
			local itemExploded = explode(";", value)
			return "Una elegante tarjeta de identificación de plástico tiene el nombre de "..tostring(itemExploded[1]):gsub("_", " ")..", foto y otra información básica."
		elseif id == 178 then
			local yup = split(value, ":")
            return yup[1] .." por ".. yup[2]
		...
		elseif id == 273 then --Generic fish
			local FishValue = explode(":", value)
			return "Pesa nada menos que: " .. FishValue[2] .. "lbs"
		else
			return desc:gsub("#v",value)
		end
	end
...
splittableItems = {
	[30]=" gramo(s)", [31]=" gramo(s)", [32]=" gramo(s)", [33]=" gramo(s)", [34]=" gramo(s)", [35]=" ml(s)", [36]=" tableta(s)",
	[37]=" gramo(s)", [38]=" gramo(s)", [39]=" gramo(s)", [40]=" ml(s)", [41]=" fichas(s)", [42]=" seta(s)", [43]=" tableta(s)", [134] = " Dinero",
	[147]="cuadro", [115]="armas", [116] = "cargadores"
 }
