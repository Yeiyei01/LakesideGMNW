...
function showChemistrySet()
	if not (wItems) then
		if not (wChemistrySet) then
			...
			wChemistrySet = guiCreateWindow(x, y, width, height, "Kit de Química", false)
			...
			colChemSlot = guiGridListAddColumn(gChemicals, "Espacio", 0.1)
			colChemName = guiGridListAddColumn(gChemicals, "Nombre", 0.855)
			...
			-- buttons
			bMixItems = guiCreateButton(0.05, 0.91, 0.7, 0.15, "Mezcla seleccionada", true, wChemistrySet)
			addEventHandler("onClientGUIClick", bMixItems, mixItems, false)
			guiSetEnabled(bMixItems, false)
			
			bChemClose = guiCreateButton(0.8, 0.91, 0.15, 0.15, "Cerrar", true, wChemistrySet)
...
local drugMessages = { "Vete a la mierda amigo", "¡Mira amigo! Estas vooolaaaandooo", "Necesitamos mas drogas amigo", "¿Quieres bailar hijo de puta?", "Chico, necesito mas de esta mierda. ¿Qué es?", "Estoy llamando a los policías." }
	...
	for key, value in ipairs(players) do
		if (value~=localPlayer) then
			...
			if (getDistanceBetweenPoints3D(x, y, z, px, py, pz)<15) then
				...
				outputChatBox(charname .. " dice: " .. drugMessages[rand2], 255, 255, 255)
				break
			end
		end
	end
