...
function openMetadataEditor(item)
    ...
    elseif not canOpenMetadataEditor(localPlayer, item) then
        ...
        wEditor = guiCreateWindow(0, 0, main_width, main_height, "Propiedades del Item", false)
        guiWindowSetSizable(wEditor, false)
        centerUnderCursor(wEditor)

        local bClose = guiCreateButton(10, main_height - 30, main_width, 25, "Cerrar", false, wEditor)
        ...
        local lValue = guiCreateLabel(8, 37, main_width - 10, 13, "Valor del Item: " .. getItemValue(item[1], item[2]), false, wEditor)
        ...
        cKey = guiGridListAddColumn(grid, "Llave", 0.3)
        cValue = guiGridListAddColumn(grid, "Valor", 0.63)
...
function showEditingWindow()
    ...
    if row == -1 then
        ...
        if metadata_item.type == 'string' or metadata_item.type == 'integer' then
            ...
            wEditing = guiCreateWindow(0, 0, width, height, "Edición de la propiedad del artículo", false)
            ...
            local bClose = guiCreateButton(10, 60, width / 2 - 15, 25, "Cancelar", false, wEditing)
            addEventHandler("onClientGUIClick", bClose, closeEditingWindow, false)

            local bSave = guiCreateButton(width / 2, 60, width / 2 - 10, 25, "Guardar", false, wEditing)
            ...
            local commitChanges = function()
                ...
                outputChatBox("No se puede guardar, el artículo ya no está en el inventario.", 255, 0, 0)
            end
            ...
            outputChatBox("Edicion de " .. tostring(metadata_item.name) .. " aún no se admite.", 255, 0, 0)
        end
    end
    -- editing_item
end
