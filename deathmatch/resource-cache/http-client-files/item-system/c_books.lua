...
function createBook( bookName, bookTitle )
	...
	if not (wBook) then
		...
		buttonPrev = guiCreateButton( 0.85, 0.25, 0.14, 0.05, "Atras", true, wBook)
		addEventHandler( "onClientGUIClick", buttonPrev, prevButtonClick, false )
		guiSetVisible(buttonPrev, false)

		buttonClose = guiCreateButton( 0.85, 0.45, 0.14, 0.05, "Cerrar", true, wBook)
		addEventHandler( "onClientGUIClick", buttonClose, closeButtonClick, false )
		
		buttonNext = guiCreateButton( 0.85, 0.65, 0.14, 0.05, "Siguiente", true, wBook)
...
function showBook(title, author, book, readOnly, slot, id)
    ...
    BookGUI.memo[1] = guiCreateMemo(9, 54, 413, 386, "Cargando datos...", false, BookGUI.window[1])
    BookGUI.label[1] = guiCreateLabel(7, 25, 81, 23, "Título:", false, BookGUI.window[1])
    guiSetFont(BookGUI.label[1], "default-bold-small")
    BookGUI.label[2] = guiCreateLabel(223, 25, 62, 25, "Por:", false, BookGUI.window[1])
    ...
    BookGUI.button[1] = guiCreateButton(9, 475, 413, 25, "Cerrar", false, BookGUI.window[1])
    guiSetProperty(BookGUI.button[1], "NormalTextColour", "FFAAAAAA")
    BookGUI.button[2] = guiCreateButton(10, 440, 191, 36, "Terminar libro y guardar", false, BookGUI.window[1])
    guiSetProperty(BookGUI.button[2], "NormalTextColour", "FFAAAAAA")
    BookGUI.button[3] = guiCreateButton(231, 440, 191, 36, "Guardar", false, BookGUI.window[1])
    guiSetProperty(BookGUI.button[3], "NormalTextColour", "FFAAAAAA")

    if title then
    	guiSetText(BookGUI.window[1], title.. " por "..author)
    ...
    addEventHandler("onClientGUIClick", BookGUI.button[2], function()
    	if string.find(guiGetText(BookGUI.edit[1]), ":") or string.find(guiGetText(BookGUI.edit[2]), ":") then
    		guiSetText(BookGUI.window[1], "No puedes usar ':' en el título o nombre de autor.")
    		return
    	end
