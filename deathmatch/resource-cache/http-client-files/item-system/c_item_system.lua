...
wWait = guiCreateButton( ( sx - 200 ) / 2, ( sy - 60 ) / 2, 200, 60, "Espera un momento...", false )
...
function showItemMenu(element, absX, absY)
	...
	if itemID == 81 or itemID == 103 then
		...
		bPickup = guiCreateButton(0.05, y, 0.9, 0.1, "Recoger el Item", true, wRightClick)
		addEventHandler("onClientGUIClick", bPickup, pickupItem, false)
	end
	y = y + 0.14

	if itemID == 54 then
		bPreviousTrack = guiCreateButton(0.05, y, 0.42, 0.15, "Anterior estacion", true, wRightClick)
		...
		bNextTrack = guiCreateButton(0.53, y, 0.42, 0.15, "Siguiente estacion", true, wRightClick)
	...
	bCloseMenu = guiCreateButton(0.05, y, 0.9, 0.1, "Cerrar menu", true, wRightClick)
...
addCommandHandler("itemprotect",
	function(command, f)
		...
		exports.hud:sendBottomNotification(localPlayer, "Proteccion de item", "Proteccion seteada a la facción: " .. ( fp == -100 and "admins only" or tostring(fp)))
	end
)
...
function pickupItem(button, state, item)
	if (button=="left") then
		...
		if (restrain) and (restrain==1) then
			exports.hud:sendBottomNotification(localPlayer, "No se pudo recoger el item", "Estás esposado.", 255, 0, 0)
		elseif getElementData(item, "itemID") > 0 and not hasSpaceForItem(localPlayer, getElementData(item, "itemID"), getElementData(item, "itemValue")) then
			exports.hud:sendBottomNotification(localPlayer, "No se pudo recoger el item", "Tu inventario está lleno. "..("%.2f/%.2f" ):format( getCarriedWeight( localPlayer ), getMaxWeight( localPlayer ) ).." kg(s)." , 255, 0, 0)
...
function copyClipboard()
	...
	elseif setClipboard(text) then
		exports.hud:sendBottomNotification(localPlayer, "Copiando", "Copiado '" .. text .. "' al borrador (usa CTRL + V para pegar).")
	else
		exports.hud:sendBottomNotification(localPlayer, "Copiando", "Error al copiar '" .. text .. "' al borrador.")
	end
end
...
function showInventory(player, type)
	if not (wItems) then
		...
		tabWallet = guiCreateTab("Billetera", tabPanel)
		tabItems = guiCreateTab("Items", tabPanel)
		tabKeys = guiCreateTab("Llaves", tabPanel)
		...
		colSlot = guiGridListAddColumn(gWallet, "Espacio", 0.1)
		colName = guiGridListAddColumn(gWallet, "Nombre", 0.225)
		...
		colSlot = guiGridListAddColumn(gItems, "Espacio", 0.1)
		colName = guiGridListAddColumn(gItems, "Nombre", 0.225)
		...
		colSlot = guiGridListAddColumn(gKeys, "Espacio", 0.1)
		colName = guiGridListAddColumn(gKeys, "Nombre", 0.625)
		...
		colWSlot = guiGridListAddColumn(gWeapons, "Espacio", 0.1)
		colWName = guiGridListAddColumn(gWeapons, "Nombre", 0.225)
		colID = guiGridListAddColumn(gWeapons, "Item ID", 0.1)
		colWValue = guiGridListAddColumn(gWeapons, "Munición", 0.625)

		-- GENERAL
		lDescription = guiCreateLabel(0.025, 0.87, 0.95, 0.1, "Haz click en un item para ver la descripción.", true, wItems)
		guiLabelSetHorizontalAlign(lDescription, "center", true)
		guiSetFont(lDescription, "default-bold-small")
		
		bClose = guiCreateButton(0.200, 0.91, 0.2, 0.15, "Cerrar inventario", true, wItems)
		addEventHandler("onClientGUIClick", bClose, hideInventory, false)
		
		bTakeItem = guiCreateButton(0.400, 0.91, 0.2, 0.15, "Tomar Item", true, wItems)
		addEventHandler("onClientGUIClick", bTakeItem, takeInventoryItem, false)

		bDeleteItem = guiCreateButton(0.600, 0.91, 0.2, 0.15, "Borrar Item", true, wItems)
	...
	if (button=="left") then
		if (guiGetSelectedTab(tabPanel)==tabItems or guiGetSelectedTab(tabPanel)==tabKeys) then -- ITEMS
			...
			if (row==-1) or (col==-1) then
				guiSetText(lDescription, "Haz click en un item para ver la descripción.")
			else
				...
				if not items[slot] then
					guiSetText(lDescription, "Un espacio vacío.")
				else
		...
		elseif (guiGetSelectedTab(tabPanel)==tabWeapons) then -- WEAPONS
			...
			if (row==-1) or (col==-1) then
				...
				local desc = "Un " .. name .. " con " .. ammo .. " municiones."
...
--function useItem(button)
	...
	if itemSlot > 0 then -- ITEMS
		...
		if itemID > 0 and not g_items[itemID] then
			exports.hud:sendBottomNotification(localPlayer, "Uso del Item", "Este item ya no está disponible.", 255, 194, 14)
			return
		end
		...
		elseif (itemID==6) then -- radio
			exports.hud:sendBottomNotification(localPlayer, "Uso del Item", "Presiona Y para usar este item. Utiliza /tuneradio para cambiar de frecuencia.", 255, 194, 14)
			return
		elseif (itemID==7) then -- phonebook
			exports.hud:sendBottomNotification(localPlayer, "Uso del Item", "Usa /phonebook para usar este item.", 255, 194, 14)
			return
		...
		elseif (itemID==30) or (itemID==31) or (itemID==32) or (itemID==33) then
			exports.hud:sendBottomNotification(localPlayer, "Uso del Item", "Usa el kit de química que puedes comprar en un 24/7 para usar este item.", 255, 0, 0)
			return
		--[[
		...
		elseif (itemID==45) or (itemID==46) or (itemID==47) or (itemID==66) then
			exports.hud:sendBottomNotification(source, "Uso del Item", "Haz clic con el botón derecho del ratón en un jugador para utilizar este elemento.")
			return
		elseif (itemID==50) or (itemID==51) or (itemID==52) then
			triggerServerEvent("useItem", localPlayer, itemSlot, additional)
			hideInventory()
		elseif (itemID==53) then -- Breathalizer
			exports.hud:sendBottomNotification(localPlayer, "Uso del Item", "Usa /breathtest para usar este item.")
			return
		elseif (itemID==55) then
			exports.hud:sendBottomNotification(localPlayer, itemName, "La tarjeta de negocio dice: " .. itemValue)
			return
		...
		elseif (itemID==61) then -- Emergency Light Becon
			exports.hud:sendBottomNotification(localPlayer, "Uso del Item", "Póngalo en el inventario de su coche y pulse 'P' para activarlo/desactivarlo.")
			return
		...
		elseif (itemID==67) then -- GPS
			exports.hud:sendBottomNotification(localPlayer, itemName, "Póngalo en el inventario de su coche y pulse 'F5'.")
			return
		elseif (itemID==70) then -- First Aid Kit
			exports.hud:sendBottomNotification(localPlayer, itemName, "Haz clic con el botón derecho del ratón sobre un jugador noqueado para estabilizarlo.")
			return
		elseif (itemID==71) then -- Notebook
			exports.hud:sendBottomNotification(localPlayer, itemName, "Usa /writenote [texto] para escribir una nota. Esto tiene " .. itemValue .. " páginas restantes.")
			return
		...
		elseif (itemID==78) then
			exports.hud:sendBottomNotification(localPlayer, itemName, "Esta licencia de piloto Riverside fue emitida para " .. itemValue .. ".")
			return
		elseif (itemID==81) then
			exports.hud:sendBottomNotification(localPlayer, itemName, "Tira este refrigerador en un interior")
			return
		elseif (itemID==84) then
			exports.hud:sendBottomNotification(localPlayer, itemName, "Póngalo en el coche o llévelo consigo para saber cuándo la policía está cerca.")
			return
		elseif (itemID==85) then -- Emergency Light Becon
			exports.hud:sendBottomNotification(localPlayer, itemName, "Póngalo en el coche y presiona 'N' para activar/desactivar.")
			return
		elseif (itemID==103) then
			exports.hud:sendBottomNotification(localPlayer, itemName, "Colocar esta estantería en un interior.")
			return
		...
		elseif itemID == 115 then
			...
			if info and info[1] == '46' then
				exports.hud:sendBottomNotification(localPlayer, itemName, "Desplázate hasta tu paracaídas mientras estás en el aire para utilizarlo.")
			elseif info and info[1] == '2' then
				triggerEvent("golf:start", root)
			else
				setClipboard(info[2])
				exports.hud:sendBottomNotification(localPlayer, itemName, "Serial del arma copiada al borrador (usa CTRL + V para pegar)")
		...
		elseif itemID == 116 then
			...
			if ammo_id and rounds then
				...
				if ammo and ammo.weapons then
					exports.hud:sendBottomNotification(localPlayer, ammo.cartridge..' '..itemName, "Cartucho: "..ammo.cartridge..", Estilo de bala: "..ammo.bullet_style..", Peso de la bala: "
						..exports.global:round(ammo.bullet_weight*1000,2).." gramos, Aplicación: "..ammo.application..", Compatible con "..exports.weapon:formatWeaponNames(ammo.weapons)..".")
				end
			end
			return
		elseif itemID == 117 then
			exports.hud:sendBottomNotification(localPlayer, itemName, "Por favor, coloque este elemento en un vehículo, y luego contrólelo a través del menú del botón derecho del ratón de los vehículos.")
			return
		elseif itemID == 174 then
			triggerServerEvent("startFAAmapGUI", localPlayer)
			return
		elseif (itemID==118) then
			exports.hud:sendBottomNotification(localPlayer, itemName,  "Póngalo en su coche y acérquese a un peaje.")
			return
		elseif itemID == 121 then
			exports.hud:sendBottomNotification(localPlayer, itemName, "Una caja pesada llena de suministros!")
			return
		...
		elseif (itemID==147) then -- Picture Frame --  Adams
			if(itemValue and itemValue ~= 1) then
				...
				if(#split > 1) then
					exports.hud:sendBottomNotification(localPlayer, itemName, "Coloque esto en un interior para reemplazar la textura '"..tostring(split[2]).."'.")
				else
					local url = itemValue
		...
		elseif (itemID==150) then -- ATM card
			...
			if tonumber( itemExploded[2] ) > 0 then
				exports.hud:sendBottomNotification(localPlayer, g_items[itemID][1] , "Número de Tarjeta: '"..itemExploded[1].."', Dueño: '"..tostring(exports.cache:getCharacterNameFromID(itemExploded[2])):gsub("_", " ").."'. Arrastra y suelta en un ATM cercano (Cajero).")
			else
				exports.hud:sendBottomNotification(localPlayer, g_items[itemID][1] , "Número de Tarjeta: '"..itemExploded[1].."', Dueño: '"..tostring(exports.cache:getBusinessNameFromID(math.abs(itemExploded[2]))).."'. Arrastra y suelta en un ATM cercano (Cajero).")
			end
			return
		elseif (itemID==152) then -- ID card
			local itemExploded = explode(";", itemValue )
			local text = "Nombre '"..itemExploded[1]:gsub("_", " ").."', Género: '"..itemExploded[2].."', DoB: '"..itemExploded[3].."', Huellas dactilares: '"..itemExploded[4].."' "
		...
		elseif (itemID==133) or (itemID==153) or (itemID==154) or (itemID==155)then
			exports.hud:sendBottomNotification(localPlayer, itemName, "Este "..itemName.." fue emitido para " .. itemValue .. ".")
			return
		elseif (itemID==169) then -- Keypad door lock
			exports.hud:sendBottomNotification(localPlayer, g_items[itemID][1].." - Equipamiento para el interior ID #"..itemValue , "coloque este dispositivo a una pared estable para instalarlo. Puede desinstalar este dispositivo y volver a instalarlo libremente más adelante.")
			return
		elseif (itemID==170) then -- Keycard
			exports.hud:sendBottomNotification(localPlayer, itemName, "Pase esta tarjeta en "..tostring(itemValue).." para acceder.")
			return
		...
		elseif (itemID==211) then -- Christmas Lottery Ticket
			outputChatBox("Prueba a darle el billete a Papá Noel para ver qué has ganado.", 255, 194, 14)
			return
		elseif (itemID==212) then -- Snow Tire
			outputChatBox("Ponga esto en su vehículo para mejorar la tracción en la nieve.", 155, 255, 155)
			return
		elseif itemID == 232 then -- car battery charger
			for i, veh in pairs( exports.global:getNearbyElements( localPlayer, 'vehicle', 5 ) ) do
				if veh and isElement(veh) then
					...
					if #doors > 0 and doors[1] and doors[1][1] == 'Hood' and getVehicleDoorOpenRatio ( veh, 0 ) < 0.5 then
						exports.hud:sendBottomNotification(localPlayer, itemName, "Abre el capó del vehículo primero." )
					elseif isVehicleLocked ( veh ) then
						exports.hud:sendBottomNotification(localPlayer, itemName, "Desbloquea el vehículo primero." )
					else
			...
			exports.hud:sendBottomNotification(localPlayer, itemName, "No hay ningún vehículo a su alrededor." )
		else
			triggerServerEvent("useItem", localPlayer, itemSlot, additional)
		end
	else
		if itemSlot == -100 then
			exports.hud:sendBottomNotification(localPlayer, itemName, "Llevas una armadura corporal.")
		else
