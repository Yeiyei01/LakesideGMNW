...
function newCharacter_init()
	...
	gui["lblCharNameExplanation"] = guiCreateLabel(10, 40, 240, 80,"Necesita estar en este orden: \n     Nombre Apellido\nPor ejemplo: Jonathan Conway.\nNo puedes usar nombres de famosos.", false, gui["_root"])
...
--[[	gui["lblCharDesc"] = guiCreateLabel(10, 125, 230, 100, "When you first spawn type /editlook and describe your character in more detail.", false, gui["_root"])
	...
	gui["lblGender"] = guiCreateLabel(10, 160, 46, 13, "Género:", false, gui["_root"])
	...
	gui["lblRace"] = guiCreateLabel(10, 140, 111, 16, "Razgos:", false, gui["_root"])
	...
	gui["chkAsian"] =  guiCreateCheckBox ( 180, 140, 55, 16, "Asiático", false, false, gui["_root"] )
	...
	gui["lblDay"] = guiCreateLabel(10, 282, 111, 16, "Fecha de nacimiento:", false, gui["_root"])
	...
	gui["lblMonth"] = guiCreateLabel(10, 260, 111, 16, "Mes de nacimiento", false, gui["_root"])
	guiLabelSetHorizontalAlign(gui["lblDay"], "left", false)
	guiLabelSetVerticalAlign(gui["lblDay"], "center")
	...
	guiComboBoxAddItem(gui["drpMonth"], "Mayoo")
	...
	--gui["btnLanguagePrev"] = guiCreateButton(110, 305, 16, 16, "<", false, gui["_root"])-- Desactivado por Dreko.
	...
	--gui["btnLanguageNext"] = guiCreateButton(224, 305, 16, 16, ">", false, gui["_root"])-- Desactivado por Dreko.
	...
	--gui["btnLangs"] = guiCreateButton(10, 330, 231, 41, "All Languages", false, gui["_root"])-- Desactivado por Dreko.
...
function newCharacter_viewAllLangs(button)
	if (source == gui["btnLangs"] and button == "left") then
		...
		guiGridListAddColumn(gui['langWindowGrid'], "Idiomas", 0.9)
...
function newCharacter_updateScrollBars()
	...
	--scrAge = math.floor( (scrAge * 0.8 ) + 16 )
	guiSetText(gui["lblAge"], "Años: "..scrAge.." años")
	...
	guiSetText(gui["lblDay"], "Dia: "..(scrDay or "1"))
end
...
function newCharacters_getSkinArray()
	...
	if (guiCheckBoxGetSelected( gui["chkBlack"] )) then -- BLACK
		...
		elseif (guiRadioButtonGetSelected( gui["rbFemale"] )) then -- BLACK FEMALE
			...
			outputChatBox("Selecciona un genero primero", 0, 255, 0)
		end
...
function newCharacter_attemptCreateCharacter()
	...
	if not nameCheckPassed then
		LoginScreen_showWarningMessage( "Error procesando el nombre de tu personaje:\n".. nameCheckError )
		return
	end
	--[[
	...
	elseif (guiCheckBoxGetSelected(gui["chkAsian"])) then
		...
		LoginScreen_showWarningMessage( "Error procesando la raza de tu personaje:\nNinguno seleccionado." )
		return
	end
	...
	elseif (guiRadioButtonGetSelected( gui["rbFemale"] )) then
		...
		LoginScreen_showWarningMessage( "Error procesando el genero de tu personaje:\nNinguno seleccionado." )
		return
	end

	local skin = getElementModel( dummyPed )
	if not skin then
		LoginScreen_showWarningMessage( "Error procesando la skin de tu personaje:\nNinguno seleccionado." )
		return
	end
	...
	if languageselected == nil then
		LoginScreen_showWarningMessage( "Error procesando el idioma de tu personaje:\nNinguno seleccionado." )
		return
	end
...
function newCharacter_response(statusID, statusSubID)
	if (statusID == 1) then
		LoginScreen_showWarningMessage( "Algo no funcionó. Intenta denuevo\no contacta un administrador.\nError ACC"..tostring(statusSubID) )
	elseif (statusID == 2) then
		if (statusSubID == 1) then
			LoginScreen_showWarningMessage( "El nombre\nestá en uso" )
		else
...
function selectStartPointGUI(characterName, characterDescription, race, gender, skin, scrollHeight, scrWeight, scrAge, languageselected, selectedMonth, scrDay )
	...
	local locations = {
		...
		["default"] = {282.681640625, -64.18359375, 1.578125, 178.89587402344, 0, 0, "A bus stop near the mall"},
