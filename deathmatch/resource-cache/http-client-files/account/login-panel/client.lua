...
function startLoginSound()
	...
	if setting == 0 then
		...
		local sound = 1
		local bgMusic = playSound ( "login-panel/"..random..".mp3", true )
		if bgMusic then
...
function startLoggingIn()
	if not getElementData(localPlayer, "clickedLogin") then
		...
		authen_msg("Login", "Enviando peticion al servidor..")
	else
		Error_msg("Login", "Paciencia..")
	end
end
...
function registerValidation(username, password, passwordConfirm, email)
	if not username or username == "" or not password or password == "" or not passwordConfirm or passwordConfirm == "" or not email or email == ""  then
		guiSetText(panel.login.toplabel, "Por favor, llena todos los campos vacíos.")
		guiLabelSetColor ( panel.login.toplabel, 255, 0, 0 )
		playSoundFrontEnd ( 4 )
	elseif string.len(username) < 3 then
		guiSetText(panel.login.toplabel, "El nombre de usuario debe ser de 3 caracteres como mínimo.")
		guiLabelSetColor ( panel.login.toplabel, 255, 0, 0 )
		playSoundFrontEnd ( 4 )
	elseif string.len(username) >= 19 then
		guiSetText(panel.login.toplabel, "El nombre de usuario no puede tener más de 20 caracteres.")
		guiLabelSetColor ( panel.login.toplabel, 255, 0, 0 )
		playSoundFrontEnd ( 4 )
	elseif string.find(username, ' ') then
		guiSetText(panel.login.toplabel, "Nombre de usuario inválido.")
		guiLabelSetColor ( panel.login.toplabel, 255, 0, 0 )
		playSoundFrontEnd ( 4 )
	...
	elseif string.len(password) < 8 then
		guiSetText(panel.login.toplabel, "La contraseña debe ser de 8 caracteres como mínimo.")
		guiLabelSetColor ( panel.login.toplabel, 255, 0, 0 )
		playSoundFrontEnd ( 4 )
	elseif string.len(password) > 25 then
		guiSetText(panel.login.toplabel, "La contraseña no puede tener más de 25 caracteres.")
		guiLabelSetColor ( panel.login.toplabel, 255, 0, 0 )
		playSoundFrontEnd ( 4 )
	...
	elseif string.match(username,"%W") then
		guiSetText(panel.login.toplabel, "Los símbolos no están permitidos (\"!@#$\"%'^&*()\").")
		guiLabelSetColor ( panel.login.toplabel, 255, 0, 0 )
		playSoundFrontEnd ( 4 )
	else
		...
		if not validEmail then
			...
			authen_msg("Register", "Enviando peticion al servidor.")
		end
	end
end
...
function displayRegisterConpleteText(username)
	...
	GUIEditor.window[1] = guiCreateWindow(667, 381, 357, 189+extend, "¡Hecho! La cuenta se ha creado correctamente", false)
    ...
    local temp = "Bienvenido a Lakeside RP, prometemos que lo pasarás bien; recuerda entrar en el discord clickeando abajo a la derecha"
    GUIEditor.label[1] = guiCreateLabel(8, 50, 339, 121+extend, "Tu cuenta '"..username.."' está lista para ser usada\n\n"..temp.."\n\nAtentamente, \nDesarroladores de Lakeside RP\"", false, GUIEditor.window[1])
    guiLabelSetHorizontalAlign(GUIEditor.label[1], "left", true)

    GUIEditor.button[1] = guiCreateButton(10, 153+extend, 337, 26, "Aceptar", false, GUIEditor.window[1])
...
function screenStandBy(action, value) -- Maxime / 2015.3.25
	if action == "add" then
		...
		if screenStandByShowing then
			authen_msg("Login", "Cargando recursos necesarios.."..screenStandBy("getPercentage").."%")
		end
	...
	elseif action == "getPercentage" then
		...
		if screenStandByShowing then
			authen_msg("Login", "Cargando recursos necesarios.."..percentage.."%")
		end
		return percentage
	end
end
