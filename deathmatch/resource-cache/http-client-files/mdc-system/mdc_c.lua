...
function getAccountData( )
	local file = xmlLoadFile( filePath )
	if not file then
		return { username = "PoliciaLSPD", password = "PoliciaLSPD" }
	end
	local username = xmlNodeGetValue( xmlFindChild( file, "username", 0 ) ) or "PoliciaLSPD"
	local password = xmlNodeGetValue( xmlFindChild( file, "password", 0 ) ) or "PoliciaLSPD"
...
function login ( )
	if hasMDCPermissions() then
		...
		window.rememberCheck = guiCreateCheckBox( 10, 110, width - 20, 30, "Recordarme", accountData.username ~= "Username", false, window.window )
		...
		outputChatBox( "No estas en una unidad con computadora.", 255, 155, 155 )
	end
end

------------------------------------------
addCommandHandler( "computadorapolicial", login, false, false )
...
function createLoginWindow()
	...
	local x = SCREEN_X / 2 - width / 2
	local y = SCREEN_Y / 2 - height / 2
	window.window = guiCreateWindow( x, y, width, height, "Computadora Policial", false )
