...
function main ( warrants, apb, calls )
	...
	window.window = guiCreateWindow( x, y, width, height, "MDC Main", false )
	...
	--Search Button
	window.searchButton = guiCreateButton( button.x, button.y, button.width, button.height, "Search Database", false, window.window )
	...
	window.addButton = guiCreateButton( button.x, button.y, button.width, button.height, "Add APB", false, window.window )
	...
	button.x = button.x + button.width + spacer
	window.accountButton = guiCreateButton( button.x, button.y, button.width, button.height, "Account Settings", false, window.window )
	...
	window.apbTab		= guiCreateTab( "APB", window.mainPanel )
	window.apbTable		= guiCreateGridList ( 10, 10, width - 35, height - 190, false, window.apbTab )
	
	window.personCol	= guiGridListAddColumn( window.apbTable, "Person", 0.25 )
	window.wantedCol	= guiGridListAddColumn( window.apbTable, "APB", 0.5 )
	window.issuedByCol	= guiGridListAddColumn( window.apbTable, "Issued By", 0.25 )
	...
	if ( #apb > 0 ) then
		...
		local row = guiGridListAddRow ( window.apbTable )
		guiGridListSetItemText ( window.apbTable, row, window.personCol, "No APBs", false, false )
	end
	
	
	window.warrantTab	= guiCreateTab( "Warrants", window.mainPanel )
	window.warrantTable	= guiCreateGridList ( 10, 10, width - 35, height - 190, false, window.warrantTab )
	window.charCol		= guiGridListAddColumn( window.warrantTable, "Suspect", 0.25 )
	window.warrantCol	= guiGridListAddColumn( window.warrantTable, "Warrant", 0.45 )
	window.issuedCol	= guiGridListAddColumn( window.warrantTable, "Issued By", 0.25 )
	...
	if ( #warrants > 0 ) then
		...
		local row = guiGridListAddRow ( window.warrantTable )
		guiGridListSetItemText ( window.warrantTable, row, window.charCol, "No Warrants", false, false )
	end
	
	
	window.callsTab			= guiCreateTab( "911 Calls", window.mainPanel )
	window.callsTable		= guiCreateGridList( 10, 10, width - 35, height - 190, false, window.callsTab )
	window.callerCol		= guiGridListAddColumn( window.callsTable, "Caller", 0.2 )
	window.phoneCol			= guiGridListAddColumn( window.callsTable, "Phone Number", 0.12 )
	window.convoCol			= guiGridListAddColumn( window.callsTable, "Description", 0.5 )
	window.timeCol			= guiGridListAddColumn( window.callsTable, "Time", 0.1 )
	...
	if ( #calls > 0 ) then
		...
		local row = guiGridListAddRow ( window.callsTable )
		guiGridListSetItemText ( window.callsTable, row, window.callerCol, "No 911 Calls", false, false )
	end
