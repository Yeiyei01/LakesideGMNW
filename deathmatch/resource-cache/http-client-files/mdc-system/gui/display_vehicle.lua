...
function display_vehicle ( id, model, color1, color2, color3, color4, plate, faction, owner, owner_type, impounded, crimes )
	...
	window.window = guiCreateWindow( x, y, width, height, "MDC Search - Vehicle: ".. plate, false )
	
	window.nameLabel	= guiCreateLabel( 10, 30, 180, 20, "Vehicle: ", false, window.window )
	window.plateLabel 	= guiCreateLabel( 10, 50, 180, 20, "Plate: ", false, window.window )
	window.primaryLabel	= guiCreateLabel( 10, 70, 180, 20, "Primary Color: ", false, window.window )
	window.secondLabel	= guiCreateLabel( 10, 90, 180, 20, "Secondary Color: ", false, window.window )
	window.vinLabel		= guiCreateLabel( 10, 110, 180, 20, "VIN Number: ", false, window.window )
	window.ownerLabel	= guiCreateLabel( 10, 130, 220, 20, "Owner: ", false, window.window )
	window.impoundLabel	= guiCreateLabel( 10, 150, 220, 20, "Impounded: ", false, window.window )
	...
	window.impound2Label	= guiCreateLabel( 105, 150, 220, 20, ( tonumber( impounded ) == 1 and "Yes" or "No" ), false, window.window )
	
	window.mainPanel	= guiCreateTabPanel ( 10, 190, width - 20, 270, false, window.window )
	window.crimesTab	= guiCreateTab( "Speeding Violations", window.mainPanel )
	...
	window.speedCol		= guiGridListAddColumn( window.crimesTable, "Speed", 0.15 )
	window.locCol		= guiGridListAddColumn( window.crimesTable, "Location", 0.25 )
	window.personCol	= guiGridListAddColumn( window.crimesTable, "Person", 0.3 )
	...
	if ( #crimes > 0 ) then
		...
		local row = guiGridListAddRow ( window.crimesTable )
		guiGridListSetItemText ( window.crimesTable, row, window.dateCol, "No Speeding Violations", false, false )
	end
