...
local tstgs = {
  text = "Presiona '"..bdKey[ 1 ].."' para entrar en la cabina como pasajero",       -- Text that will show up when cabin gets closer to a player (Default, change it below also, lines 33 and 65)
  ...
  for i, col in ipairs( t ) do
    addEventHandler( 'onClientColShapeHit', col, function( player, dim )
      if player == localPlayer and dim then
        ...
        tstgs.text = "Presiona '"..bdKey[ 1 ].."' para entrar en la cabina";
...
function getInside( key, state, col )
  ...
  tstgs.text = "Presiona '"..bdKey[ 2 ].."' para salir de la cabina";
  unbindKey( bdKey[ 1 ], 'down', getInside );
