...
function deactivateCruiseControl()
    ...
    setPedControlState(localPlayer, "accelerate",false)
    ccEnabled = false
    exports.hud:sendBottomNotification(localPlayer, "Control Crucero", "Control Crucero Desactivado.")
end
				...
				if (ccEnabled) then
					...
					if targetSpeed > 10 then
						if (getVehicleType(theVehicle) == "Automobile" or getVehicleType(theVehicle) == "Bike" or getVehicleType(theVehicle) =="Boat" or getVehicleType(theVehicle) == "Train" or getVehicleType(theVehicle) == "Plane" or getVehicleType(theVehicle) == "Helicopter") or specialVehicles[getElementModel(theVehicle)] then
							exports.hud:sendBottomNotification(localPlayer, "Control Crucero Activado", "Utilice - y + para ajustar la velocidad.")
							activateCruiseControl()
						end
					else
                        exports.hud:sendBottomNotification(localPlayer, "Control Crucero", "El Control Crucero se utiliza para mantener la velocidad, no para subirla.")
					end
				end
            else
                exports.hud:sendBottomNotification(localPlayer, "Control Crucero", "¡El motor no está encendido!")
			end
		end
	end
end
