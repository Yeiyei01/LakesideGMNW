local a = dxCreateShader("assets/treeshader.fx")
local b = dxCreateTexture("assets/alpha.png")
dxSetShaderValue(a, "gTexture", b)
...
addEventHandler("onClientResourceStart", resourceRoot,
	function()
		...
		for g, h in ipairs(getElementsByType("object")) do
			local i = getElementModel(h)
			engineSetModelLODDistance(i, 100)
		end
	end
)
...
function increase()
	...
	for j, k in ipairs(d) do
		engineApplyShaderToWorldTexture(a, k)
	end
	...
	setFarClipDistance(c.far)
	setFogDistance(c.far - 175)
	...
	for l, m in pairs(c.graphic) do
		engineApplyShaderToWorldTexture(SHADER, tostring(m))
	end
...
local n, o = guiGetScreenSize()
local p = 256
local q = 1

function handleMinimize()
	eightBitShader = dxCreateShader("assets/8bit.fx")
	screenSource = dxCreateScreenSource(n, o)
	setCloudsEnabled(false)
	setHeatHaze(0)
	setInteriorSoundsEnabled(false)
	setMoonSize(0)
	...
	eightBitShader:setValue("bitDepth", p)
	eightBitShader:setValue("tiles", r)
	dxDrawImage(0, 0, n, o, eightBitShader)
end
